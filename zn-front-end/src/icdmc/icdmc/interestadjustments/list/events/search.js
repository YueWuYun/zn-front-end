 ...
 */
export function searchBtnClick (props, condition, type, querycondition, isToast= true, isRefresh= false) {
    if (!condition) {//刷新按钮
        condition= deepClone(getDefData(this.searchKey, this.dataSource));//缓存的查询条件{需要先克隆一下}
        if (!condition) {
            toast({color: 'warning', content: this.state.json['36360ICIA-000108']});/* 国际化处理： 请选择必输的查询条件!*/
            return;
        }
    } else {//查询按钮
...
export function onTabChange (status) {
    ...
    let def1;
    if (!searchVal) {
       // searchVal = this.props.search.getQueryInfo(this.searchId, false).querycondition;
       // def1 = "true";
        return;
    }
...
function listRender (res, isToast, isRefresh) {
    ...
    if (success && data && data.grid && data.grid[this.tableId]) {
        ...
        if (isToast) {
            if (flag) {
                if(isRefresh){
                    toast({color: 'success', content: this.state.json['36360ICIA-000033']});/* 国际化处理： 刷新成功!*/
                } else {
                    toast({color: 'success', content: this.state.json['36360ICIA-000026']});/* 国际化处理： 查询成功!*/
                }
            } else {
                toast({color: 'warning', content: this.state.json['36360ICIA-000027']});/* 国际化处理： 未查询出符合条件的数据!*/
            }
        }
    } else {
