...
import { list, card, gotocardcheck, btnLimit ,linkdataSource, islink,pkName} from "../../cons/constant.js";
import {
        ...
        data => {
            if (data) {
                if (data.template) {
                    ...
                    linkVoucher.call(this, props, "repayintstreceipt");
                }
                if (data.button) {
...
if (!getDefData(linkdataSource, islink)) {
    ...
    meta[this.tableId].items.push({
        attrcode: "opr",
        label: this.state.json['36362IPIR-000009'],/* 国际化处理： 操作*/
        width: 200,
        fixed: "right",
        itemtype: "customer",
        className: "table-opr",
        ...
        render: (text, record, index) => {
            let buttonAry = [];
            let tallyflag = record.tallyflag && record.tallyflag.value; //制证、记账同步 所以判断哪一个都行
            if(!getDefData(linkdataSource, islink)){
            if (!tallyflag) {//没有记账 显示记账
