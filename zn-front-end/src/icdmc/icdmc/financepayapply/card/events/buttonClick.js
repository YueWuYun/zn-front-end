...
import {linkFinancepay} from "../../../public/listHeadBtnClick";
import {
    fileMgr,
    setEditStatus,
    getCardData,
...
export function buttonClick(props, id) {
    ...
    switch (id) {
        ...
        //头部保存
        case "save":
            cardSave.call(this, props, "browse", javaUrl.save, this.state.json['36362IAP-000013']);/* 国际化处理： 保存成功*/
            break;
        //头部新增保存
        case "saveAdd":
            cardSaveAdd.call(this, props, "add", javaUrl.save, this.state.json['36362IAP-000013']);/* 国际化处理： 保存成功*/
            break;
        // 头部保存提交
        case "saveCommit":
        ...
        case "delete":
            promptBox({
                ...
                content: this.state.json['36362IAP-000014'],/* 国际化处理： 确定要删除么？*/
                beSureBtnClick: () => {
                    cardBtnOperation.call(
                        this,
                        `${baseReqUrl}${javaUrl.delete}`,
                        this.state.json['36362IAP-000015']/* 国际化处理： 删除成功!*/
                    );
                }
            });
            break;
        ...
        case "cancel":
            promptBox({
                color: "warning",
                title: this.state.json['36362IAP-000016'],/* 国际化处理： 取消*/
                content: this.state.json['36362IAP-000017'],/* 国际化处理： 确定要取消么？*/
                beSureBtnClick: () => {
        ...
        case "commit":
            cardBtnOperation.call(
                ...
                this.state.json['36362IAP-000018']/* 国际化处理： 提交成功!*/
            );
            break;
        //头部 收回
        case "unCommit":
            cardBtnOperation.call(
                ...
                this.state.json['36362IAP-000019']/* 国际化处理： 收回成功!*/
            );
            break;
        //头部 附件
        case "Attachment":
        ...
        //头部 联查放款单
        case "financepay":
            linkFinancepay.call(this, props, pk);
            break;
        default:
            break;
    }
...
function cardSaveCommit(props, type, url, content) {
    saveBefore.call(this, () => {
        saveBill.call(this, props, type, url, content).then(id => {
            ...
            // getCardData.call(this, this.cardUrl, id);
            // setEditStatus.call(this, "browse");
            cardBtnOperation.call(this, this.cardCommitUrl, this.state.json['36362IAP-000022']);/* 国际化处理： 保存提交成功!*/
        });
    });
}
/**
