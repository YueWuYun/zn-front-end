			...
			() => {
				if (key == "tryIntst") {
					...
					BusinessOperatorHead.call(this, key,
						this.state.json['36362IWI-000012'],
						TryCalInst_URL,
						this.state.json['36362IWI-000013']
					);
				} else {
					// 预提
					BusinessOperatorHead.call(this, key,
						this.state.json['36362IWI-000008'],
						PreCalInst_URL,
						this.state.json['36362IWI-000009']
					);
				}
			}
		);
			...
			success: (res) => {
				...
				if (success) {
					if (data.grid.table.rows.length > 0) {
						...
						toast({
                            duration: 3,
                            title: this.props.MutiInit.getIntl("36362IWI") && this.props.MutiInit.getIntl("36362IWI").get('36362IWI-000027') /**国际化处理：刷新成功 */,
                            color: 'success'
                        })
					} else {
						this.props.table.setAllTableData(tableId, { rows: [] });
						toast({ color: 'warning', content: this.props.MutiInit.getIntl("36362IWI") && this.props.MutiInit.getIntl("36362IWI").get('36362IWI-000025') });
					}
				}
			...
			<div className="nc-bill-list">
                ...
                <NCDiv areaCode={NCDiv.config.HEADER}  className="nc-bill-header-area">
					<div className="header-title-search-area">
						{createPageIcon()}
						<h2 className="title-search-detail">{this.props.MutiInit.getIntl("36362IWI") && this.props.MutiInit.getIntl("36362IWI").get('36362IWI-000000')}</h2>
					</div>
				...
				<BeginEndDateModal
					...
					begin={{
						label: this.state.json["36362IWI-000016"]
					}} /* 国际化处理： 试算开始日期*/
					end={{
						label: this.state.json["36362IWI-000017"]
					}} /* 国际化处理： 试算结束日期*/
				...
				{this.state.trycalData && (
					<TrycalModal
						...
						title={
							this.state.json["36362IWI-000018"]
						} /* 国际化处理： 试算结果*/
				...
				>
					<NCModal.Header>
						<NCModal.Title>{this.state.json["36362IWI-000008"]}</NCModal.Title>
					</NCModal.Header>
	...
	trycontent () {
		return (
			<div className="addModal">
				...
				<span className="modal-label">{this.state.json['36362IWI-000016']/**国际化处理：开始日期 */}</span>
				<NCDatePicker
					fieldid="startdate"
					format={dateFormat}
					placeholder={this.state.json['36362IWI-000016']/**国际化处理：开始日期 */}
				...
				<span className="modal-label">
					{this.state.json['36362IWI-000017']/**国际化处理：结束日期 */}
				</span>	
				<NCDatePicker
					fieldid="enddate"
					format={dateFormat}
					placeholder={this.state.json['36362IWI-000017']/**国际化处理：结束日期 */}
