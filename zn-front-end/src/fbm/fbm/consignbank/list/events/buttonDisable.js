/*QCRu/PDcDggPUTbrjwK8iGzRr5RUvPADxW/a2/qUudO8fia6NJPh3NZjkYlHpo8+*/
import { ajax, base, toast } from 'nc-lightapp-front';
import { LIST ,btns,BANKINFO} from '../../cons/constant.js';
import {  selectedEvent} from './index';


export function buttonDisable () {
    selectedEvent.call(this);
    let tabKey = this.state.activeTab;
    let disBtn = [];
    let selectDatas = this.props.table.getCheckedRows(this.tableId);
    if (selectDatas.length == 1) {
        // if (tabKey == 'all') {
        // // let disabledBtn = this.disabledBtn.filter(item => item !== 'Refresh');
        //     return;
        // }
        // 此处不需要处理,在selectedEvent处理了
        // if (selectDatas.length > 1) {
        //     this.props.button.setButtonDisabled(LIST.disabled_btn, false);
        //     return;
        // }
        // 进行遍历一下
        let hasapprove = false;
        let hasnet = false;
        let hasDis = false;
        let hasDisBill = false;
        let hasInit= false;
        let hasNetSucc=false;
        let hasVoucher=false;
        let item = selectDatas[0];
        // console.log(item);
        let vbillstatus = item.data.values.vbillstatus.value;
        let paymentstatus = item.data.values.paymentstatus.value;
        let onlinebankflag = item.data.values.onlinebankflag.value;
        let disableflag = item.data.values.disableflag.value;
        let disableflagbill = item.data.values["pk_register.disableflag"] && item.data.values["pk_register.disableflag"].value;//票据作废
        let fbmbilltype = item.data.values["pk_register.fbmbilltype"] && item.data.values["pk_register.fbmbilltype"].value;//票据作废
        let voucher = item.data.values.voucher.value;
        let initflag = item.data.values.initflag.value;
        let busistatus = item.data.values.busistatus.value;
        if (vbillstatus!=1) {
            // 待提交
            hasapprove = true;
        }else if (vbillstatus==1) {
            // 审批通过
            if (onlinebankflag) {
                // 有网银
                hasnet = true;
                if (paymentstatus &&　paymentstatus==1) {
                    // 交易成功
                    hasNetSucc = true;
                }
                if (disableflag) {
                    hasDis = true;
                }
            }else{
                if (voucher) {
                    hasVoucher = true;
                }
                
            }
            if ((fbmbilltype == 'ebank' || fbmbilltype == 'ebusi')) {
                disBtn.push(btns.consignBankDisable);
                disBtn.push(btns.consignCancelDisable);
            } else {
                if(disableflagbill  !== 'N'){
                    disBtn.push(btns.consignBankDisable);
                }else{
                    disBtn.push(btns.consignCancelDisable);
                }
               
            }
           
        }
        if (!hasapprove) {
            // 审批通过的
            disBtn.push(btns.commitBtn);
            if (!hasnet) {
                // 非网银
                disBtn.push(btns.sendCommandBtn);
                disBtn.push(btns.takeCommandBtn);
                disBtn.push(btns.disabledBtn);
                disBtn.push(btns.unDisabledBtn);
                if (!hasDis) {
                    disBtn.push(btns.unDisabledBtn);
                }else{
                    disBtn.push(btns.disabledBtn);
                }
                if ((fbmbilltype == 'ebank' || fbmbilltype == 'ebusi')) {
                    disBtn.push(btns.consignBankDisable);
                    disBtn.push(btns.consignCancelDisable);
                } else {
                    if(disableflagbill  !== 'N'){
                        disBtn.push(btns.consignBankDisable);
                    }else{
                        disBtn.push(btns.consignCancelDisable);
                    }
                   
                }
                
            }else{
                // 网银的
                if (!hasDis) {
                    disBtn.push(btns.unDisabledBtn);
                }else{
                    disBtn.push(btns.disabledBtn);
                }
               
                if (hasNetSucc) {
                    // 支付成功
                    disBtn.push(btns.sendCommandBtn);
                }else{
                    disBtn.push(btns.takeCommandBtn);
                }
                disBtn.push(btns.consignBankDisable);
                disBtn.push(btns.consignCancelDisable);
            }
            if (!hasVoucher) {
                // 非已制证
                disBtn.push(btns.linkVoucherBtn);
               
                // if(!hasnet){
                //     if (busistatus === 'has_disable') {//已办理托收    
                //         disBtn.push(btns.consignBankDisable);
                //     }else if (busistatus === 'has_collection'){
                //         disBtn.push(btns.consignCancelDisable);
                //     }else{
                //         disBtn.push(btns.consignBankDisable);
                //         disBtn.push(btns.consignCancelDisable);
                //     }
                // }else{
                //     disBtn.push(btns.consignBankDisable);
                //     disBtn.push(btns.consignCancelDisable);
                // }
               
            }else{
                // 已制证
                disBtn.push(btns.uncommitBtn);
                disBtn.push(btns.consignBankDisable);
                disBtn.push(btns.consignCancelDisable);
            }
        }else{
            // 待提交的，已提交未审批通过的
            if (vbillstatus==-1) {
                disBtn.push(btns.linkApproveBtn);
                disBtn.push(btns.uncommitBtn);
            }else{
                disBtn.push(btns.commitBtn);
            }
            // 非网银
            disBtn.push(btns.sendCommandBtn);
            disBtn.push(btns.takeCommandBtn);
            disBtn.push(btns.disabledBtn);
            disBtn.push(btns.unDisabledBtn);
            disBtn.push(btns.linkVoucherBtn);
            disBtn.push(btns.consignBankDisable);
            disBtn.push(btns.consignCancelDisable);
        }
        this.props.button.setButtonDisabled(disBtn, true);
        // return;
    }
    if (tabKey == '-1') {
        // 待提交
        disBtn.push(btns.uncommitBtn);
        disBtn.push(btns.sendCommandBtn);
        disBtn.push(btns.takeCommandBtn);
        disBtn.push(btns.disabledBtn);
        disBtn.push(btns.unDisabledBtn);
        disBtn.push(btns.linkVoucherBtn);
        disBtn.push(btns.linkApproveBtn);
        disBtn.push(btns.consignBankDisable);
        disBtn.push(btns.consignCancelDisable);

    }else if (tabKey == '2,3') {
        // 审批中
        disBtn.push(btns.deleteBtn);
        disBtn.push(btns.commitBtn);
        disBtn.push(btns.sendCommandBtn);
        disBtn.push(btns.takeCommandBtn);
        disBtn.push(btns.disabledBtn);
        disBtn.push(btns.unDisabledBtn);
        disBtn.push(btns.linkVoucherBtn);
        disBtn.push(btns.consignBankDisable);
        disBtn.push(btns.consignCancelDisable);
    }else if (tabKey == '10') {
        // 支付处理中
        disBtn.push(btns.deleteBtn);
        disBtn.push(btns.commitBtn);
        disBtn.push(btns.uncommitBtn);
        disBtn.push(btns.linkVoucherBtn);
        disBtn.push(btns.consignBankDisable);
        disBtn.push(btns.consignCancelDisable);
    }
    this.props.button.setButtonDisabled(disBtn, true);




}
/*QCRu/PDcDggPUTbrjwK8iGzRr5RUvPADxW/a2/qUudO8fia6NJPh3NZjkYlHpo8+*/