...
import { card } from '../../../../public/container';
export default function (props) {
	let app_code = props.getSearchParam('c');
	let excelimportconfig = excelImportconfig(props, "fbm", '36H5', true, "", { "appcode": app_code, "pagecode": CARD.page_id });
		...
		(data) => {
			if (data) {
				if (data.button) {
					...
					props.button.setPopContent('DeleteInner', this.props.MutiInit.getIntl("36180BC") && this.props.MutiInit.getIntl("36180BC").get('36180BC-000004'));/* 国际化处理： 确认要删除吗?*/
...
function modifierMeta(props, meta) {
	if (meta[this.searchId] && meta[this.searchId].items) {
		meta[this.searchId].items.map(item => {
			...
			} else if (item.attrcode === 'pk_finvariety') { //发债品种多选
	...
	meta[this.tableId].items.push({
		...
		render: (text, record, index) => {
			...
			let disableflagbill = record["pk_register.disableflag"] && record["pk_register.disableflag"].value;//票据作废
			let fbmbilltype = record["pk_register.fbmbilltype"] && record["pk_register.fbmbilltype"].value;//票据类型
