...
export function afterEvent(props, moduleId, key, value, oldValue) {
	...
	//	this.props.resMetaAfterPkorgEdit();
		...
		if (!oldValue.value) {
			...
			promptBox({
				color: "warning",
				title: this.props.MutiInit.getIntl("36180BT") && this.props.MutiInit.getIntl("36180BT").get('36180BT-000000'),/* 国际化处理： 确认*/
				content: this.props.MutiInit.getIntl("36180BT") && this.props.MutiInit.getIntl("36180BT").get('36180BT-000001'),/* 国际化处理： 切换组织将会清空您录入的信息，请确认?*/
				beSureBtnClick: () => {
					changeOrg.call(this, value, eventData);
				},
				cancelBtnClick:() => {
	...
	} else if (key === 'olcrate' || key ==='poundagemoney' || key ==='usedccamount'|| key ==='money'|| key ==='securitymoney'
			|| key ==='occucommoney'|| key ==='occusharemoney' || key === 'invoicedate'   || key==='hidereceivebankacc'|| key==='pk_payacc' || key ==='inputdate'|| key ==='glcrate'|| key ==='gllcrate') {
		
		if (value.value !== oldValue.value && value.value) {
			if( key ==='money'  || key ==='poundagemoney' || key ==='occucommoney'|| key ==='occusharemoney' || key ==='securitymoney'){
				if(value.value <= 0){
					toast({ color: 'warning', content:this.props.MutiInit.getIntl("36180BT") && this.props.MutiInit.getIntl("36180BT").get('36180BT-000002')});/* 国际化处理： 金额不能等于零*/
					return;
				}
			}
			cardEvent.getAfterEventData.call(this, eventData).then(res => {
