...
class Card extends Component {
        ...
        window.onbeforeunload = () => {
            if (!["browse"].includes(this.props.getUrlParam("status"))) {
                return (
                    ...
                    this.props.MutiInit.getIntl("36180BP").get("36180BP-000033")
    ...
    // 原页面toggleShow处理逻辑 跨服务按钮刷新不需要做按钮显隐性控制
    toggleShow4Ncc = () => {
        let status = this.props.getUrlParam("status");
        let form = this.props.form;
        if (status === "browse") {
            form.setItemsVisible(CARD_FORM_CODE, { pk_org: false });
            form.setItemsVisible(CARD_FORM_CODE, { pk_org_v: true });
            form.setFormStatus(CARD_FORM_CODE, status);
            ...
            form.setFormStatus(CARD_FORM_CODE, "edit");
            this.props.BillHeadInfo.setBillHeadInfoVisible({
                //控制显示返回按钮: true为显示,false为隐藏
                showBackBtn: false,
                //控制显示单据号：true为显示,false为隐藏
            ...
            form.setFormStatus(CARD_FORM_CODE, "add");
            this.props.BillHeadInfo.setBillHeadInfoVisible({
                //控制显示返回按钮: true为显示,false为隐藏
                showBackBtn: false,
                //控制显示单据号：true为显示,false为隐藏
    ...
    render() {
        ...
        return (
            <div className="nc-bill-card">
                ...
                />
                {/* 内部账户余额弹框 */}
                {showInnerAcc && (<InnerAccoutDialog
                    //是否显示
