...
import { BILL_TYPE, BTN_GROUP, CARD_PAGE_CODE, FULL_AGGCLASSNAME, LIST_DISABLENOTE_CODE, LINK_LIST_PAGE_CODE, LIST_TABLE_CODE, NODE_KEY, URL_LIST } from "./../../cons/constant";
...
function doDelete(props) {
  ...
  if (selectedData.length == 0) {
    toast({ color: "warning", content: mutiInit && mutiInit.get('36180PBR-000026') });/* 国际化处理： 请选择至少一条数据！*/
    return;
  ...
  let successCallback = function(res) {
    if (res.success) {
      ...
      if ("_LinkVouchar2019" == srcCode) {
        ...
        toast({ color: "warning", content: mutiInit && mutiInit.get('36180PBR-000018') });/* 国际化处理： 未查到凭证*/
        return;
      }
    }
  };
 ...
 */
function doRefresh() {
  searchButtonClick.call(this, this.props.MutiInit.getIntl("36180PBR") && this.props.MutiInit.getIntl("36180PBR").get('36180PBR-000019'));/* 国际化处理： 刷新成功！*/
}
...
function doOutput(props) {
  ...
  if (!outputData || outputData.length == 0) {
    toast({ color: "warning", content: this.props.MutiInit.getIntl("36180PBR") && this.props.MutiInit.getIntl("36180PBR").get('36180PBR-000026') });/* 国际化处理： 请选择至少一条数据！*/
    return;
  }
  let outputpks = [];
