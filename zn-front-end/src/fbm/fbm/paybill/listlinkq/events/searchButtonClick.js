...
import { DATASOURCELINK, LINK_LIST_PAGE_CODE, LIST_SEARCH_CODE, LIST_TABLE_CODE, URL_LIST } from "./../../cons/constant";
...
export function searchButtonClick(content, props, searchVal) {
  ...
  let querystatus = this.state.activeKey  && 0;
  ...
  if (!searchVal || !searchVal.conditions || searchVal.conditions.length == 0) {
    searchVal = getDefData("searchVal", DATASOURCELINK);
    if (!searchVal) {
      return;
    }
  } else {
    setDefData("searchVal", DATASOURCELINK, searchVal); //放入缓存
  }
  let data = {
    ...
    success: res => {
      ...
      if (success) {
        ...
        if (data.numvalues) {
          ...
          //放入缓存
          setDefData("numvalues", DATASOURCELINK, data.numvalues);
          setDefData("activeKey", DATASOURCELINK, querystatus);

          if (parseInt(data.numvalues.ALL) < 1) {
            toast({ color: "warning", content: this.props.MutiInit.getIntl("36180PBR") && this.props.MutiInit.getIntl("36180PBR").get('36180PBR-000035') });/* 国际化处理： 未查询出数据！*/
          } else {
            if (content && content.length > 0) {
            toast({ color: "success", content: content });}
          }
        }
        setDefData(LIST_TABLE_CODE, DATASOURCELINK, data.grid); //放入缓存
      }
    }
  });
}
