  ...
  switch (key) {
    ...
    case "InnerUnCommit":
      doListInnerAjaxAndReturn.call(
        ...
        index,
        mutiInit &&
          mutiInit.get("36180PBR-000006") /* 国际化处理： 收回成功！*/,
        URL_LIST.UN_COMMIT
      );
      break;
    // 制证
    case "InnerMakeVoucher":
      doListInnerAjaxAndReturn.call(
        ...
        index,
        mutiInit &&
          mutiInit.get("36180PBR-000007") /* 国际化处理： 制证成功！*/,
        URL_LIST.VOUCHER
      );
      break;
    //取消制证
    ...
    case "InnerSendCmd":
      doListInnerAjaxAndReturn.call(
        ...
        index,
        mutiInit &&
          mutiInit.get("36180PBR-000009") /* 国际化处理： 发送指令成功！*/,
        URL_LIST.SEND_CMD
      );
      break;
    // 撤回指令
    case "InnerWithdrawCmd":
      doListInnerAjaxAndReturn.call(
        ...
        index,
        mutiInit &&
          mutiInit.get("36180PBR-000023") /* 国际化处理： 撤回指令成功！*/,
...
function doInnerEidt(props, record, index) {
  let pk = record.pk_paybill && record.pk_paybill.value;
  ...
  let successCallback = function(res) {
    if (res.data.errMsg) {
      ...
      toast({
        ...
        content:
          mutiInit &&
          mutiInit.get("36180PBR-000024") /* 国际化处理： 删除成功！*/
      });
    ...
    if (
      ...
      if (successIndexs == 1) {
        toast({
          ...
          content:
            mutiInit &&
            mutiInit.get("36180PBR-000015") /* 国际化处理： 提交成功！*/
        });
      } else {
        toast({
          color: "error",
