...
export function buttonClick(props, id) {
  switch (id) {
    ...
    case BTN_GROUP.CANCEL_DISABLE:
      if (!props.table.getCheckedRows(LIST_TABLE_CODE) || props.table.getCheckedRows(LIST_TABLE_CODE).length == 0) {
          toast({
            color: "warning",
            content:
              this.props.MutiInit.getIntl("36180PBR") &&
    ...
    let successCallback = function(res) {
      ...
      if (res.data.errMsg) {
        ...
        toast({
          ...
          content:
            mutiInit &&
            mutiInit.get("36180PBR-000016") /* 国际化处理： 作废成功！*/
        });
      }
    };
...
function doManyAjaxAndReturn(that, props, operation, url) {
  ...
  if (!selectDatas || selectDatas.length == 0) {
    toast({
      ...
      content:
        mutiInit &&
        mutiInit.get("36180PBR-000026") /* 国际化处理： 请选择至少一条数据！*/
    });
    return;
  }
  ...
  let successCallback = function(res) {
    ...
    if (res.data.successpks) {
      ...
      for(let i = 0;i<pks.length; i++){
        // 后台删除成功之后 进行前端数据删除
        props.table.deleteTableRowsByIndex(LIST_TABLE_CODE, pks[i]);
      }
    }
    failIndexs = selectDatas.length - successIndexs;
    // 全部成功
...
function doDelete(props) {
  ...
  if (selectedData.length == 0) {
    ...
    deleteContent =
      mutiInit &&
      mutiInit.get("36180PBR-000028"); /* 国际化处理： 您确定要删除所选数据吗?*/
  } else {
    deleteContent =
      mutiInit &&
      mutiInit.get("36180PBR-000029"); /* 国际化处理： 确定要删除吗?*/
  }
  promptBox({
    /* 国际化处理：删除*/
...
function doLinkVoucher(props) {
  ...
  if (!voucher) {
    toast({
      ...
      content:
        mutiInit &&
        mutiInit.get("36180PBR-000032") /* 国际化处理： 未制证，不能联查凭证！*/
    });
    return;
  }
  //拼接联查数据,
...
function doRefresh() {
  searchButtonClick.call(
    this,
    this.props.MutiInit.getIntl("36180PBR") &&
      this.props.MutiInit.getIntl("36180PBR").get("36180PBR-000019")
  ); /* 国际化处理： 刷新成功！*/
}
...
function doOutput(props) {
  ...
  if (!outputData || outputData.length == 0) {
    toast({
      ...
      content:
        this.props.MutiInit.getIntl("36180PBR") &&
        this.props.MutiInit.getIntl("36180PBR").get("36180PBR-000026")
    }); /* 国际化处理： 请选择至少一条数据！*/
    return;
  }
  let outputpks = [];
