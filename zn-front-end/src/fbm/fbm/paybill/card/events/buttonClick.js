...
import { BILL_TYPE, BTN_CARD, CARD_DISABLENOTE_CODE, CARD_FORM_CODE, CARD_PAGE_CODE, CARD_TABLE_CODE1, CARD_TABLE_CODE2, CARD_TABLE_CODE3, DATASOURCE, FULL_AGGCLASSNAME, LIST_DISABLENOTE_CODE, NODE_KEY, PRIMARY_ID, URL_LIST } from "./../../cons/constant";
...
export function buttonClick(props, id) {
  let nowId=props.getUrlParam("id");
  ...
  switch (id) {
    ...
    case BTN_CARD.CANCEL:
      promptBox({
        title:
          mutiInit && mutiInit.get("36180PBR-000002") /* 国际化处理： 取消*/,
        color: "warning",
        content:
          mutiInit &&
          mutiInit.get("36180PBR-000003") /* 国际化处理： 是否确认要取消?*/,
    ...
    case BTN_CARD.DELETE:
      promptBox({
        title:
          mutiInit && mutiInit.get("36180PBR-000004") /* 国际化处理： 删除*/,
        color: "warning",
        content:
          mutiInit &&
          mutiInit.get("36180PBR-000005") /* 国际化处理： 是否确认要删除?*/,
    ...
    case BTN_CARD.UN_COMMIT:
      ...
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get("36180PBR-000006"),
        URL_LIST.UN_COMMIT
      ); /* 国际化处理： 收回成功！*/
      break;
    ...
    case BTN_CARD.MAKE_VOUCHER:
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get("36180PBR-000007"),
        URL_LIST.VOUCHER
      ); /* 国际化处理： 制证成功！*/
      break;
    ...
    case BTN_CARD.VOUCHER_CANCEL:
      doAjaxAndReturn.call(
        ...
        props,
        mutiInit &&
          mutiInit.get("36180PBR-000008") /* 国际化处理： 取消制证成功！*/,
    ...
    case BTN_CARD.SEND_CMD:
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get("36180PBR-000009"),
        URL_LIST.SEND_CMD
      ); /* 国际化处理： 发送指令成功！*/
      break;
    ...
    case BTN_CARD.WITHDRAW_CMD:
      doAjaxAndReturn.call(
        ...
        props,
        mutiInit &&
          mutiInit.get("36180PBR-000010") /* 国际化处理： 撤销指令成功！*/,
    ...
    case BTN_CARD.CANCEL_DISABLE:
      doAjaxAndReturn.call(
        ...
        props,
        mutiInit &&
          mutiInit.get("36180PBR-000011") /* 国际化处理： 取消作废成功！*/,
  ...
  if (toSave) {
    ...
    let saveCallback = function(res) {
      if (res.data.card.head) {
        ...
        let headData = res.data.card.head[CARD_FORM_CODE].rows[0].values;
        const tbbMsg = headData && headData["tbbmessage"] && headData["tbbmessage"].value;
        if (tbbMsg) toast({ color: "warning", content: tbbMsg }); //预算提示
  ...
  let successCallback = function(res) {
    if (res.data && res.data.errMsg) {
      ...
      toast({
        ...
        content:
          this.props.MutiInit.getIntl("36180PBR") &&
          this.props.MutiInit.getIntl("36180PBR").get("36180PBR-000013")
      }); /* 国际化处理： 删除成功!*/
...
function doEdit(props) {
  let tableName = "fbm_paybill";
  let primaryId = "pk_paybill";
  let pk =
    props.form.getFormItemsValue(CARD_FORM_CODE, "pk_paybill") &&
    props.form.getFormItemsValue(CARD_FORM_CODE, "pk_paybill").value;
  ...
  apiSaga.call(this, {
    ...
    success: res => {
      ...
      if (this.props.form.getFormItemsValue(CARD_FORM_CODE, 'saga_frozen') 
          && this.props.form.getFormItemsValue(CARD_FORM_CODE, 'saga_status')){
        this.props.form.setFormItemsValue(CARD_FORM_CODE,{'saga_frozen':{value:'0'},'saga_status':{value:'0'}});
      }
      this.componentDidMount();
    }
  })
    ...
    if (res.success) {
      ...
      if ("_LinkVouchar2019" == srcCode) {
          ...
          if (res.data.pklist) {
            if (res.data.pklist.length == 1) {
              ...
              this.props.openTo(res.data.url, {
                ...
                n:
                  this.props.MutiInit.getIntl("36180PBR") &&
                  this.props.MutiInit.getIntl("36180PBR").get("36180PBR-000017") //'联查凭证'/* 国际化处理： 联查凭证*/
              });
              return;
            }
          }
        ...
        toast({
          ...
          content:
            this.props.MutiInit.getIntl("36180PBR") &&
            this.props.MutiInit.getIntl("36180PBR").get("36180PBR-000018")
        }); /* 国际化处理： 未查到凭证*/
        return;
      }
    }
