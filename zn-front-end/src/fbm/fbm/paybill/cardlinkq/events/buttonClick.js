...
import { BILL_TYPE, BTN_CARD, CARD_DISABLENOTE_CODE, CARD_FORM_CODE, LINK_CARD_PAGE_CODE, CARD_TABLE_CODE1, CARD_TABLE_CODE2, CARD_TABLE_CODE3, DATASOURCELINK, FULL_AGGCLASSNAME, LIST_DISABLENOTE_CODE, NODE_KEY, PRIMARY_ID, URL_LIST } from "./../../cons/constant";
...
let mutiInit=this.props.MutiInit.getIntl("36180PBR");
 switch (id) {
    ...
    case BTN_CARD.UN_COMMIT:
      ...
      doAjaxAndReturn.call(this, props, mutiInit && mutiInit.get('36180PBR-000006'), URL_LIST.UN_COMMIT);/* 国际化处理： 收回成功！*/
      break;

    //制证
    case BTN_CARD.MAKE_VOUCHER:
      doAjaxAndReturn.call(this, props, mutiInit && mutiInit.get('36180PBR-000007'), URL_LIST.VOUCHER);/* 国际化处理： 制证成功！*/
      break;
    ...
    case BTN_CARD.VOUCHER_CANCEL:
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get('36180PBR-000008'),/* 国际化处理： 取消制证成功！*/
    ...
    case BTN_CARD.SEND_CMD:
      doAjaxAndReturn.call(this, props, mutiInit && mutiInit.get('36180PBR-000009'), URL_LIST.SEND_CMD);/* 国际化处理： 发送指令成功！*/
      break;
    ...
    case BTN_CARD.WITHDRAW_CMD:
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get('36180PBR-000010'),/* 国际化处理： 撤销指令成功！*/
    ...
    case BTN_CARD.CANCEL_DISABLE:
      doAjaxAndReturn.call(
        this,
        props,
        mutiInit && mutiInit.get('36180PBR-000011'),/* 国际化处理： 取消作废成功！*/
...
function doDelete(props) {
  ...
  let successCallback = function(res) {
    toast({ color: "success", content: this.props.MutiInit.getIntl("36180PBR") && this.props.MutiInit.getIntl("36180PBR").get('36180PBR-000013') });/* 国际化处理： 删除成功!*/

    let deleteid = this.props.getUrlParam("id");
    let deletenextId = getNextId(deleteid, DATASOURCELINK);
    deleteCacheById(PRIMARY_ID, deleteid, DATASOURCELINK);
  ...
  let successCallback = function(res) {
    if (res.success) {
      ...
      if ("_LinkVouchar2019" == srcCode) {
        ...
        toast({ color: "warning", content: this.props.MutiInit.getIntl("36180PBR") && this.props.MutiInit.getIntl("36180PBR").get('36180PBR-000018') });/* 国际化处理： 未查到凭证*/
        return;
      }
    }
  };
