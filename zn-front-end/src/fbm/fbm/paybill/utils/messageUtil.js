...
export const BatchToast = function(
  ...
  let operate = "";
  let mutiInit = props.MutiInit.getIntl("36180PBR");
  if (op === "delete") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000004"); /* 国际化处理： 删除*/
  } else if (op === "commit") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000044"); /* 国际化处理： 提交*/
  } else if (op === "uncommit") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000045"); /* 国际化处理： 收回*/
  } else if (op === "disable") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000046"); /* 国际化处理： 作废*/
  } else if (op === "canceldisable") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000047"); /* 国际化处理： 取消作废*/
  } else if (op === "sendcmd") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000048"); /* 国际化处理： 发送指令*/
  } else if (op === "withdrawcmd") {
    operate =
      mutiInit && mutiInit.get("36180PBR-000049"); /* 国际化处理： 撤销指令*/
  }
  let title =
    operate + mutiInit &&
    mutiInit.get("36180PBR-000050"); /* 国际化处理： 完毕*/
  let color = "success";

  if (status == 0) {
    title =
      title + mutiInit &&
      mutiInit.get("36180PBR-000051"); /* 国际化处理： 全部失败*/
    color = "danger";
  } else if (status == 1) {
    title =
      title + mutiInit &&
      mutiInit.get("36180PBR-000052"); /* 国际化处理： 全部成功*/
    color = "success";
  } else if (status == 2) {
    title =
      title + mutiInit &&
      mutiInit.get("36180PBR-000053"); /* 国际化处理： 部分失败*/
    color = "danger";
  }
  if (!content) {
    content =
      mutiInit &&
      `${mutiInit.get("36180PBR-000054")}${operate}${total}${mutiInit.get(
        "36180PBR-000055"
      )},${mutiInit.get("36180PBR-000056")}${successIndex}${mutiInit.get(
        "36180PBR-000055"
      )},${mutiInit.get("36180PBR-000057")}${failIndex}${mutiInit.get(
        "36180PBR-000055"
      )}`;
  }
  console.log(content);
