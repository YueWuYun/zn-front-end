...
export function buttonDisable () {
    ...
    if (selectDatas.length == 1) {
        ...
        let onlinebankflag = item.data.values.cyberbankflag.value;
        ...
        let registerstatus = item.data.values.registerstatus.value;
        let LinkInnerAccount = item.data.values.pk_inbalaacc && item.data.values.pk_inbalaacc.value;
        let LQueryInSecurityAcc = item.data.values.pk_insecurityacc && item.data.values.pk_insecurityacc.value;
        if(!LinkInnerAccount){
            disBtn.push(btns.LinkInnerAccount);
        }
        if(!LQueryInSecurityAcc){
            disBtn.push(btns.LQueryInSecurityAcc);
        }
        if (vbillstatus!=1) {
            // 待提交
            hasapprove = true;
