...
import { CARD_FORM_CODE,DATASOURCE,URL_LIST } from "./../../cons/const";
...
export function pageInfoClick(props, pk){
	...
	// 有缓存用缓存的数据，没有缓存就重新查
	if (cardData) {
		props.form.setAllFormValue({ [CARD_FORM_CODE]: cardData.head[CARD_FORM_CODE] });
		...
		let callback = function (res) {
			if (res.data.card) {
				...
				cardCache.updateCache('pk_accept', pk, res.data.card, CARD_FORM_CODE, DATASOURCE);
				this.titleno = res.data.card.head[CARD_FORM_CODE].rows[0].values.vbillno.value;
