    ...
    switch (id) {
        ...
        case CARD_BTN.CANCEL:
            promptBox({
                title:
                    mutiInit &&
                    mutiInit.get("36180BPA-000002") /* 国际化处理： 取消*/,
                color: "warning",
                content:
                    mutiInit &&
                    mutiInit.get(
        ...
        case CARD_BTN.DELETE:
            promptBox({
                title:
                    mutiInit &&
                    mutiInit.get("36180BPA-000004") /* 国际化处理： 删除*/,
                color: "warning",
                content:
                    mutiInit &&
                    mutiInit.get(
...
function doAdd(props) {
    let pk =
        props.form.getFormItemsValue(CARD_FORM_CODE, "pk_accept") &&
        props.form.getFormItemsValue(CARD_FORM_CODE, "pk_accept").value;
    if (!pk) {
        pk = "";
    }
            ...
            if (res.data.card.head) {
                ...
                if (showToast) {
                    toast({
                        color: "success",
                        content:
                            this.props.MutiInit.getIntl("36180BPA") &&
                            this.props.MutiInit.getIntl("36180BPA").get(
                                "36180BPA-000006"
                            ) /* 国际化处理： 保存成功*/
                    });
                }
...
function doDelete(props) {
    ...
    let successCallback = function(res) {
        toast({
            ...
            content:
                this.props.MutiInit.getIntl("36180BPA") &&
                this.props.MutiInit.getIntl("36180BPA").get("36180BPA-000007")
        }); /* 国际化处理： 删除成功!*/
    ...
    if (res.success) {
      ...
      if ("_LinkVouchar2019" == srcCode) {
        ...
        toast({
          ...
          content:
            this.props.MutiInit.getIntl("36180BPA") &&
            this.props.MutiInit.getIntl("36180BPA").get("36180BPA-000032")
        }); /* 国际化处理： 未查到凭证*/
        return;
      }
    }
