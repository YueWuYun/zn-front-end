...
import { BatchToast } from "./../../../gather/utils/messageUtil";
import {
    CARD_PAGE_CODE,
    FULL_AGGCLASSNAME,
    LIST_BTN,
...
function doDelete(props) {
    ...
    if (selectedData.length == 0) {
        ...
        deleteContent =
            mutiInit &&
            mutiInit.get("36180BPA-000019"); /* 国际化处理： 确定要删除吗?*/
    }
    promptBox({
        title:
            mutiInit && mutiInit.get("36180BPA-000004") /* 国际化处理： 删除*/,
        color: "warning",
...
function doCopy(props) {
    let seldata = props.table.getCheckedRows(LIST_TABLE_CODE);
