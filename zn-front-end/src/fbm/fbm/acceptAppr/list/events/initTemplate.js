...
import { LIST_PAGE_CODE,APPCODE,LIST_TABLE_CODE,LIST_SEARCH_CODE,CARD_PAGE_CODE,LIST_INNERBTN,LIST_INNERAREA,DATASOURCE,BILL_TYPE } from "../../cons/const";
        ...
        (data)=>{
            if (data) {
				...
				if (data.button) {
					...
					props.button.setPopContent('InnerDelete',this.props.MutiInit.getIntl("36180BPA") && this.props.MutiInit.getIntl("36180BPA").get('36180BPA-000019') );/* 国际化处理： 确定要删除吗?*/
					// 导入
					props.button.setUploadConfig('Import', excelimportconfig);
				}
				if (data.template) {	
