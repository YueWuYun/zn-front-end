      ...
      if (data) {
        if (data.button) {
          ...
          props.button.setPopContent(
            "DeleteInner",
            this.props.MutiInit.getIntl("36180PDT").get("36180PDT-000003")
          ); /* 国际化处理： 确认要删除吗?*/
          buttonVisiable.call(this, props);
        }
        if (data.template) {
...
function modifierMeta(props, meta) {
  ...
  meta[this.tableId].items.push({
    ...
    label:
      ...
      this.props.MutiInit.getIntl("36180PDT").get("36180PDT-000004"), //显示名称/* 国际化处理： 操作*/
    width: 200,
    fixed: "right", //存放位置
    className: "table-opr",
    visible: true,
    render: (text, record, index) => {
      ...
      sucessCallBack: () => {
        ...
        let voucher = record.voucher && record.voucher.value; // true or false
        switch (vbillstatus) {
          case "-1": //待提交
