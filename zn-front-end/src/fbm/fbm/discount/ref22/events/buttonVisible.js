...
export function buttonVisible (props) {
    ...
    let editBtn = ['Save', 'SaveAdd', 'SaveCommit', 'Cancel','CancelTransfer'];//编辑态显示按钮
    let allBtns = [...buttons, ...editBtn, ...unionBtn, 'Add','Edit', 'Delete','Uncommit','Commit'];
    ...
    let transactresult = this.props.form.getFormItemsValue(this.formId, 'transactresult').value;
    // 控制错误按钮的显隐性
    ...
    if (!status) {
        ...
        if (!id) {//新增浏览态
            ...
            showBtn.push(btns.CancelTransfer);
            if (pk_discount) {
