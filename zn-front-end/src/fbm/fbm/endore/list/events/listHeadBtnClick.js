...
import { toast, promptBox, print, cacheTools } from 'nc-lightapp-front';
import { URI, LIST, CARD, LIST_BTN, FULL_AGGCLASSNAME, BILLTYPE } from '../../cons/constant';
import { doSearch } from './searchBtnClick';
...
export function listHeadBtnClick(props, key) {
	let selectedRows;
	if (key != LIST_BTN.add && key != LIST_BTN.import &&  key != LIST_BTN.export &&  key != LIST_BTN.refresh) {
		// 获取选中行
		selectedRows = this.props.table.getCheckedRows(LIST.tableCode);
		if (isEmptyObject(selectedRows)) {
			toast({ color: 'warning', content: this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000037') });/* 国际化处理： 请选中至少一条数据！*/
			return;
		}
	}
	switch (key) {
		...
		case LIST_BTN.export: // 导出格式文件
			...
			break;
		case LIST_BTN.unionApprovalDetails: // 联查审批详情
			doLinkApprove.call(this, selectedRows);
			break;
		case LIST_BTN.unionFundsBudget: // 联查资金预算
			linkNtb.call(this, selectedRows);
			break;
		case LIST_BTN.unionBillAccount: // 联查票据台账
			linkSDBook.call(this, selectedRows);
			break;
		case LIST_BTN.unionVoucher: //联查 凭证
			linkVoucher.call(this, selectedRows);
			break;
		case LIST_BTN.unionPayBill: // 联查付款单
...
const handleRefreshClick = function() {
	// 重新查询数据
	doSearch.call(this, null, LIST_BTN.refresh);
};
...
const handleCommitClick = function(selectDatas) {
	...
	let successCallback = function(res) {
		...
		if (res.data.workflow && (res.data.workflow === 'approveflow' || res.data.workflow === 'workflow')) {
			...
			// 全部成功
			if (failIndexs == 0) {
				BatchToast.call(this,OPERA_TYPE.COMMIT, 1, selectDatas.length, successIndexs, failIndexs, null, null);
...
function linkNtb(selectedRows) {
	...
	doAjax.call(this, sendData, URI.endoreListLinkNtb, successCallback);
}
...
function linkVoucher(selectedRows){
	...
	doAjax.call(this, querydata, URI.endoreCardLinkVoucher, successCallback);
}
...
function linkPayBill(selectedRows){
	...
	doAjax.call(this,sendData,URI.endoreListLinkPayBill,successaCallback)
}
