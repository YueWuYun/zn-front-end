...
const handleDelInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000068') : res.data.errMsgs[0]/* 国际化处理： 删除失败！*/
			});
		}
		// 全部成功
...
const handleUncommitInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000030')/* 国际化处理： 存在后续操作，不可收回！*/ : res.data.errMsgs[0]
			});
		}
		// 全部成功
		if (res.data.status == 1) {
...
const handleVoucherInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000031') : res.data.errMsgs[0]/* 国际化处理： 制证失败！*/
			});
		}
		// 全部成功
		if (res.data.status == 1) {
...
const handleUnVoucherInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000032') : res.data.errMsgs[0]/* 国际化处理： 取消制证失败！*/
			});
		}
		// 全部成功
		if (res.data.status == 1) {
...
const handleSendCommandInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000033') : res.data.errMsgs[0]/* 国际化处理： 发送指令失败！*/
			});
		}
		// 全部成功
		if (res.data.status == 1) {
...
const handleCancelCommandInnerClick = function(pk, index, record) {
	...
	let successCallback = function(res) {
		...
		if (res.data.status == 0) {
			toast({
				color: 'danger',
				content: isEmptyObject(res.data.errMsgs[0]) ? this.props.MutiInit.getIntl("36180ET") && this.props.MutiInit.getIntl("36180ET").get('36180ET-000034') : res.data.errMsgs[0]/* 国际化处理： 撤回指令失败！*/
			});
		}
		// 全部成功
		if (res.data.status == 1) {
