...
export function buttonVisible (props) {
    ...
    let transform = props.form.getFormItemsValue(this.formId, 'transform') && props.form.getFormItemsValue(this.formId, 'transform').value;
    let btnObj= {};
    let showBtn= [];
    let editBtn = ['save_group', 'SaveCommit', 'Cancel', 'addRow', 'deleteRow', 'insertRow', 'delRow', 'expand'];
    let unionBtn = ['LinkSDBook','Voucher','LinkBudgetPlan','LinkReceAndPaybill', 'ApproveDetail'];
    ...
    if(!status){
        ...
        if (!id) {              //新增浏览态
            ...
            switch (vbillstatus) {
                case '-1':	    //待提交
                    commonBtn = commonBtn.filter(item => item !== 'ApproveDetail').filter(item => item !== 'LinkReceAndPaybill').filter(item => item !== 'Voucher');
                ...
                case '2':	    //审批进行中
                case '3':	    //提交
                    commonBtn = commonBtn.filter(item => item !== 'Voucher').filter(item => item !== 'LinkReceAndPaybill');
