...
export default function clickSerachBtn(props, queryInfo) {
  ajax({
    ...
    success: res => {
      ...
      if (success) {
        if (data) {
          let multiLang = this.props.MutiInit.getIntl("36200BC");
          if (data.grid) {
             /* 国际化处理： 查询成功!*/
             toast({ color: 'success', content: multiLang && multiLang.get('36200BC-000016') });
            ...
            toast({ color: 'warning', content: multiLang && multiLang.get('36200BC-000017') });
            this.props.transferTable.setTransferTableValue(
              REF21_CONST.formId,
              REF21_CONST.tableId,
              [],
