...
export default function clickSerachBtn(props, queryInfo) {
  ajax({
    ...
    success: res => {
      ...
      if (success) {
        if (data) {
          let multiLang = this.props.MutiInit.getIntl("36200DA");
          if (data.grid) {
            /* 国际化处理： 查询成功!*/
            toast({ color: 'success', content: multiLang && multiLang.get('36200DA-000017') })
            ...
            /* 国际化处理： 未查询出数据!*/
            toast({ color: 'warning', content: multiLang && multiLang.get('36200DA-000018') });
            this.props.transferTable.setTransferTableValue(
              REF21_CONST.formId,
              REF21_CONST.tableId,
              [],
