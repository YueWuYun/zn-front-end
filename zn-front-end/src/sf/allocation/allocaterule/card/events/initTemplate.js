				...
				if (data.template) {
					...
					props.meta.setMeta(meta, () => {
						...
						if (props.getUrlParam('status') == 'edit' || props.getUrlParam('status') == 'copy') {
							...
							props.form.setFormItemsDisabled(jsoncode.formcode, { 'billmaker': true, 'dbilldate': true, 'approver': true, 'dapprovedate':true, 'dapprovetime':true, 'vapprovenote': true, 'dbilltime': true });
						}
					});
					//组织多版本
...
function modifierMeta(that, props, meta) {
	...
	meta['childform2_allocaterule_01'].items.find((e) => e.attrcode == 'pk_financeorg').isShowUnit = true;
					...
					BankaccSubUseTreeGridRef({
						queryCondition: () => {
							return {
								...
								pk_currtype: props.form.getFormItemsValue(formId, 'pk_currtype').value,
								refnodename: loadMultiLang(props, '36320AAC-000015'),/* 国际化处理： 使用权参照*/
								GridRefActionExt: 'nccloud.web.sf.allocation.allocateapply.filter.AllocateApplyCurrentRefBankAccFilter,' +
