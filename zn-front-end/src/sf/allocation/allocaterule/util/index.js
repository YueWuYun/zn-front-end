...
export const resetBodysRowno = function (props) {
    let allRows1 = props.cardTable.getAllRows(jsoncode.ctablecode, true);
    ...
    for (let index = 0; index < allRows1.length; index++) {
        if(allRows1[index].status == '3'){
            props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, index, 'dr', { value: Number(parseInt(1)).toString() });
        }
        props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, index, 'rowno', { value: Number(parseInt(index) + parseInt(1)).toString() });
    }
    this.props.updatePage(jsoncode.formcode, [jsoncode.ctablecode]);
}
