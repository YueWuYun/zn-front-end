...
class List extends Component {
	constructor(props) {
		...
		this.tableId = 'table_allocaterule_01';
		this.pkMapTs = null;
		this.state = {
			//选中某个页签后 需要赋值
			activeKey: '0',
	...
	DoubleClick = (record, index, props, e) => {
		go2card(props, { pagecode: jsoncode.cpageid, status: 'browse', id: record.pk_allocaterule_h.value }, this.getState.bind(this));
	}
	//处理网银补录返回信息
			...
			<div className="nc-bill-list">
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
					<div className="header-title-search-area">
						{createBillHeadInfo(
							{
								title: loadMultiLang(this.props, '36320AAC-000023'),//{/* 国际化处理： 下拨规则设置*/}
				...
				{/* <div className="tab-definInfo-area"> */}
					<NCTabs activeKey={this.state.activeKey}
						...
						<NCTabPane key={'0'} tab={
						<span>
							{loadMultiLang(this.props, '36320AAC-000035')/* 国际化处理： 待提交*/
								+ '('}<span>{groupCount.needCommit || 0}</span>{')'}
						</span>}
						/>
						<NCTabPane key={'1'} tab={
						<span>
							{loadMultiLang(this.props, '36320AAC-000036')/* 国际化处理： 审批中*/
								+ '('}<span>{groupCount.needApprove || 0}</span>{')'}
						</span>}
						/>
						<NCTabPane key={'2'} tab={
						<span>
							{loadMultiLang(this.props, '36320AAC-000037')/* 国际化处理： 全部*/}
						</span>}
						/>
					</NCTabs>
				{/* </div> */}
				...
				<div>
					{assignShow && <ApprovalTrans
						...
						getResult={(value) => {
							...
							listSingleOperatorNoRecord(this.props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchcommit, billID, ts, index, loadMultiLang(this.props,'36320AAC-000038'), jsoncode.dataSource, true, extParam);/* 国际化处理： 提交成功*/
