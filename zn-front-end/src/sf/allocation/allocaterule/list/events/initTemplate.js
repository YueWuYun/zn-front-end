		...
		function (data) {
			if (data) {
				...
				if (data.button) {
					...
					props.button.setPopContent('DeleteInner', loadMultiLang(props, '36320AAC-000030'));/* 国际化处理： 确定要删除吗？*/
				}
			}
		}
	)
...
function modifierMeta(that, props, meta) {
	meta[jsoncode.searchcode].items.find((e) => e.attrcode == 'allocaterule_b.pk_financeorg').isShowUnit = true;
	...
	const tableButtonClick = (props, key, text, record, index) => {
		...
		switch (key) {
				...
				ajax({
					...
					success: function (res) {
						...
						if (success) {
							...
							toast({ color: 'warning', content: loadMultiLang(props, '36320AAC-000033') });/* 国际化处理： 删除失败，请稍后再试！*/
						}
					}
				});
				break;
			// 提交
			case 'CommitInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchcommit, record, 'pk_allocaterule_h', index, loadMultiLang(props, '36320AAC-000008'), jsoncode.dataSource, false, null, (props, data) => {/* 国际化处理： 提交*/
					let { workflow } = data;
					//有指派信息，则指派，没有则重绘界面
			...
			// 收回
			case 'UncommitInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchuncommit, record, 'pk_allocaterule_h', index, loadMultiLang(props, '36320AAC-000013'), jsoncode.dataSource);/* 国际化处理： 收回*/
				break;
			// 启用
			case 'EnableInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchenable, record, 'pk_allocaterule_h', index, loadMultiLang(props, '36320AAC-000011'), jsoncode.dataSource);/* 国际化处理： 启用*/
				break;
			// 停用
			case 'DisableInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchdisable, record, 'pk_allocaterule_h', index, loadMultiLang(props, '36320AAC-000012'), jsoncode.dataSource);/* 国际化处理： 停用*/
				break;
			// 补录
