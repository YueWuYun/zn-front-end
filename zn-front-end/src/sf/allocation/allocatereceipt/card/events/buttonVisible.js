...
export default function buttonVisible(props) {
	...
	//所有按钮隐藏
	props.button.setButtonVisible(['Bookkeeping', 'UnBookkeeping', 'Certification', 'UnCertification','Voucher'], false);

	//记账标识
	let istally = props.form.getAllFormValue('form_allocatereceipt_head').rows[0].values.istally.value === true ? true : false;//是否记账;
	//制证标识
	let isvoucher = props.form.getAllFormValue('form_allocatereceipt_head').rows[0].values.isvoucher.value === true ? true : false;//是否制证;
	...
	if (getDefData(dataSource, islink)) {
		...
		if (!istally) {//没有记账
			...
			if (isvoucher) {//已经制证
				//显示取消制证
				props.button.setButtonVisible(['UnCertification','Voucher'], true);
			} else {//没有制证
				//显示制证 取消记账
...
export const hideAllBtn = function (props) {
	...
	setButtonDisabled(['Bookkeeping', 'UnBookkeeping', 'Certification', 'UnCertification'], ture);
}
