...
let searchId = '36320FAR_list_search';
let tableId = '36320FAR_list_table';
let pageId = '36320FAR_L01';
...
function modifierMeta(props, meta, that) {
	...
	//收款单位跨集团参照
	meta[searchId].items.find((e) => e.attrcode == 'pk_org_p').isShowUnit = true;
	...
	meta[searchId].items.map((item) => {
		...
		if (item.attrcode == 'pk_accid_r') {
			item.queryCondition = () => {
				let search_currtype_value = props.search.getSearchValByField('36320FAR_list_search', 'pk_currtype');//币种
				if (search_currtype_value) {
					search_currtype_value = search_currtype_value.value.firstvalue;
				} else {
					search_currtype_value = null;
				}
				let search_org_value = props.search.getSearchValByField('36320FAR_list_search', 'pk_org_p');//币种
				if (search_org_value) {
		...
		if (item.attrcode == 'pk_bankacc_p') {
			item.queryCondition = () => {
				...
				//币种
				let search_currtype_p = props.search.getSearchValByField('36320FAR_list_search', 'pk_currtype');//币种
				...
				//下拨组织
				let search_payorg_value = props.search.getSearchValByField('36320FAR_list_search', 'pk_org_p');
				...
				return {
					...
					refnodename: loadMultiLang(props,'36320FAR-000035'),/* 国际化处理： 使用权参照*/
					noConditionOrg: 'Y',
		...
		if (item.attrcode == 'pk_bankacc_r') {
			item.queryCondition = () => {
				//币种过滤
				let search_currtype_r = props.search.getSearchValByField('36320FAR_list_search', 'pk_currtype');//币种
				...
				//上收组织
				let search_payorg_value = props.search.getSearchValByField('36320FAR_list_search', 'pk_org');
				if (search_payorg_value) {
					search_payorg_value = search_payorg_value.value.firstvalue;
				} else {
					search_payorg_value = null;
	...
	if (!getDefData(dataSource, islink)) {
		...
		meta[tableId].items.push({
			...
			label: loadMultiLang(props,'36320FAR-000036'),/* 国际化处理： 操作*/
			...
			render: (text, record, index) => {
				...
				if (record.istally.value == false) {//没有记账 显示记账
					...
					console.log(record.isvoucher.value);
