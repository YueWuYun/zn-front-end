...
import { ajax, base, toast,cardCache,print,promptBox,output } from 'nc-lightapp-front';
import { base_url,printnodekey,PAYMODEL_COMBINEPAY, billtype,card_page_id, card_from_id, card_table_id, viewmod_deal ,dataSource,funcode,SHOWMODEL_LIULAN,SHOWMODEL_BULU, list_page_id} from '../../cons/constant';
...
import { go2Card, listMultiOperator, listSingleOperator,showTBBMsg } from '../../../../pub/utils/SFButtonUtil';
import { buttonVisible,pageInfoClick } from "../events";
import {setEditDisable } from '../../util/index';
import { loadMultiLang,elecSignCardPrint } from "../../../../../tmpub/pub/util/index";
//引入下拨api
// import { versionControl, qryDataByPK, loadNeedDealDataByPK, repaintView, loadData2Card, save, tbbCallBack } from "../../util/index";
...
async function buttonClick(props, key, text, record, index) {
  ...
  switch (key) {
    ...
    case 'savecommit':
      save.call(this,props,()=>{
        bodyCodeMapBodyPKName[card_table_id] = 'pk_allocate_b';
        cardOperator(props, card_page_id, card_from_id,bodyCodeMapBodyPKName,'pk_allocate_h', base_url + 'commit.do', loadMultiLang(this.props,'36320FA-000005'), dataSource ,commitAssign.bind(this),true);/* 国际化处理： 保存提交成功！*/
      });
      break;
    //头部 编辑
    case 'edit':
      ...
      let data={ pk: pk, pageid: card_page_id ,status:'edit',pageCode:card_page_id};
      ajax({
          url,
          data: data,
          success: (res) => {
    ...
    //头部，提交
    case 'commit':
      cardOperator(props, card_page_id, card_from_id,bodyCodeMapBodyPKName,'pk_allocate_h', base_url + 'commit.do', loadMultiLang(this.props,'36320FA-000006'), dataSource ,commitAssign.bind(this),true);/* 国际化处理： 提交成功！*/
      break;
    //头部，收回
    case 'uncommit':
      cardOperator(props, card_page_id, card_from_id,bodyCodeMapBodyPKName,'pk_allocate_h', base_url + 'uncommit.do', loadMultiLang(this.props,'36320FA-000007'), dataSource ,this.toggleShow.bind(this));/* 国际化处理： 收回成功！*/
      break;
    //头部，退回
    case 'return':
    ...
    //头部 制证
    case 'make':
      cardOperator(props, card_page_id, card_from_id,bodyCodeMapBodyPKName,'pk_allocate_h', base_url + 'make.do', loadMultiLang(this.props,'36320FA-000008'), dataSource ,this.toggleShow.bind(this));/* 国际化处理： 制证成功！*/
      break;
    //头部 取消制证
    case 'cancelmake':
      cardOperator(props, card_page_id, card_from_id,bodyCodeMapBodyPKName, 'pk_allocate_h', base_url + 'unmake.do', loadMultiLang(this.props,'36320FA-000009'), dataSource, this.toggleShow.bind(this));/* 国际化处理： 取消制证成功！*/
      break;
    //头部 经办
    case 'decide':
    ...
    case 'field':
      ...
      this.setState({
        //单据pk
        // billId: filepk_allocate_h && filepk_allocate_h.value,
        //附件管理使用单据编号
      ...
      success: (res) => {
        ...
        if (success) {
          ...
          if (body) {
            ...
            that.props.cardTable.updateDataByIndexs(card_table_id, body[card_table_id]);
...
const doAllocatecheck =function(props){
  // let srcbilltype=props.form.getFormItemsValue(card_from_id,'srcbilltype');
  let pk = props.form.getFormItemsValue(card_from_id,'pk_srcbill');  
...
const doField = function(props){
  ...
  this.billId=pk_allocate_h && pk_allocate_h.value;
  this.billno=vbillno && vbillno.value;
  this.setState({
    showUploader: !this.state.showUploader,
    target: null
    })
