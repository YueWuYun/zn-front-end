...
import { app_id, card_from_id, card_table_id, card_page_id ,button,dataSource,allocatePk} from '../../cons/constant.js';
...
export const buttonVisible = function (props, status) {
    ...
    props.button.setButtonVisible(['savegroup', 'add', 'cancel', 'edit', 'delete', 'copy', 'decide', 'return', 'commit',
        'uncommit', 'linkgroup', 'print','elecsignformalPrint','elecsigninformalPrint', 'make', 'cancelmake', 'e_bank', 'pay', 'pay_merge', 'paystatus', 'payconfirm',
        'againhandworkpay', 'entrycancel', 'e_bank_browse', 'refresh', 'unpay', 'allocateagree',
        'commissionpayment', 'receipt', 'evidence',
        'payagree', 'queryapprove', 'e_bank_browse_1'], false);
    // 编辑态
    if (flag) {
        ...
        //待提交,推送
        props.button.setButtonVisible(['add', 'e_bank', 'e_bank_browse', 'copy', 'decide', 'return', 'commit', 'linkgroup', 'print', 'field', 'refresh'], true);
        props.button.setMainButton(['commit'], true);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '1' && srcbusitype == '7') {
        //待提交,手动录入
        props.button.setButtonVisible(['add', 'edit', 'e_bank', 'e_bank_browse', 'delete', 'copy', 'commit', 'linkgroup', 'print', 'field', 'refresh'], true);
        props.button.setMainButton(['commit'], true);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '2') {
        //待审批
        props.button.setButtonVisible(['add', 'copy', 'e_bank', 'e_bank_browse', 'uncommit', 'field', 'linkgroup', 'queryapprove', 'print', 'refresh'], true);
        props.button.setMainButton(['uncommit'], false);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '3') {
        //审批后待支付
        props.button.setButtonVisible(['add', 'copy', 'uncommit', 'e_bank', 'e_bank_browse', 'pay', 'pay_merge', 'field', 'linkgroup', 'print', 'refresh', 'queryapprove'], true);
        props.button.setMainButton(['pay'], true);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '4') {
        //支付中
        props.button.setButtonVisible(['e_bank', 'e_bank_browse', 'add', 'copy', 'pay', 'pay_merge', 'paystatus', 'payconfirm', 'payagree', 'againhandworkpay', 'entrycancel', 'field', 'linkgroup', 'print', 'refresh', 'queryapprove'], true);
        props.button.setMainButton(['payconfirm'], true);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '5' && !ismakevoucher) {
        //支付后制证
        props.button.setButtonVisible(['add', 'copy', 'make','e_bank_browse_1', 'unpay', 'linkgroup', 'payagree', 'receipt', 'evidence', 'print','elecsignformalPrint','elecsigninformalPrint', 'refresh', 'queryapprove'], true);
        props.button.setMainButton(['add'], false);
    } else if (!flag && billstatus == '5' && ismakevoucher) {
        //取消制证
        props.button.setButtonVisible(['add', 'copy', 'e_bank_browse_1','cancelmake', 'field', 'linkgroup', 'payagree', 'receipt', 'evidence', 'print','elecsignformalPrint','elecsigninformalPrint', 'refresh', 'queryapprove'], true);
        props.button.setMainButton(['add'], false);
    ...
    if (flag && !copyflag && status != 'decide') {
        ...
        if (props.getUrlParam('status') == 'add' && !pk_org) {
...
export const hideAllBtn = function (props) {
    ...
    setButtonDisabled(['savegroup', 'cancel', 'edit', 'delete', 'decide', 'return', 'commit',
    'uncommit', '','elecsignformalPrint','elecsigninformalPrint', 'make', 'cancelmake', 'e_bank', 'pay', 'pay_merge', 'paystatus', 'payconfirm',
    'againhandworkpay', 'entrycancel', 'e_bank_browse', 'unpay', 'e_bank_browse_1'], true);

    //若单据状态未支付 则正式打印可用
    if(props.form.getFormItemsValue(formId, 'billstatus').value == '5'){
        setButtonDisabled(['elecsignformalPrint','elecsigninformalPrint'],false);
    }
}
