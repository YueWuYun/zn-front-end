...
export default function buttonClick(props, id, text, record, index, tableId) {
	...
	switch (id) {
		//头部保存
		case 'Save':
			props.validateToSave(saveDataForValidate, this.saveBill.bind(this, false), saveaddobjForValidate, '');
			// this.saveBill(false);
			break;
		//保存提交
		case 'SaveCommit':
		...
		case 'Cancel':
			promptBox({
				color: "warning",
				title: loadMultiLang(props, '36320FAA-000060'),/* 国际化处理： 取消*/
				content: loadMultiLang(props, '36320FAA-000061'),/* 国际化处理： 是否确认取消？*/
		...
		case 'Commit':
			cardOperator(
				...
				[card_table_id],
				'pk_allocateagree_h',
				base_url + 'alloagreecommit.do',
				loadMultiLang(props, '36320FAA-000002'),
				AllocateAgreeCache,
				commitAssign.bind(this),
				true
			); /* 国际化处理： 提交成功！*/ /* 国际化处理： 提交*/
			break;
		//头部 收回
		case 'Uncommit':
			cardOperator(
				...
				[card_table_id],
				'pk_allocateagree_h',
				base_url + 'alloagreeuncommit.do',
				loadMultiLang(props, '36320FAA-000003'),
				AllocateAgreeCache,
		...
		case 'CreateAllocate':
			...
			if (checkBodyDate && checkBodyDate.length > 0) {
				for (let index = 0; index < checkBodyDate.length; index++) {
					...
					bodyPKs.push(
						element.data.values.pk_allocateagree_b && element.data.values.pk_allocateagree_b.value
					);
				}
			}
			...
			cardCreateAllocate(
				...
				[card_table_id],
				'pk_allocateagree_h',
				base_url + 'alloagreecreateallocatecard.do',
				loadMultiLang(props, '36320FAA-000029'),
				AllocateAgreeCache,
				bodyPKs
			); /* 国际化处理： 生成下拨单成功！*/
			break;
		...
		case 'LinkBankAccR': //收款账号
			...
			if (checkAllCardDate.length > 1) {
				if (!checkeddata || checkeddata.length != 1) {
					toast({
						content: loadMultiLang(props, '36320FAA-000030'),
						color: 'warning'
					}); /* 国际化处理： 请选择一行表体进行操作!*/
					return;
				} else {
					...
					checkeddata.forEach((val) => {
						if (val.data.values.pk_bankacc_r && val.data.values.pk_bankacc_r.value) {
				...
				if (checkAllCardDate[0].values.pk_bankacc_r && checkAllCardDate[0].values.pk_bankacc_r.value) {
			...
			if (paymentAccountAllrows.length > 1) {
				if (paymentAccountSelectrow.length != 1) {
					...
					paymentAccountSelectrow.forEach((val) => {
						...
						if (val.data.values.pk_bankacc_p && val.data.values.pk_bankacc_p.value) {
				...
				//处理选择数据
				let restpk_org_p, pk_bankacc_p;
				if (paymentAccountAllrows[0].values.pk_org && paymentAccountAllrows[0].values.pk_org.value) {
					restpk_org_p = paymentAccountAllrows[0].values.pk_org.value;
				}
				if (
					paymentAccountAllrows[0].values.pk_bankacc_p &&
		...
		//表体展开
		case 'Open':
			Open(props, this.tableId, index, record, 'edit');
			break;
...
export const cardCreateAllocate = function (
	that,
	props,
	pageCode,
	headCode,
	...
	ajax({
		...
		success: (res) => {
			...
			if (success) {
				...
				that.toggleShow(), toast({ color: 'success', content: successMess });
...
const saveCommitForValidate = function (props) {
	saveBill.call(this, props, () => {
		cardOperator(
			...
			'pk_allocateagree_h',
			base_url + 'alloagreecommit.do',
			loadMultiLang(props, '36320FAA-000001'),
			AllocateAgreeCache,
			commitAssign.bind(this),
			true
		); /* 国际化处理： 保存提交*/ /* 国际化处理： 提交成功！*/
	});
  }
			...
			setTimeout(() => {
				...
				if (callback) {
					...
					if (!hasTbbMsg) {
						toast({
							color: 'success',
							content: loadMultiLang(this.props, '36320FAA-000035')
						}); /* 国际化处理： 保存成功！*/
						return;
					}
				}
