...
//点击查询，获取查询区数据
export const searchBtnClick = function clickSearchBtn(props, qryCondition, groupKey,isRefresh) {
            ...
            ajax({
                ...
                success: (res) => {
                    ...
                    if (success) {
                        ...
                        //只要刷新动作不报错 就提示刷新成功 不管是否有数据
                        if(isRefresh){
                            toast({ color: 'success', title: loadMultiLang(this.props,'36320FAA-000065') });/* 国际化处理： 刷新成功*/
                        }
                    }else {
    ...
    //如果有查询条件则表明是查询按钮逻辑，故将查询条件存入缓存
            ...
            if(groupKey == '0'){
                ...
                groupKey = this.state.activeKey+1;
                cardCache.setDefData('activeKey', AllocateAgreeCache,groupKey);
            }else if(!this.state.activeKey && cardCache.getDefData('activeKey', AllocateAgreeCache)){
                //这里添加缓存 如果在某个页签下跳到卡片 会取缓存数据
                groupKey = cardCache.getDefData('activeKey', AllocateAgreeCache) + 1;
            }else{
                groupKey = 1;
            }
            ...
            const that = this;
            let pageCode = list_page_id;
            let searchData=JSON.stringify(querydata);
            let reqData = {data:searchData,pageCode};
            ajax({
                url: '/nccloud/sf/allocation/alloagreepagequery.do',
                data: reqData,
                success: (res) => {
                    ...
                    if (success) {
                        if (data) {
                            ...
                            //更新列表数据
                            if (grid) {
                                toast({ color: 'success' , title: loadMultiLang(this.props,'36320FAA-000066')});/* 国际化处理： 查询成功*/
...
const getGroupCondition = function (groupKey) {
    ...
    switch (groupKey) {
        ...
        case 1:
            ...
            //缓存当前的页签值 用于卡片返回列表时使用
            cardCache.setDefData('activeKey', AllocateAgreeCache,0);
            break;
        //待审批
        case 2:
            groupCondition = {
            ...
            cardCache.setDefData('activeKey', AllocateAgreeCache,1);
            break;
        //待下拨
        case 3:
            groupCondition = {
            ...
            cardCache.setDefData('activeKey', AllocateAgreeCache,2);
            break;
        //全部
        case 4:
            groupCondition = {};
            this.setState({ activeKey: 3 });
            cardCache.setDefData('activeKey', AllocateAgreeCache,3);
            break;
