...
let searchId = 'search_allocatelog_01';
let tableId = 'table_allocatelog_01';
let pageId = '36321AALQ_L01';
...
function modifierMeta(props, meta) {
	//缴款单位跨集团
	meta[jsoncode.searchcode].items.find((e) => e.attrcode == 'allocatelog_b.pk_financeorg').isShowUnit = true;
	meta[jsoncode.searchcode].items.map((item) => {
		...
		// 缴款单位银行账户
		if (item.attrcode == 'allocatelog_b.pk_bankacc_r') {
			item.queryCondition = () => {
				let pk_org_psearchVal = props.search.getSearchValByField(jsoncode.searchcode, 'allocatelog_b.pk_financeorg');
				let pkorgVal = '';
					...
					<a
						...
						onClick={() => {
							props.pushTo("/card", {
								...
								status: 'browse',
								id: record.pk_allocatelog_h.value
							});
						}}
					>
