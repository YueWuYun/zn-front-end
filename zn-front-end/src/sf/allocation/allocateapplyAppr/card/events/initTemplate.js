...
import { base_url, card_from_id, card_table_id, card_page_id, module_id, app_code, card_editform_id } from '../../cons/constant';
...
function modifierMeta(props, meta, that) {
		...
		if (item.attrcode == 'pk_bankacc_r') {
			...
			item.queryCondition = () => {
				return {
					...
					refnodename: loadMultiLang(props, '36320AAA-000025'),/* 国际化处理： 使用权参照*/
	...
	let porCol = {
		attrcode: 'opr',
		label: loadMultiLang(props, '36320AAA-000026'),/* 国际化处理： 操作*/
		fixed: 'right',
		itemtype: 'customer',
		visible: true,
		width: '200px',
