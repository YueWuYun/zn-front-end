...
import { card_table_id } from '../../../allocate_pay/cons/constant.js';
...
export default async function bodyButtonClick(props, key, text, record, index) {
    ...
    switch (key) {
        //表体 提交
        case 'commit_inner':
            listSingleOperator(props,list_page_id, list_table_id,base_url + 'commit.do', record, 'pk_allocate_h',index, loadMultiLang(this.props,'1880000025-000001'),dataSource,false, null, (props, data) => {/* 国际化处理： 提交成功！*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
        ...
        //表体 收回
        case 'uncommit_inner':
            listSingleOperator(props,list_page_id,list_table_id, base_url + 'uncommit.do', record, 'pk_allocate_h', index,loadMultiLang(this.props,'1880000025-000002'),dataSource);/* 国际化处理： 收回成功！*/
            break;
        //表体 编辑行
        case 'edit':
        ...
        //表体 删除行
        case 'delete_inner':
            listSingleOperator(props, list_page_id,list_table_id, base_url + 'delete.do', record, 'pk_allocate_h', index, loadMultiLang(this.props,'1880000025-000056'),dataSource);/* 国际化处理： 删除成功！*/
            break;
        //表体经办
        case 'decide':
        ...
        //制证
        case 'make':
            listSingleOperator(props,list_page_id, list_table_id,base_url + 'make.do', record, 'pk_allocate_h',index, loadMultiLang(this.props,'1880000025-000003'),dataSource);/* 国际化处理： 制证成功！*/
            break;
        //取消制证
        case 'unmake':
            listSingleOperator(props,list_page_id, list_table_id,base_url + 'unmake.do', record, 'pk_allocate_h',index, loadMultiLang(this.props,'1880000025-000004'),dataSource);/* 国际化处理： 取消制证成功！*/
            break;
        //支付
        case 'pay_inner':
          ...
          success: (res) => {
              ...
              if (success) {
                if(data.errorMsg){
                    ...
                    toast({ color: 'success', content: this.props.MutiInit.getIntl("36320FA_PAY") && this.props.MutiInit.getIntl("36320FA_PAY").get('36320FA_PAY-000017') +data.warningMsg  });/* 国际化处理： 支付成功*/
                }
...
export const backConfirmInner = (props,record,index,value) => {
    ...
    listSingleOperator(props,list_page_id, list_table_id,base_url + 'allocateback.do', record, 'pk_allocate_h',index, loadMultiLang(this.props,'1880000025-000039'),dataSource);/* 国际化处理： 退回成功！*/
}
