/*lH1FduKEW5AUjgxCxIDkSP+GVucd1Eqx8/bKjAI5ilCCBhNhls4N/vQHVhlHWdad*/
...
export const BatchToast = function(op, status, total, successIndex, failIndex, message, content, that) {
	let operate = '';
	let mutiInit = that.props.MutiInit.getIntl("36320FA_PAY");
	if(op === 'pay'){
		// 国际化处理： 支付
		operate = mutiInit.get('36320FA_PAY-000018');
	}else if(op === 'unpay'){
		// 国际化处理： 取消支付
		operate = mutiInit.get('36320FA_PAY-000070');
	}
	/* 国际化处理：完毕， */
	let title = operate + mutiInit.get('36320FA_PAY-000072');
	let color = 'success';

	if (status == 0) {
		/* 国际化处理：全部失败 */
		// title = title + '全部失败';
		title = title + mutiInit.get('36320FA_PAY-000073');
		color = 'danger';
	}
	else if (status == 1) {
		/* 国际化处理：全部成功 */
		// title = title + '全部成功';
		title = title + mutiInit.get('36320FA_PAY-000074');
		color = 'success';
	}
	else if (status == 2) {
		/* 国际化处理： 部分失败 */
		// title = title+ '部分失败';
		title = title + mutiInit.get('36320FA_PAY-000075');
		color = 'warning';
	}
	if (!content) {
		...
		/* 国际化处理：共 */
		content = mutiInit.get('36320FA_PAY-000071')
		+ operate + total
		/* 国际化处理：条 */
		+ mutiInit.get('36320FA_PAY-000076')
		+','
		/* 国际化处理：成功 */
		+ mutiInit.get('36320FA_PAY-000077') 
		+ successIndex
		/* 国际化处理：条 */
		+ mutiInit.get('36320FA_PAY-000076')
		+','
		/* 国际化处理：失败 */
		+ mutiInit.get('36320FA_PAY-000078')
		+ failIndex 
		/* 国际化处理：条 */
		+ mutiInit.get('36320FA_PAY-000076');
	}
	...
	if (status == 1) {
		...
		toast({
			...
			TextArr: [
				...
				/* 国际化处理： 展开 */
				mutiInit.get('36320FA_PAY-000025'),
				/* 国际化处理：收起 */
				mutiInit.get('36320FA_PAY-000080'),
				/* 国际化处理：我知道了 */
				mutiInit.get('36320FA_PAY-000081') ], 
