...
class Card extends Component {
	...
	componentDidMount() {
		...
		if (this.props.getUrlParam('status') != 'add') {
			let data = { pk: this.props.getUrlParam('id'), pageid: '36321ACLQ_C01' };
			ajax({
				url: '/nccloud/sf/deliverylog/deliverylogquerycard.do',
				data: data,
				success: (res) => {
					if (res.data) {
						if (res.data.head) {
	...
	saveBill = () => {
		...
		if (flag) {
			let CardData = this.props.createMasterChildData('36321ACLQ_C01', this.formId, this.tableId);
			let url = '/nccloud/sf/deliverylog/deliverylogupdateaction.do'; //新增保存
			if (this.props.getUrlParam('status') === 'edit') {
				url = '/nccloud/sf/deliverylog/deliverylogupdateaction.do'; //修改保存
			}
			ajax({
				url: url,
				data: CardData,
				success: (res) => {
					let pk_deliverylog_b = null;
					if (res.success) {
						if (res.data) {
							toast({ color: 'success', content: this.props.MutiInit.getIntl("36321ACLQ") && this.props.MutiInit.getIntl("36321ACLQ").get('36321ACLQ-000004') });/* 国际化处理： 保存成功*/
							if (res.data.head && res.data.head[this.formId]) {
								this.props.form.setAllFormValue({ [this.formId]: res.data.head[this.formId] });
								pk_deliverylog_b = res.data.head[this.formId].rows[0].values.pk_deliverylog_b.value;
							}
							...
							<div className="header-title-search-area">
								{
									createBillHeadInfo(
										{
											title: this.props.MutiInit.getIntl("36321ACLQ") && this.props.MutiInit.getIntl("36321ACLQ").get('36321ACLQ-000005'),
											billCode: null,     //单据号
