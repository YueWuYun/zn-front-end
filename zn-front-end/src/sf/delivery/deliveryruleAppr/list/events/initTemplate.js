...
function modifierMeta(that, props, meta) {
	...
	const tableButtonClick = (props, key, text, record, index) => {
		...
		switch (key) {
			...
			// 提交
			case 'CommitInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchcommit, record, 'pk_deliveryrule_h', index, '提交', jsoncode.dataSource, false, null, (props, data) => {
					let { workflow } = data;
					//有指派信息，则指派，没有则重绘界面
			...
			// 收回
			case 'UncommitInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchuncommit, record, 'pk_deliveryrule_h', index, '收回', jsoncode.dataSource);
				break;
			// 启用
			case 'EnableInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchenable, record, 'pk_deliveryrule_h', index, '启用', jsoncode.dataSource);
				break;
			// 停用
			case 'DisableInner':
				listSingleOperator(props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchdisable, record, 'pk_deliveryrule_h', index, '停用', jsoncode.dataSource);
				break;
			// 补录
