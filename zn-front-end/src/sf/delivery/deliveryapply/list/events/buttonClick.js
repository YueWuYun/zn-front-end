...
export default function buttonClick(props, id) {
    ...
    switch (id) {
        ...
        case 'Refresh':
            ...
            clickSearchBtn.call(this, props, null, this.state.activeKey, 'refresh');
            break;
        //委托办理
        case 'Entrust':
            listMultiOperator(props, jsoncode.pagecode, jsoncode.tablecode, 'pk_deliveryapply_h', requesturl.batchsubmit, loadMultiLang(props, '36320DA-000019'), jsoncode.dataSource);/* 国际化处理： 委托办理*/
            break;
        //取消委托
        case 'Unentrust':
        ...
        case 'Linkplan':
            if (selectedData.length > 1) {
                toast({ color: 'warning', content: loadMultiLang(props, '36320DA-000054') });/* 国际化处理： 仅可选择一条数据联查预算*/
                return;
            }
            if (selectedData.length == 0) {
                toast({ color: 'warning', content: loadMultiLang(props, '36320DA-000051') });/* 国际化处理： 请您选择一条数据*/
                return;
            }
            let data = {
