...
import { setCardShouderBtnUseful, getNewRow } from "../util/index";
// ca
...
class Card extends Component {
				...
				success: (res) => {
					if (res.data) {
						if (res.data.head) {
							...
							this.props.form.setFormItemsDisabled(this.formId, { 'pk_gatherorg': false, 'memo': false, 'busitype': false, 'pk_currtype': false, 'isreversebustype': false });
						}
						if (res.data.body) {
							this.props.cardTable.setTableData(that.tableId, res.data.body[that.tableId]);
							//this.props.cardTable.setStatus(jsoncode.ctablecode, 'edit');
							//解决新增--取消--复制，编辑性错乱，这里统一再赋编辑性一下
							this.props.cardTable.setColEditableByKey(jsoncode.ctablecode, ['memo', 'pk_bankacc_p', 'pk_accid', 'amount', 'pk_planitem_p', 'pk_bankacc_r', 'pk_balatype'], false);
							//begin tm tangleic 20190806 复制操作分录状态后端初始赋值
							//给第一行分录状态设置默认值
							// this.props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, 0, 'recordstatus', { display: loadMultiLang(this.props, '36320DA-000007'), value: '1' });/* 国际化处理： 正常*/
							//end tangleic
						}
	...
	getCardData = (istoast = false) => {
		...
		if (this.props.getUrlParam('status') === 'edit') {
			...
			data = { pk: !this.props.getUrlParam('id') ? this.state.billId : this.props.getUrlParam('id'), pageid: jsoncode.cpageid, status: 'edit' };
		} else {
			...
			data = { pk: !this.props.getUrlParam('id') ? this.state.billId : this.props.getUrlParam('id'), pageid: jsoncode.cpageid };
		}
		if (data.pk) {
			ajax({
	...
	saveBillBeforeCheck = () => {
		...
		if (rowCount <= 0) {
			toast({ color: 'warning', content: loadMultiLang(this.props, '36320DA-000073') })/* 国际化处理： 表体不能为空!*/
			return false;
		}
		// form必输项校验
					...
					success: (res) => {
						...
						if (res.success) {
							if (res.data) {
								...
								//begin tm tangleic 20190301 保存后对是否表体复制操作标志初始化
								// this.setState({
								// 	billId: res.data.head[this.formId].rows[0].values.pk_deliveryapply_h && res.data.head[this.formId].rows[0].values.pk_deliveryapply_h.value
								// });
								this.setState({
									billId: res.data.head[this.formId].rows[0].values.pk_deliveryapply_h && res.data.head[this.formId].rows[0].values.pk_deliveryapply_h.value,
									copyflag: false
								...
								initTemplate.call(this, this.props);
								this.toggleShow();
								//let	pk_deliveryapply_h = res.data.head[this.formId].rows[0].values.pk_deliveryapply_h.value;
							}
					...
					success: (res) => {
						...
						if (res.success) {
							if (res.data) {
								...
								//begin tm tangleic 20190301 保存后将表体复制标志初始化
								// this.setState({
								// 	billno: billnoNo,
							...
							<div className="header-title-search-area">
								{
									createBillHeadInfo(
										{
											title: loadMultiLang(this.props, '36320DA-000028'),  //标题/* 国际化处理： 上缴单*/
				...
				<div>
					{assignShow && <ApprovalTrans
						...
						getResult={(value) => {
							...
							cardOperator(this.props, jsoncode.cpageid, jsoncode.formcode, [jsoncode.ctablecode], 'pk_deliveryapply_h', requesturl.batchcommit, loadMultiLang(this.props, '36320DA-000033'), jsoncode.dataSource, this.toggleShow.bind(this), false, extParam);/* 国际化处理： 提交成功！*/
						}}
						cancel={() => {
							this.setState({ assignShow: false, assignData: null })
						}}
