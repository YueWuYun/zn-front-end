...
import { loadMultiLang, buildLightBodyAfterEditData,bodyRateEditOnAfterEdit } from "../../../../../tmpub/pub/util/index";
...
export default function afterEvent(props, moduleId, key, value, changedrows, i, s, g, haveDefaultPkorg) {
	...
	if (key === 'busitype') {
		...
		if (props.form.getFormItemsValue(jsoncode.formcode, 'busitype').display != undefined) {
			//如果出现编辑后事件 先清除下拨组织
			props.form.setFormItemsValue(jsoncode.formcode, { pk_gatherorg: { display: '', value: '' } });
			ajax({
			...
			ajax({
				...
				success: (res) => {
					...
					if (res.success) {
						let { success, data } = res;
						if (success) {
							let { card, retExtParam, headProp, bodyItemProps, columnPrecisions, rateInfo } = data;
							let { head, bodys } = card;
							//设置表体数据
							props.cardTable.setTableData('table_deliveryapply_C01', bodys.table_deliveryapply_C01);
							//更新列精度
							...
							bodyRateEditOnAfterEdit({
								props,
								bodyCodes: ['table_deliveryapply_C01'],
								rateInfo,
						...
						if (res.success) {
							...
							if (success) {
								...
								if (extParam.hasOwnProperty('pk_AccidData')) {
									...
									setDefData('accidData', jsoncode.dataSource, accidDefDate);
								} else {
									setDefData('accidData', jsoncode.dataSource, null);
								}
							}
						}
					}
	...
	//是否网银支付
	if (key === 'isnetpay') {
		let eventData = buildLightBodyAfterEditData(props, jsoncode.cpageid, this.formId, this.tableId, key, changedrows);
	...
	//申请金额
	if (key === 'amount') {
		//begin xuhrc 2019-2-15 单独输入-号时，自动赋予默认值0.00
		let amount = props.cardTable.getValByKeyAndIndex(jsoncode.ctablecode, i, 'amount').value;
		if (amount == '-') {
			props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, i, 'amount', { value: '0.00', display: '' });
			return
		}
		//end xuhrc 2019-2-15 单独输入-号时，自动赋予默认值0.00
		//let eventData = buildLightBodyAfterEditData(props, jsoncode.cpageid, this.formId, this.tableId, key, changedrows);
	...
	if (key === 'pk_bankacc_p') {
		...
		props.cardTable.setEditableByIndex(jsoncode.ctablecode, i, 'isnetpay', true);
		let eventData = buildLightBodyAfterEditData(props, jsoncode.cpageid, this.formId, this.tableId, key, changedrows);
		eventData['areaname'] = this.tableId;
		...
		if (pk_bankacc_p.value) {
			...
			if (pk_bankacc_p && pk_bankacc_r
				...
				//props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, i, 'pk_bankacc_r', { value: '', display: '' });BANKNAME_P
				return;
			}
			ajax({
				url: requesturl.afterevent,
	...
	if (key === 'pk_bankacc_r') {
		...
		let pk_bankacc_p = props.cardTable.getValByKeyAndIndex(jsoncode.ctablecode, i, 'pk_bankacc_p');
		let pk_bankacc_r = props.cardTable.getValByKeyAndIndex(jsoncode.ctablecode, i, 'pk_bankacc_r');
		let pk_gatherorg = props.form.getFormItemsValue(this.formId, 'pk_gatherorg');
		if (!pk_bankacc_p.value) {
		...
		if (!pk_gatherorg.value) {
			toast({ color: 'warning', content: loadMultiLang(props, '36320DA-000072') });/* 国际化处理： 请先选择上收组织*/
			props.cardTable.setValByKeyAndIndex(jsoncode.ctablecode, i, 'pk_bankacc_r', { value: '', display: '' });
			return
		}
		//判断上缴银行账户不能和上收银行账户相同
