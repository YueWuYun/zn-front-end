...
export default function bodyBeforeEvent(props, moduleId, key, value, i, record, status) {
	...
	switch (key) {
		...
		//汇款速度
		case "pay_type":
		let isnetpay = props.cardTable.getValByKeyAndIndex(jsoncode.ctablecode, i, 'isnetpay')&& props.cardTable.getValByKeyAndIndex(jsoncode.ctablecode, i, 'isnetpay').value;
		if(isnetpay){
			return true
		}else{
			return false;
