...
function modifierMeta(that, props, meta) {
	...
	meta[tableId].items.find((e) => e.attrcode === 'isreceipttally').initialvalue = { value: 'N', scale: -1, display: 'N' };
	meta['childform1_deliveryapply_01'].items.find((e) => e.attrcode === 'isreceipttally').initialvalue = { value: 'N', scale: -1, display: 'N' };
	meta['childform2_deliveryapply_01'].items.find((e) => e.attrcode === 'isreceipttally').initialvalue = { value: 'N', scale: -1, display: 'N' };
	...
	meta[tableId].items.map((item) => {
		...
		// 					queryCondition: () => {
		// 						return {
		// 							TreeRefActionExt: 'nccloud.web.sf.delivery.deliveryapply.filter.DefaultRecBankRefAccFilter'
		// 						};
		// 					}
		// 				})
		// 			);
		...
		// 			GridRefActionExt: 'nccloud.web.sf.delivery.deliveryapply.filter.DefaultRefAccFilter'
		// 			//GridRefActionExt: 'nccloud.web.sf.delivery.delivery.filter.DefaultRefAccFilter4NCC'
		// 			//GridRefActionExt:'nccloud.web.sf.allocation.allocateapply.filter.DefaultRefAccFilter'
		// 		};
		// 	};
		// }
	...
	let porCol = {
		...
		render(text, record, index) {
			let buttonAry = getBtnArr.call(that, props, record);
...
const getBtnArr = function (props, record) {
	let status = props.cardTable.getStatus(tableId);
	let copyflag = this.state.copyflag;
	if (copyflag) {
		return ['copythis'];
	}
