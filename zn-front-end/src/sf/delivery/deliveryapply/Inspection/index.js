...
const columns2 = [
    {
        title: this.state.json['36320DA-000034'],/* 国际化处理： 主体*/
        dataIndex: "main",
        key: "main",
    },
    {
        title: this.state.json['36320DA-000035'],/* 国际化处理： 系统*/
        dataIndex: "sys_name",
        key: "sys_name",
    },
    {
        title: this.state.json['36320DA-000036'],/* 国际化处理： 单据类型*/
        dataIndex: "bill_type",
        key: "bill_type"
    },
    {
        title: this.state.json['36320DA-000037'],/* 国际化处理： 基本档案*/
        dataIndex: "basic_file",
        key: "basic_file"
    },
    {
        title: this.state.json['36320DA-000038'],/* 国际化处理： 执行数*/
        dataIndex: "u_data",
        key: "u_data"
    },
    {
        title: this.state.json['36320DA-000039'],/* 国际化处理： 预占数*/
        dataIndex: "pre_data",
        key: "pre_data"
    }
];
export default class Inspection extends Component {
    ...
    static defaultProps = {
        ...
        // 取消回调函数
        cancel: () => {
            console.warn(this.state.json['36320DA-000040']);/* 国际化处理： 没有设置取消回调函数*/
        },
        // 确认回调函数
        affirm: () => {
            console.warn(this.state.json['36320DA-000041']);/* 国际化处理： 没有设置确认回调函数*/
        }
    }
    componentDidMount() { }
    ...
    nextStep(info) {
        ...
        if (info.target === 'm_readydata') {
            ...
            console.warn(this.state.json['36320DA-000042'])/* 国际化处理： 点击类型错误*/
        }
        let param = {
    ...
    affirm() {
        if (this.state.selectValue === undefined) {
            toast({
                'color': 'warning',
                'content': `${this.state.json['36320DA-000043']}`/* 国际化处理： 请选择数据*/
            })
            return;
        ...
        let content = (() => {
            if (this.state.step === 1) {
                return (
                    ...
                    >
                        <NCModal.Header>
                            <NCModal.Title>this.state.json['36320DA-000044']/* 国际化处理： 预算执行情况*/
                        ...
                        <NCModal.Footer>
                            <NCButton className="button-undefined u-button-border" onClick={this.props.cancel}>this.state.json['36320DA-000012']</NCButton>/* 国际化处理： 取消*/
                        </NCModal.Footer>
                    </NCModal>
                )
            } else {
                return (
                    ...
                    >
                        <NCModal.Header>
                            <NCModal.Title>this.state.json['36320DA-000045']/* 国际化处理： 联查执行*/
                        ...
                        <NCModal.Footer>
                            <NCButton className="button-primary" onClick={this.affirm}>this.state.json['36320DA-000046']</NCButton>/* 国际化处理： 联查*/
                            <NCButton className="button-undefined u-button-border" onClick={this.cancelDialog}>this.state.json['36320DA-000012']</NCButton>/* 国际化处理： 取消*/
                        </NCModal.Footer>
