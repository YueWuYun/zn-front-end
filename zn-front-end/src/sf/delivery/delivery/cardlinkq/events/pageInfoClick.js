...
import {
    app_id, card_page_id, card_from_id,
    card_fromtail_id, card_table_id, link_card_page_id,
    dataSourceLink
...
export default function (props, pks) {
    ...
    */
    if (pks) {
        let cardData = getCacheById(pks, dataSourceLink);
        if (cardData) {
                ...
                success: (res) => {
                    if (res.data) {
                        ...
                        if (res.data.head) {
                            ...
                            * dataSource: 缓存数据命名空间
                            */
                            updateCache('pk_delivery_h', pks, res.data, card_from_id, dataSourceLink);
                        ...
                        //begin tm tangleic 20190906 卡片界面状态变化不要pushTo 屡教不改！！！垃圾
                        // props.pushTo("/card",{
                        //     status: 'browse',
                        //     id: pks,
