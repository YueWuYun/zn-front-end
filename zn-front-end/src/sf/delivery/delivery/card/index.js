...
import { buttonClick, initTemplate, beforeEvent, afterEvent, pageInfoClick, buttonVisible, setButtonUsability } from './events';
// 网银补录
...
import {
	...
	card_fromtail_id, card_table_id, card_table_id_edit,deliveryBillType,deliveryPk,
	dataSource
} from '../cons/constant';
import Modal from '../../../../tmpub/pub/util/modal/index';
import { clsRowno, operErr } from "../util/index";

let { NCFormControl, NCAnchor, NCScrollLink, NCScrollElement, NCAffix, NCDiv } = base;
//返回button
...
class Card extends Component {
	...
	refresh = () => {
		...
		if (status === 'browse') {
			...
			if (pk) {
				...
				this.props.cardTable.setTableData(this.tableId, { rows: [] });
				//begin yangjn 20181212 修改当只有一条数据时 在单据卡片态点击删除后 单据号不应该显示
				this.backvbillno = '';
				//end
				this.toggleShow();
			}
		...
		//复制
			...
			ajax({
				...
				error: (e) => {
					operErr({ props: this.props, headCode: this.formId, bodyCode: this.tableId, repainFunc: this.toggleShow.bind(this), error: e });
				}
			});
		}
		//经办
				...
				success: (res) => {
					if (res.data) {
						if (res.data.billCard.head) {
							...
							that.props.form.setFormItemsDisabled(card_from_id, { 'busitype': true });
						}
						// 设置字段编辑性
	...
	toggleShow = () => {
		...
		if (status === 'add' || status === 'edit' || status === 'decide' || status === 'copy') {
			let pk_org = this.props.form.getFormItemsValue(card_from_id, 'pk_org');
						...
						if (res.data.billCard.head) {
                            ...
                            */
							updateCache('pk_delivery_h', res.data.billCard.head[card_from_id].rows[0].values.pk_delivery_h.value,
								res.data.billCard, card_from_id, dataSource);
							this.backvbillno = res.data.billCard.head[card_from_id].rows[0].values.vbillno.value;
							this.props.setUrlParam({
								status: 'browse',
								id: res.data.billCard.head[card_from_id].rows[0].values.pk_delivery_h.value,
							})
						}
						if (res.data.billCard.body) {
	...
	saveBill = async () => {
		...
		if (saveflag) {
			...
			let cardData = createSimpleBillData(this.props, card_page_id, card_from_id, card_table_id, false);
					...
					if (res.success) {
						if (res.data) {
							...
							if(!hasTbbMsg){
								toast({
									color: 'success', content: this.props.MutiInit.getIntl("36320FDA")
										&& this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000040')/* 国际化处理： 保存成功*/
								});
							}
				...
				if (res.success) {
					if (res.data && res.data.errorMsg) {
						...
						if (res.data.workflow &&
							...
							if (res.data.billCard.head) {
								this.props.form.setAllFormValue({ [card_from_id]: res.data.billCard.head[card_from_id] });
								updateCache('pk_delivery_h', pk_delivery_h && pk_delivery_h.value, res.data.billCard, card_from_id, dataSource);
							}
							if (res.data.billCard.body) {
								this.props.cardTable.setTableData(card_table_id, res.data.billCard.body[card_table_id]);
							}
	...
	backConfirm = (value) => {
		...
		let backData = {
			...
			isCardOpt: true,
			returnnote: value,
			pkMapTs: { [this.props.getUrlParam('id')]: this.ts }
		};
		ajax({
	...
	render() {
		...
		// //console.log("backvbillno:", this.backvbillno);
		return (
			<div className="nc-bill-card">
					...
					<NCScrollElement name='businfo'>
						<div className="nc-bill-table-area">
							...
							{createCardTable(this.tableId, {
								...
								tableHead: this.getTableHead.bind(this, buttons, card_table_id),
				...
				<div>
					...
					{this.state.compositedisplay ? <ApprovalTrans
						/* 国际化处理： 指派*/
						title={this.props.MutiInit.getIntl("36320FDA")
							&& this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000120')}
				...
				<div>
					<Modal
						title={this.props.MutiInit.getIntl("36320FDA")
							&& this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000090')}
						// label ='退回原因'
						/* 国际化处理： 退回*/
						label={this.props.MutiInit.getIntl("36320FDA")
