...
export default function buttonClick(props, id) {
    ...
    function getrequestdata() {
        let selectdata = props.table.getCheckedRows('table_deliveryrule_01');
        if (!selectedData || selectdata.length == 0) {
            toast({
                color: 'warning',
                content: loadMultiLang(props,'36320ACC-000028')/* 国际化处理： 请选择数据*/
            });
            return;
        }
        let indexArr = [];
        ...
        selectdata.forEach((val) => {
            dataArr.push(val.data.values.pk_deliveryrule_h.value);
    ...
    switch (id) {
        ...
        promptBox({
            color: "warning",
            title: loadMultiLang(props,'36320ACC-000023'),/* 国际化处理： 删除*/
            content: loadMultiLang(props,'36320ACC-000029'),/* 国际化处理： 确定要删除吗？*/
        ...
        // 头部 提交
        case 'Commit':
            listMultiOperator(props, jsoncode.pagecode, jsoncode.tablecode, 'pk_deliveryrule_h', requesturl.batchcommit, loadMultiLang(props,'36320ACC-000007'),jsoncode.dataSource, false, null, (props, data) => {/* 国际化处理： 提交*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
				if (workflow && workflow == 'approveflow' || workflow == 'workflow') {
					this.setState({
						...
						billID: selectedData[0] && selectedData[0].data && selectedData[0].data.values && selectedData[0].data.values['pk_deliveryrule_h'] && selectedData[0].data.values['pk_deliveryrule_h'].value,
        ...
        // 头部 收回
        case 'Uncommit':
            listMultiOperator(props, jsoncode.pagecode, jsoncode.tablecode, 'pk_deliveryrule_h', requesturl.batchuncommit, loadMultiLang(props,'36320ACC-000012'), jsoncode.dataSource);/* 国际化处理： 收回*/
            break;
        // 头部 启用
        case 'Enable':
            listMultiOperator(props, jsoncode.pagecode, jsoncode.tablecode, 'pk_deliveryrule_h', requesturl.batchenable, loadMultiLang(props,'36320ACC-000010'), jsoncode.dataSource);/* 国际化处理： 启用*/
            break;
        // 头部 停用
        case 'Disable':
            listMultiOperator(props, jsoncode.pagecode, jsoncode.tablecode, 'pk_deliveryrule_h', requesturl.batchdisable, loadMultiLang(props,'36320ACC-000011'), jsoncode.dataSource);/* 国际化处理： 停用*/
            break;
        //刷新
        case 'Refresh':
...
const delConfirm = function (props) {
    ...
    if (!selectedData || selectedData.length == 0) {
        ...
        toast({ color: 'warning', content: loadMultiLang(props,'36320ACC-000028') });/* 国际化处理： 请选择数据*/
        return
    }
    ...
    let delObj = {
        ...
        values: {
            ts: {
                display: loadMultiLang(props,'36320ACC-000030'),/* 国际化处理： 时间戳*/
            },
            pk_deliveryrule_h: {
                display: loadMultiLang(props,'36320ACC-000031'),/* 国际化处理： 主键*/
            }
        }
    };
    //处理选择数据
    selectedData.forEach((val) => {
        ...
        dataArr.push(val.data.values.pk_deliveryrule_h.value);//主键数组
