        ...
        if (qryCondition) {
            ...
            ajax({
                ...
                success: (res) => {
                    ...
                    if (success) {
                        if(isRefresh){
                            toast({ color: 'success', content: loadMultiLang(props, '36320ACC-000040') });
                        }
                        let { numvalues } = data;
                        if (data.grid) {
                    ...
                    if (success) {
                        ...
                        if (data.grid) {
                            if(isRefresh){
                                ...
                                toast({ color: 'success', content: loadMultiLang(props, '36320ACC-000041') });
                            }
                            props.table.setAllTableData(that.tableId, data.grid[that.tableId]);
                        } else {
                            if(isRefresh){
                                ...
                                toast({ color: 'warning', content: loadMultiLang(props, '36320ACC-000033') });/* 国际化处理： 未查询出符合条件的数据*/
                            } else {
                                toast({ color: 'success', content: loadMultiLang(props, '36320ACC-000041') });
                            }
                            props.table.setAllTableData(that.tableId, { rows: [] });
