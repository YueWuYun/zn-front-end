			...
			<div className="nc-bill-list">
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
					<div className="header-title-search-area">
						{createBillHeadInfo(
							{
								title: loadMultiLang(this.props, '36320ACC-000022'),//标题
				...
				{/* <div className="tab-definInfo-area"> */}
					<NCTabs activeKey={this.state.activeKey}
						...
						<NCTabPane key={'0'} tab={
						<span>
							{loadMultiLang(this.props, '36320ACC-000034')/* 国际化处理： 待提交*/
								+ '('}<span>{groupCount.needCommit || 0}</span>{')'}
						</span>}
						/>
						<NCTabPane key={'1'} tab={
						<span>
							{loadMultiLang(this.props, '36320ACC-000035')/* 国际化处理： 审批中*/
								+ '('}<span>{groupCount.needApprove || 0}</span>{')'}
						</span>}
						/>
						<NCTabPane key={'2'} tab={
						<span>
							{loadMultiLang(this.props, '36320ACC-000036')/* 国际化处理： 全部*/}
						</span>}
						/>
					</NCTabs>
				{/* </div> */}
				...
				<div>
					{assignShow && <ApprovalTrans
						...
						getResult={(value) => {
							...
							listSingleOperatorNoRecord(this.props, jsoncode.pagecode, jsoncode.tablecode, requesturl.batchcommit, billID, ts, index, loadMultiLang(this.props,'36320ACC-000037'), jsoncode.dataSource, true, extParam);/* 国际化处理： 提交成功*/
