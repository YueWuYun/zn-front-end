...
class Card extends Component {
		...
		if (data.pk) {
			ajax({
				...
				success: (res) => {
					if (res.data) {
						if(isRefresh){
							loadMultiLang(this.props, '36320ACC-000040'); /** 刷新成功 */
						}
						if (res.data.head) {
							this.props.form.setAllFormValue({ [this.formId]: res.data.head[this.formId] });
						}
	...
	toggleShow = () => {
		...
		//切换组织，或者新增时，操作信息编辑性错误，找不到原因，重新设置一遍
		this.props.form.setFormItemsDisabled(jsoncode.formcode, { 'billmaker': true, 'dbilldate': true, 'approver': true, 'dapprovedate':true, 'dapprovetime':true, 'vapprovenote': true, 'vbillstatus': true });
		if (status == 'browse') {
			//设置卡片头部状态
			this.props.BillHeadInfo.setBillHeadInfoVisible({
	...
	saveBillBeforeCheck = () => {
		...
		if(flag){
			...
			if (rowCount <= 0) {
				toast({ color: 'warning', content: loadMultiLang(this.props, '36320ACC-000038') })/* 国际化处理： 表体不能为空!*/
				return false;
			}
		}
		return flag;
