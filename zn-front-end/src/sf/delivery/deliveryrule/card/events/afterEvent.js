...
export default function afterEvent(props, moduleId, key, value, changedrows, i, s, g, haveDefaultPkorg) {
	...
	if (key === 'pk_bankacc_r') {
		...
		let pk_bankacc_p = props.cardTable.getColValue('table_deliveryrule_C01', 'pk_bankacc_p').map(item => item.value);
		if (pk_bankacc_r.value && pk_bankacc_p.includes(pk_bankacc_r.value)) {
			toast({ color: 'warning', content: loadMultiLang(props, '36320ACC-000002') });/* 国际化处理： 上收银行账户不能和单位银行账户相同，请重新选择*/
			props.cardTable.setValByKeyAndIndex('table_deliveryrule_C01', pk_bankacc_p.indexOf(pk_bankacc_r.value), 'pk_bankacc_p', { value: '', display: '' });
			return;
		}
		if (newvalue != null) {
			ajax({
				...
				success: (res) => {
					...
					props.cardTable.updateDataByRowId(jsoncode.ctablecode, bodys.table_deliveryrule_C01);
				...
				success: (res) => {
					if (res.success) {
						...
						if (success) {
							...
							//xuhrc 20190216 改为以下这种方式更新表体，否则用上面屏蔽的这种方式，会引起子表编辑性的错乱
							props.cardTable.updateDataByRowId(jsoncode.ctablecode, bodys.table_deliveryrule_C01);
						}
					}
				}
			...
			if (success) {
				...
				if (key == 'pk_org') {
					...
					props.cardTable.addRow(
						jsoncode.ctablecode,
						0,
						{ 'paytype': { display: loadMultiLang(props, '36320ACC-000004'), value: '0' }, 'pk_org': pk_org,'pk_group': pk_group },/* 国际化处理： 普通*/
					);
