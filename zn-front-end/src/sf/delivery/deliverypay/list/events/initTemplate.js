...
import {
	list_page_id, list_search_id, list_table_id, card_page_id, dataSource
		...
		function (data) {
			if (data) {
				...
				if (data.button) {
					...
					/* 国际化处理： 确认要删除该信息吗？*/
					props.button.setPopContent('delete_inner', that.props.MutiInit.getIntl("36320FDA")
						&& that.props.MutiInit.getIntl("36320FDA").get('36320FDA--000063'));
				}
			}
		}
	)
		...
		if (item.attrcode == 'delivery_b.pk_bankacc_r') {
			item.queryCondition = () => {
				...
				return {
					...
					refnodename: props.MutiInit.getIntl("36320FDA") && props.MutiInit.getIntl("36320FDA").get('36320FDA--000048'),/* 国际化处理： 使用权参照*/
					//自定义增加的过滤条件
