...
class Card extends Component {
	...
	componentDidMount() {
		...
		if (srcbilltype) {
			...
			if (srcbilltype === '36KC' || srcbilltype === '36K3'
				|| srcbilltype === '36J1' || srcbilltype === '36S3') {
	...
	saveBill = () => {
		...
		if (saveflag) {
			...
			let result = Sign({
				isSign: true, isKey: false, data: cardData,
					...
					<NCAffix>
						...
						<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
							<div className="header-title-search-area">
								{createBillHeadInfo({
									// {/* 国际化处理： 资金上收支付*/}
									title: this.props.MutiInit.getIntl("36320FDA") && this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000091'),
									//单据号
									billCode: this.state.vbillno,
									//返回按钮的点击事件 
									backBtnClick: () => { this.ncBackBtnClick() }
								})}
								{
									this.props.BillHeadInfo.setBillHeadInfoVisible({
										showBackBtn: this.props.getUrlParam('status') == 'browse' && showNCbackBtn,  //控制显示返回按钮
										showBillCode: this.state.vbillno ? true : false,  //控制显示单据号
										billCode: this.state.vbillno
									})
								}
							</div>
