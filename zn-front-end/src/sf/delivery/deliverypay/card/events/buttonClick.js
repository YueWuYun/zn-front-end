...
import {
  ...
  dataSource, normalappcode, printnodekey_inoffical, printnodekey_offical, url, state, field
  ...
  switch (id) {
          ...
          if (res.success) {
            if (res.data && res.data.errorMsg) {
              ...
              if (res.data.billCard.head) {
                ...
                updateCache('pk_delivery_h', props.getUrlParam('id'), res.data, card_from_id, dataSource);
              }
    ...
    case 'netpayagain':
      if (checkBodyData.length > 0) {
        ...
        promptBox({
          color: "warning",
          /* 国际化处理： 对选中的表体行进行再次网银支付?*/
          content: this.props.MutiInit.getIntl("36320FDA") && this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000020'),
          //点击确定按钮事件
          beSureBtnClick: buttonClick.bind(this, props, 'netpayagainConfirm'),
        });
      } else {
    ...
    case 'payagain':
      if (checkBodyData.length > 0) {
        ...
        promptBox({
          color: "warning",
          /* 国际化处理： 对选中的表体行进行再次手工支付?*/
          content: this.props.MutiInit.getIntl("36320FDA") && this.props.MutiInit.getIntl("36320FDA").get('36320FDA--000024'),
          //点击确定按钮事件
          beSureBtnClick: buttonClick.bind(this, props, 'payagainConfirm'),
        });
      } else {
...
const saveAdd = async function (that, props) {
  ...
  if (saveaddflag) {
    ...
    let result = await Sign({ isSign: true, isKey: false, data: saveaddData, encryptVOClassName: 'nccloud.pubitf.sf.delivery.delivery.DeliveryEncryptVO4NCC' });
    if (result.isStop) {
      return;
    }
        ...
        if (res.success) {
          if (res.data) {
            ...
            if (res.data.body) {
              ...
              that.props.cardTable.updateDataByRowId(card_table_id, res.data.body[card_table_id]);
            }
            // 清空表单form所有数据
              ...
              if (res.data.billCard.head) {
                ...
                if (savecommitBeforePk && savecommitBeforePk.value) {
                  updateCache('pk_delivery_h', res.data.billCard.head[card_from_id].rows[0].values.pk_delivery_h.value,
                    res.data, card_from_id, dataSource, res.data.billCard.head[card_from_id].rows[0].values);
                } else {
                  addCache(res.data.billCard.head[card_from_id].rows[0].values.pk_delivery_h.value, res.data,
                    card_from_id, dataSource, res.data.billCard.head[card_from_id].rows[0].values);
                }
              }
              if (res.data.billCard.body) {
                // props.cardTable.setTableData(card_table_id, res.data.billCard.body[card_table_id]);
                props.cardTable.updateDataByRowId(card_table_id, res.data.body[card_table_id]);
              }
