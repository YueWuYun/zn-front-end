...
let searchId = '36320FCRF_list_search';
let tableId = '36320FCRF_list_table';
let pageId = '36320FCRF_L01';
	...
	props.createUIDom(
		{
			...
			appid: '0001Z61000000002HF6I'//注册按钮的id
		},
		function (data) {
			if (data) {
				if (data.template) {
				...
				if (!getDefData(dataSource, islink)) {
					//查询模板主组织无法自动赋值 在这里手动赋值
					if (data.context) {
						let context = data.context;
						// 设置查询区某个字段的值
						props.search.setSearchValByField(searchId, 'pk_org_r',
							{
...
function modifierMeta(props, meta, that) {
	...
	meta[searchId].items.map((item) => {
		...
		//币种过滤 付款银行账户
		//根据 pk_org_p 过滤 付款银行账户pk_bankacc_p
		if (item.attrcode == 'pk_bankacc_p') {
			item.queryCondition = () => {
				//币种
				let search_currtype_p = props.search.getSearchValByField('36320FCRF_list_search', 'pk_currtype');//币种
				...
				//下拨组织
				let search_payorg_value = props.search.getSearchValByField('36320FCRF_list_search', 'pk_org_p');
				...
				return {
					...
					noConditionOrg: 'Y',
					refnodename: loadMultiLang(props, '36320FCRF-000024'),/* 国际化处理： 使用权参照*/
		...
		if (item.attrcode == 'pk_bankacc_r') {
			item.queryCondition = () => {
				//币种过滤
				let search_currtype_r = props.search.getSearchValByField('36320FCRF_list_search', 'pk_currtype');//币种
				...
				//上收组织
				let search_payorg_value = props.search.getSearchValByField('36320FCRF_list_search', 'pk_org_r');
				if (search_payorg_value) {
					search_payorg_value = search_payorg_value.value.firstvalue;
				} else {
					search_payorg_value = null;
