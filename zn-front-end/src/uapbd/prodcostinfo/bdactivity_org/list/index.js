...
class SingleTable extends Component {
	...
	getTableData = (btnName) => {
		ajax({
			...
			success: (res) => {
				if (btnName == 'Refresh') {
					toast({ color: 'success', content: this.state.json['10140ACTB-000003'] });/* 国际化处理： 刷新成功*/
				}
				if (res.data) {
	...
	save = () => {
		...
		if (status == 'browse') {//如果是浏览态就是走删除的逻辑
			...
			allRows.forEach(item => {
				if(!item.values.pk_org.value){
					item.values.pk_org.value=this.state.pk_org.refpk
				}
			...
			success: (res) => {
				if (res.success) {
					...
					if (status == 'browse') {
						...
						toast({ color: 'success', content: this.state.json['10140ACTB-000014'] });/* 国际化处理： 保存成功*/
					}
					//保存成功重新查询一遍数据
...
enableFun =(flag,pk,index1) =>  {
	...
	if(flag=='batch'){//批量
		...
		if(!checkedData || checkedData.length==0){
			toast({ content: `${that.state.json['10140ACTB-000017']}`, color: 'warning' });/* 国际化处理： 没有选择需要启用的数据，请重新选择！*/
			return;
		}
			...
			promptBox({
				color: 'warning',
				title: that.state.json['10140ACTB-000020'],
				content: that.state.json['10140ACTB-000019'],/* 国际化处理： 您确定要启用所选数据吗？*/
						...
						//更改数据
							if (data&&data.grid) {
								...
								for (let key in grid) {
									...
									// toast({ title: that.state.json['10140ACTB-000029'], color: 'success' });/* 国际化处理： 启用成功*/
						}
						// 页面新数据放入缓存中
					}
				});
			...
			success: function(res) {
				let { success, data } = res;
					if (data&&data.grid) {
						...
						that.props.editTable.updateDataByIndexs(that.tableId, updateValue);
							toast({ title: that.state.json['10140ACTB-000022'], color: 'success' });/* 国际化处理： 启用成功*/
				}
				// 页面新数据放入缓存中
			}
		});
...
disableFun =(flag,pk,index1) =>  {
	...
	if(flag=='batch'){//批量
		let checkedData = that.props.editTable.getCheckedRows(that.tableId);
		if(!checkedData || checkedData.length==0){
			toast({ content: `${that.state.json['10140ACTB-000027']}`, color: 'warning' });/* 国际化处理： 没有选择需要停用的数据，请重新选择！*/
			return;
		}
			checkedData.forEach((val) => {
				checkedObj.push({
		...
		promptBox({
			color: 'warning',
			title: that.state.json['10140ACTB-000020'],
			content: that.state.json['10140ACTB-000018'],/* 国际化处理： 您确定要停用所选数据吗？*/
			beSureBtnClick: function() {
			ajax({
				...
				success: function(res) {
					let { success, data } = res;
						if (data&&data.grid) {
							...
							that.props.editTable.updateDataByIndexs(that.tableId, updateValue);
							toast({ title: that.state.json['10140ACTB-000023'], color: 'success' });/* 国际化处理： 禁用成功*/
						}
					// 页面新数据放入缓存中
				}
			});
 ...
 */
 onPrint = ()=>{
	...
	if (index1.length == 0) {
		toast({ content: `${that.state.json['10140ACTB-000024']}，${that.state.json['10140ACTB-000025']}！`, color: 'warning' });/* 国际化处理： 未选中任何行,不允许操作打印*/
		return;
	}
	print(
...
onOutput = ()=>{
	...
	if (index1.length == 0) {
		toast({ content: `${that.state.json['10140ACTB-000024']}，${that.state.json['10140ACTB-000026']}！`, color: 'warning' });/* 国际化处理： 未选中任何行,不允许操作打印输出*/
		return;
	...
	toggleShow = () => {
		...
		if(status == 'browse'){//控制删除提示框
			this.props.button.setPopContent('Delete_inner', this.state.json['10140ACTB-000006']);/* 国际化处理： 确定删除吗？*/
		}else{
				...
				<NCDiv areaCode={NCDiv.config.HEADER}>
					<div className="nc-bill-header-area" >
						<div className="header-title-search-area">
							{createBillHeadInfo({
								title: this.state.json['10140ACTB-000000'],//国际化处理： 成本类型-业务单元
								initShowBackBtn: false
							})}
							<ReferLoader
								tag="test12"
								refName={this.state.json['10140ACTB-000016']}
								placeholder={this.state.json['10140ACTB-000016']}
								...
								queryCondition={{
									...
									TreeRefActionExt: 'nccloud.web.refer.sqlbuilder.PrimaryOrgSQLBuilder,nccloud.web.uapbd.bdactivity.handler.PkOrgBeforeSqlBuilder'
								}}
							/>
							...
							{this.state.disabled ? null ://参照可编辑的时候才可以看到该框
								...
								<div className="title-search-detail" style={{ lineHeight: '30px' }}>
									<NCCheckbox
										...
										{this.state.json['10140ACTB-000004']}</NCCheckbox>
								</div>
							}
