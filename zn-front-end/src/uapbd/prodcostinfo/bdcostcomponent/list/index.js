...
class SingleTable extends Component {
	...
	getTableData = (btnName) => {
		ajax({
			...
			success: (res) => {
				if (btnName == 'Refresh') {
					toast({ color: 'success', content: this.state.json['10140CCPN-000003'] });/* 国际化处理： 刷新成功*/
				}
				if (res.data) {
	...
	save = () => {
		...
		if(count != set.size){
			toast({ color: 'warning', content: this.state.json['10140CCPN-000017'] });/* 国际化处理： 成本组件的编码不能重复！*/
			return;
		}
		let grid = {
			model: {
			...
			success: (res) => {
				if (res.success) {
					if (status == 'browse') {
						...
						toast({ color: 'success', content: this.state.json['10140CCPN-000014'] });/* 国际化处理： 保存成功*/
					}
					//保存成功重新查询一遍数据
	...
	toggleShow = () => {
		...
		if(status == 'browse'){//控制删除提示框
			this.props.button.setPopContent('Delete_inner', this.state.json['10140CCPN-000006']);/* 国际化处理： 确定删除吗？*/
		}else{
				...
				<NCDiv areaCode={NCDiv.config.HEADER}>
					<div className="nc-bill-header-area" >
						<div className="header-title-search-area">
							{createBillHeadInfo({
								title: this.state.json['10140CCPN-000000'],//国际化处理： 成本组件
								initShowBackBtn: false
							})}
							<ReferLoader
								tag="test"
								refName={this.state.json['10140CCPN-000016']}
								placeholder={this.state.json['10140CCPN-000016']}
							...
							{/* {this.state.disabled ? null ://参照可编辑的时候才可以看到该框
								...
								<div className="title-search-detail" style={{ lineHeight: '30px' }}>
									<NCCheckbox
										...
										{this.state.json['10140CCPN-000004']}</NCCheckbox>
								</div>
							} */}
