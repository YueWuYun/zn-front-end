...
import { pagecode,tableId1 } from '../card/constants';

const { NCModal, NCButton, NCMessage } = base
const { Transfer } = high

class CardSubCF extends Component {
    constructor(props) {
...
    }
    ...
    save = (value) => {
        let that = this;
        let distriVos = that.props.cardTable.getAllData(tableId1);
		...
		beSureBtnClick: function () {
			ajax({
				url: '/nccloud/uapbd/costcompstruc/cardDistributeOrgs.do',
				data: {
                    pk_bill:pk_bill,
                    pk_orgs:targetOrgs,
                    pageId:pagecode
				...
				success: (res) => {
					...
					if (success) {
                        if(data){
                            ...
                            if(res.data.recard){
                                if(res.data.recard.bodys[tableId1]){
                                    that.props.parent.setTableDistri(res.data.recard.bodys[tableId1]);
                                    //that.props.cardTable.setTableData(tableId1, res.data.recard.bodys[tableId1]);
                                }
                            }
                        }
					}
...
CardSubCF = createPage({})(CardSubCF);
export default CardSubCF;
