...
import { tableTypeObj, dataSource, dataValiType, pagecode, pkname, formId, tableId, tableId1, bodypkname, billtype, funcode, nodekey, tableId_edit } from '../constants';
...
export const loadPageValue = function (data, props) {
    if (data) {
        ...
        if (data.bodys) {
            if (data.bodys[tableId]) {
                props.cardTable.setTableData(tableId, data.bodys[tableId]);
            } 
            if(data.bodys[tableId1]){
                props.cardTable.setTableData(tableId1, data.bodys[tableId1]);
            }
        }
    }
    else {
...
export const loadPageValue2 = function (data, props) {
    if (data) {
        ...
        if (data.bodys[tableId]) {
            if (data.bodys) {
                props.cardTable.updateDataByRowId(tableId, data.bodys[tableId],true);
            } 
            if (data.bodys[tableId1]) {
                props.cardTable.updateDataByRowId(tableId1, data.bodys[tableId1],true);
            } 
        }
    }
    else {
...
export const queryCard = function (props) {
    ...
    if (props.getUrlParam('status') != 'add') {
        let data = { pk_bill: props.getUrlParam('id'), pageId: pagecode };
        var url = '/nccloud/uapbd/costcompstruc/querycard.do';
        let cardData;
        ...
        if (cardData) {
            ...
            ajax({
                ...
                success: (res) => {
                    ...
                    toggleShow(that, props);
                        updateCache(pkname, that.props.getUrlParam('id'), res.data, formId, dataSource);
                },
                error: (res) => {
                    that.props.form.EmptyAllFormValue(that.formId);
                    that.props.cardTable.setTableData(that.tableId, { rows: [] });
                    that.props.cardTable.setTableData(that.tableId1, { rows: [] });
...
export const SaveBill = function (props) {
    ...
    if (props.getUrlParam('status') == 'add' ) {
        //新增保存
        url = '/nccloud/uapbd/costcompstruc/insertsave.do';
    } else if (props.getUrlParam('status') == 'edit') {
        //修改保存
        url = '/nccloud/uapbd/costcompstruc/updatesave.do';
    }
    //子表进行判空过滤,核算要素、材料为空，则过滤掉
    that.props.cardTable.filterEmptyRows(tableId, ['costcompid'], 'include');
    that.props.cardTable.filterEmptyRows(tableId1, ['distriorg'], 'include');
    ...
    if (!that.props.cardTable.checkTableRequired(tableId1)) {
        //表格验证
        return;
    }
    let CardData = that.props.createExtCardData(pagecode, formId, [tableId,tableId1]);
    let newCardData = dealCardData(that, CardData,[tableId,tableId1]);//去掉空值，减少压缩时间
    let callback = () => {
        ajax({
            url: url,
            data: newCardData,
            async: false,
            success: (res) => {
                toast({ color: 'success', content: that.state.json['10140CCSC-000007'] });/* 国际化处理： 保存成功*/
                ...
                //重新拉取页面数据作为缓存
                var data = that.props.createMasterChildData(pagecode, formId, tableId);
                if (props.getUrlParam('status') == 'add' )//新增保存
                {
                    addCache(cstuffid, data, formId, dataSource);
                }
                else if (props.getUrlParam('status') == 'edit')//修改保存
                {
                    updateCache(pkname, cstuffid, data, formId, dataSource);
                }
...
export const onPrint = function (that, props) {
    let printData = that.props.createMasterChildData(pagecode, formId, tableId);
...
export const exportcFun = function (that, props) {
    ...
    let exprotData = that.props.createMasterChildData(pagecode, formId, tableId);
...
export const openBillTrack = function (that, props) {
    var exprotData = that.props.createMasterChildData(pagecode, formId, tableId);
