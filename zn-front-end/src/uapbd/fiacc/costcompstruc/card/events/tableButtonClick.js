...
import {formId,pkname,tableId,dataSource,bodypkname} from '../constants';
...
import {resetBodyPk} from './costCompStruc';
    ...
    switch (key) {
        ...
        case bodyButton.DelLine:            
                ...
                if (delRows == null || delRows.length == 0) {
                        toast({color: 'warning',content: taht.state.json['10140CCSC-000017']/* 国际化处理： 请至少选择一行数据进行删除*/});
                        return false;
                }
                let selectIndexs = [];
        ...
        case bodyButton.CopyLine:
                ...
                if (copyRows == null || copyRows.length == 0) {
                        toast({
                        'color': 'warning',
                        'content': that.state.json['10140CCSC-000008']/* 国际化处理：  请至少选择一行数据进行复制！*/
                        });
                        return false;
                }
                let i = 0;
        ...
        case innerButton.Paste_inner:
                let pasteDatas = getDefData("CopyLine", dataSource);
