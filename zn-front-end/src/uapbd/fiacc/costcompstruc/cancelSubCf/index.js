...
class CancelSubCf extends Component {
    constructor(props) {
...
    }
    ...
    cancelDistribute = (value) => {
        ...
        if (orgsArr.length == 0) {
            toast({ content: `${this.state.json['10140CCSC-000035']}`, color: 'warning' });/* 国际化处理： 请先选中要取消分配的成本组建结构！*/
            return;
        }
		...
		beSureBtnClick: function () {
			ajax({
				url: '/nccloud/uapbd/costcompstruc/canceldistributeOrgs.do',
				data: {
                    pk_bills:orgsArr,
                    pk_orgs:targetOrgs
                },
...
CancelSubCf = createPage({})(CancelSubCf);
export default CancelSubCf;
