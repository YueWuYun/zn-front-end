...
import { searchId, tableId, pkname, billtype, funcode, nodekey } from '../constants';
/**
 * 消耗单列表公共方法提取
 */
...
export const refreshFun = function (that, props) {
	...
	let queryInfo = getDefData(searchId, props.dataSource);//获取缓存查询条件
	if (queryInfo) {//缓存中存在查询条件
		...
		ajax({
			...
			success: function (res) {
				...
				//页面新数据放入缓存中
				setDefData(tableId, that.props.dataSource, data);
				toast({ title: that.state.json['10140CECA-000013'], color: 'success' });/* 国际化处理： 刷新成功*/
			}
		});
	} else {
		toast({ color: 'success', title: that.state.json['10140CECA-000013'] }); /* 国际化处理： 刷新成功*/
	}
	return;
}
/**
...
export const batchDelRow = function (that, props) {
	...
	if (delArr.length == 0) {
		toast({ content: `${that.state.json['10140CECA-000011']}`, color: 'warning' });/* 国际化处理： 请先选中需要删除的数据*/
		return;
	}
	promptBox({
		color: 'warning',
		title: that.state.json['10140CECA-000004'],
		content: that.state.json['10140CECA-000012'],/* 国际化处理： 确定要删除所选数据吗?*/
...
export const onPrint = function (that, props) {
	...
	if (index1.length == 0) {
		toast({ content: `${that.state.json['10140CECA-000023']}，${that.state.json['10140CECA-000024']}！`, color: 'warning' });/* 国际化处理： 未选中任何行,不允许操作打印*/
		return;
	}
	print(
...
export const onOutput = function (that, props) {
	...
	if (index1.length == 0) {
		toast({ content: `${that.state.json['10140CECA-000023']}，${that.state.json['10140CECA-000025']}！`, color: 'warning' });/* 国际化处理： 未选中任何行,不允许操作打印输出*/
		return;
...
export const exportlFun = function (that, props) {
	...
	if (ExportcheckedData.length == 0) {
		toast({ color: 'warning', content: that.state.json['10140CECA-000029'] });/* 国际化处理： 请选中至少一行数据!*/
		return;
	}
	let pk_bills = [];
...
export const openBillTrack = function(that,props){
	...
	rows.forEach((ele, index) => {
		...
		if(index1.length == 0){
			toast({ content: `${that.state.json['10140CECA-000029']}！`, color: 'warning' });/* 国际化处理：  请选中至少一行数据!*/
			return; 
		}
		if(index1.length>1){
			toast({ content: `${that.state.json['10140CECA-000030']}！`, color: 'warning' });/* 国际化处理： 联查不能多选*/
			return; 
	}
	that.setState({
        trackshow: true,
