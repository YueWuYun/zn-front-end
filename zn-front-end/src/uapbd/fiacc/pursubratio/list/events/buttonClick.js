    ...
    switch (id) {
					...
					success: (res) => {
						...
						if (success) {
							if (data) {
                                ...
                                toast({ color: 'warning', content: this.state.json['10140CCSD-000004'] }); /* 国际化处理： 未查询到数据*/
		...
		case 'Delete'://批量删除
			promptBox({
				...
				content: this.state.json['10140CCSD-000014'],             /* 国际化处理： ​​确定要删除所选数据吗？*/
				noFooter: false,                 
				noCancelBtn: false,              
				...
				beSureBtnClick: ()=>{
					...
					if (deleteData.length == 0) {
						toast({ color: 'warning', content: this.state.json['10140CCSD-000015'] });/* 国际化处理： 请选中至少一行数据!*/
						return;
					}
					ajax({
						url: '/nccloud/uapbd/pursubratio/batchdelete.do',
						data: deleteData,
						success: (res) => {
							let { success, data } = res;
							if (success) {
...
let getAllCheckedData = function (props, tableId) {
    ...
    checkedData.forEach((val) => {
        checkedObj.push({
            pk_bill: val.data.values.pk_pursubratio.value,
            ts: val.data.values.ts.value,
            // billcode: val.data.values.vbillcode.value,
            index: val.index,
        }
        );
    });
