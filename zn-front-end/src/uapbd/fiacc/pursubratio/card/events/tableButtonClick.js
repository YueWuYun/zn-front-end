    ...
    switch (key) {
        ...
        case 'DelLine'://删行      
            ...
            if (selectRows == null || selectRows.length == 0) {
                toast({color: 'warning',content: this.state.json['10140CCSD-0000017']/* 国际化处理： 请至少选择一行数据进行删除*/});
                return false;
            }
            let selectIndexs = [];
        ...
        case 'CopyLine'://复制行
            ...
            if (copyData.length == 0) {
                toast({ color: 'warning', content:this.state.json['10140CCSD-0000016'] });/* 国际化处理： 请至少选择一行数据进行复制！*/
                return false;
            }
            let arr = [];
