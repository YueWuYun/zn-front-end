...
import {dealCardData} from '../../../../mapub/public/components/pubUtils/dealCardData';
const { NCDiv } = base;
...
        }
	...
	initShow = () =>{
		...
		if(status == 'browse' || status =='edit'){
			...
			ajax({
				url: '/nccloud/uapbd/pursubratio/querycard.do',
				data: {
					pk_bill: pk_bill,
					pageId : this.pageId
				},
				success: (res) => {
					...
					if (this.props.getUrlParam('status') == 'edit') {
						...
						// this.props.form.setFormItemsValue({ pk_costcomponent : res.data.body[this.tableId].rows[0].values.pk_costcomponent.value });
						// console.log("===pk_costcomponent===:"+res.data.body[this.tableId].rows[0].values.pk_costcomponent.value);
						this.props.form.setFormItemsDisabled(this.formId, { pk_org: true });
						// this.props.form.setFormItemsDisabled(this.formId, { pk_costcomponent: true });
	...
	copy =() =>{
		let pageId = this.pageId;
  		let pk_bill = this.props.getUrlParam('id');
		ajax({
			url: '/nccloud/uapbd/pursubratio/copy.do',
			data: {
				pk_bill: pk_bill,
				pageId : pageId
			},
		...
		ajax({
			...
			success: (res) => {
				if (res.data) {
					//根据不同的按钮，进行不同的刷新提示
					if(btnName == 'Refresh'){
						toast({ color: 'success', title: this.state.json['10140CCSD-000009']});/* 国际化处理： 刷新成功*/
					}
	...
	controlBillno =()=>{
		...
		if(pk_org && pk_org.value){
			ajax({
				url: '/nccloud/uapbd/pursubratio/controlbillno.do',
				async: false,
				data: {
					pk_org: pk_org.value
				},
	...
	saveBill = () =>{
			...
			this.props.cardTable.filterEmptyRows(tableId, ['celementid','cmaterialid'], 'include');
			...
			let callback = () =>{
				ajax({
					url: '/nccloud/uapbd/pursubratio/save.do',
					data: datas,
					success: (res) => {
						if(res.data){
							toast({color: 'success',	content: this.state.json['10140CCSD-000011']});//保存成功
							let newId = res.data.head[this.formId].rows[0].values.pk_pursubratio.value;
							//渲染数据和页面状态
							this.props.beforeUpdatePage(); //打开开关
							this.props.form.setAllFormValue({ [this.formId]: res.data.head[this.formId] });
					...
					<NCDiv areaCode={NCDiv.config.HEADER}>
						<div className="nc-bill-header-area">
							<div className="header-title-search-area">
								{createBillHeadInfo({
									title: this.state.json['10140CCSD-000000'],//国际化处理： 期初在产共用材料
									backBtnClick: () => {
										this.backList();
									}
								})
