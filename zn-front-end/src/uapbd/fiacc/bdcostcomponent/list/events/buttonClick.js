	...
	switch (id) {
		case 'Add'://新增
			if(!this.state.pk_org.refpk){
				toast({ color: 'warning', content: this.state.json['10140CECF-000015'] });/* 国际化处理： 请先选择业务单元!*/
				return;
			}
		...
		case 'Delete'://删除
			...
			if(status == 'edit'){
				...
				if(checkData.length == 0){
					toast({ color: 'warning', content: this.state.json['10140CECF-000002'] });/* 国际化处理： 请至少选择一条要删除的数据!*/
				}
				let delindex = [];
				...
				promptBox({
					color: 'warning',                 
					title: this.state.json['10140CECF-000009'],                  /* 国际化处理： 删除*/
					content: this.state.json['10140CECF-000011'],             /* 国际化处理： ​​确定要删除所选数据吗？*/
					noFooter: false,                 
					noCancelBtn: false,              
					beSureBtnName: this.state.json['10140CECF-000007'],           /* 国际化处理： 确定*/
					cancelBtnName: this.state.json['10140CECF-000008'] ,          /* 国际化处理： 取消*/
					beSureBtnClick: ()=>{
						//浏览态直接去调用保存的逻辑
						this.save();
					}
		...
		case 'Cancel'://取消
			promptBox({
				color: 'warning',                 
				title: this.state.json['10140CECF-000007'],                  /* 国际化处理： 取消*/
				content: this.state.json['10140CECF-000010'],             /* 国际化处理： ​确定要取消吗？*/
				noFooter: false,                 
				noCancelBtn: false,              
				beSureBtnName: this.state.json['10140CECF-000007'],           /* 国际化处理： 确定*/
				cancelBtnName: this.state.json['10140CECF-000008'] ,          /* 国际化处理： 取消*/
