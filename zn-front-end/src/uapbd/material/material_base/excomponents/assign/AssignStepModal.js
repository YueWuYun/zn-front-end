        ...
        if(this.state.step.current === 3){
            ...
            ajax({
                ...
                success : (res) => {
                    ...
                    if(success){
                        if(res&&res.data&&res.data.detailMessage){
                            toast({content:res.data.detailMessage,color:'warning'})
                        }else{
                ...
                <NCModal.Footer>
                    <div style={{float:"left",display:'flex',alignItems:'center',visibility:this.state.step.current == 2 ? '': 'hidden'}}>
                        ...
                        <div style={{marginRight:'40px',width:'100px'}}>
                            <NCSelect fieldid = 'selects' onChange={(value)=>{this.setState({assignStatus:value},()=>{this.AssignFilter.setAssignStatus(value);})}} >
                    ...
                    <span style={{display: this.state.buttonShow.pre ? '' : 'none' }}><NCButton fieldid={'preBtn'} onClick={ onStep(-1) }>{this.props.MutiInit.getIntl("10140MATERIAL") && this.props.MutiInit.getIntl("10140MATERIAL").get('10140MATERIAL-0000117')/* 国际化处理： 上一步*/}</NCButton></span>
                    <span style={{display: this.state.buttonShow.next  ? '' : 'none' }}><NCButton  fieldid={'NexBtn'} onClick={ onStep(1)}>{this.props.MutiInit.getIntl("10140MATERIAL") && this.props.MutiInit.getIntl("10140MATERIAL").get('10140MATERIAL-0000118')/* 国际化处理： 下一步*/}</NCButton></span>
