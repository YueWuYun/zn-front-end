...
import { createPage, base, ajax ,NCCreateSearch,toast,high,print,promptBox,getMultiLang,createPageIcon,excelImportconfig } from 'nc-lightapp-front';
...
/****************默认参数  开始***********************/
let formId = 'marsaleclass';//卡片组件Id
let urls={
    loadTreeDataUrl:"/nccloud/uapbd/mscl/MsclLoadTreeData.do",
    queryCardUrl:"/nccloud/uapbd/mscl/MsclLoadFormData.do",
    enablestateUrl:"/nccloud/uapbd/mscl/MsclEDnableAction.do",
    addCardUrl:"/nccloud/uapbd/mscl/MsclAddAction.do",
    deleteUrl:'/nccloud/uapbd/mscl/MsclDeleteAction.do',

    saveUrl:'/nccloud/uapbd/mscl/MsclSaveAction.do',
    validateUrl:'/nccloud/uapbd/mscl/MsclManageModeAction.do',
    editUrl:'/nccloud/uapbd/mscl/MsclEditAction.do',
    cancelUrl:'/nccloud/uapbd/mscl/MsclCancelAction.do',
    printUrl:'/nccloud/uapbd/mscl/MarSaleClassPrint.do',
};
let pageCode="10140MSCLG";//默认集团
/***************默认参数  结束********************/
...
class Mscl extends Component {
    ...
    constructor(props){
        ...
        this.config =Object.assign({
            ...
            pageCode:"10140MSCLG",
            nodeType:'GROUP_NODE',
            refresh:'uapbd/material/mscl_grp/main/index.html',
            primaryKey:'pk_marsaleclass',
            urls:urls,
            actions:{
        ...
        this.initTemplate(this.props,
            ()=>{
                this.config.title=(this.config.nodeType ==='GROUP_NODE'?this.state.json['10140MSCLG-000000']:this.state.json['10140MSCLG-000001']);// 国际化处理： 物料成本分类-集团,物料成本分类-业务单元
 ...
 */
 initTemplate = (props,callback)=>{
    ...
    createUIDom(props)(
        ...
        {
            moduleId: "10140MSCLG",domainName: 'uapbd'
        },  
        (data,langData,inlt)=>{
            if(langData){
...
/*     dealTreeData(data){
    ...
    dealTreeData(data){
        ...
        componentWillMount() {
            ...
            let callback = (json) => {
                this.setState({json})
                this.config.title=(this.config.nodeType ==='GROUP_NODE'?"物料销售分类-集团":"物料销售分类-销售组织");
                this.root.title = this.config.title;
                this.root.refname = this.config.title;
            }
            getMultiLang({moduleId: "10140MSCLG",domainName: 'uapbd',callback})
            */
        }
...
/*
    ...
    onAddClass(selectNode){
        ...
        if(this.config.nodeType=="ORG_NODE" && !this.state.curOrg.refpk){
            Message.create({content: this.state.json['10140MSCLG-000002'], color: 'warning'});/* 国际化处理： 销售组织为空，不能增加！*/
            return;
        }  		
        /******************************************************
         * 设置当前选中节点
    ...
    onEditClass(selectedTreeNode){
        ...
        if (!this.state.curSelectedNode) {
            Message.create({content: this.state.json['10140MSCLG-000003'], color: 'warning'});//默认top/* 国际化处理： 请选中需要编辑的节点*/
            return;
        }
        /**************************************************
         * 请求参数对象
        ...
        ajax({
            ...
            success: (result) => {
                if(result.success){
					...
					toast({title : this.state.json['10140MSCLG-000004'],color : 'success'});/* 国际化处理： 保存成功！*/
                    /**************************************************************
                     * 清空选中节点的缓存
                     **************************************************************/
    ...
    onDeleteClass(selectedTreeNode){
         ...
         **************************************************************/
        if (!selectedTreeNode) {
            Message.create({content: this.state.json['10140MSCLG-000005'], color: 'warning'});/* 国际化处理： 请选中需要删除的节点*/
            return;
        }
        /**************************************************************
         * 根节点不能删除
        ...
        if(selectedTreeNode.refpk == this.root.refpk){
            Message.create({content: this.state.json['10140MSCLG-000006'], color: 'warning'});/* 国际化处理： 根节点不能删除*/
            return;
        }
        /**************************************************************
         * 设置当前选中节点
        ...
        ajax({
            ...
            success:(result)=>{
                ...
                if(result.success && result.data){
                    promptBox({
                        color:'warning',
                        title: this.state.json['10140MSCLG-000007'],/* 国际化处理： 确认删除*/
                        content: this.state.json['10140MSCLG-000008'],/* 国际化处理： 您确定要删除所选数据吗?*/
                        beSureBtnClick: ()=>{
                            promptBox({
                                color:'warning',
                                title: this.state.json['10140MSCLG-000007'],/* 国际化处理： 确认删除*/
                                content: this.state.json['10140MSCLG-000009'],/* 国际化处理： 删除时要做业务引用校验，可能等待时间较长，是否确认删除?*/
                                beSureBtnClick: () => {
                                    ajax({
                                        ...
                                        success:(result)=>{
                                            if(result.success){
                                                 ...
                                                 * 删除成功提示
                                                 **************************************************************/
                                                toast({title:this.state.json['10140MSCLG-000010'],color : 'success'});/* 国际化处理： 删除成功！*/
                                                /**************************************************************
                                                 * 重新设置按钮状态
                                                 **************************************************************/
    ...
    onCancelClass(){
        promptBox({
            color:'warning',
            title: this.state.json['10140MSCLG-000011'],/* 国际化处理： 确认取消*/
            content: this.state.json['10140MSCLG-000012'],/* 国际化处理： 是否确认要取消?*/
            beSureBtnClick:()=>{
        /**********************************************************
         * 获得选中的树节点
         **********************************************************/
    ...
    onStartClass(){
        ...
        if(!selectedTreeNode){
            ...
            Message.create({content: this.state.json['10140MSCLG-000013'], color: 'warning'});//默认top/* 国际化处理： 请选中需要启用的树节点*/
        }else if(formData.rows[0].values.enablestate.value == '2'){
            Message.create({content: this.state.json['10140MSCLG-000014'], color: 'warning'});//默认top/* 国际化处理： 该数据已启用，无需多次启用*/
            return;
        }
        requestParam = {
    ...
    onStopClass(){
        ...
        if(!selectedTreeNode){
            ...
            Message.create({content: this.state.json['10140MSCLG-000015'], color: 'warning'});//默认top/* 国际化处理： 请选中需要停用的树节点*/
        }else if(formData.rows[0].values.enablestate.value == '1'){
            Message.create({content: this.state.json['10140MSCLG-000016'], color: 'warning'});//默认top/* 国际化处理： 该数据已停用，无需多次停用*/
            return;
        }
        requestParam = {
            primaryKey:formData.rows[0].values[this.config.primaryKey].value,
    ...
    onAfterFormEvent(props, moduleId, key, value, index){
        switch(key){
            case "enablestate":
                ...
                if(!selectedTreeNode){
                    let content = value.value?this.state.json['10140MSCLG-000013']:this.state.json['10140MSCLG-000015'];/* 国际化处理： 请选中需要启用的树节点,请选中需要停用的树节点*/
                ...
                */                
                ...
                promptBox({
                    color:"warning",
                    title:this.state.json['10140MSCLG-000017'],/* 国际化处理： 提示*/
                    content:value.value ?this.state.json['10140MSCLG-000018']:this.state.json['10140MSCLG-000019'],/* 国际化处理： 确认启用该数据？,您确定要停用所选数据及其所有下级数据吗？*/
                    beSureBtnClick:()=>{
                                ...
                                if(res.success && res.data){
                                    ajax({
                                        ...
                                        success:(result)=>{
                                            ...
                                            toast({title:value.value  ?this.state.json['10140MSCLG-000020']:this.state.json['10140MSCLG-000021'],color : 'success'});/* 国际化处理： 启用成功！,停用成功！*/
                                        }
                                    });
                                }
                            }
    ...
    onRefreshClass(){
        this.setState({
            ...
            this.props.form.EmptyAllFormValue(this.config.formId);//清空表单数据
            this.changeButtonStatus('','Refresh');
			toast({ color: 'success', title: this.state.json['10140MSCLG-000022'] });/* 国际化处理： 刷新成功！*/
        });
    }
    onBeforeEvent(props,moduleId,key,value,data){
        ...
        if("pk_father" === key){
            let meta = props.meta.getMeta();
            meta.marsaleclass.items.map((ele)=>{
        ...
        formStatus = formStatus == 'add' ? 'edit' : ''
        if (formStatus != 'edit' && !data.pk_marsaleclass.value) {
            return false
        }
		return true;
    }
    ...
    /*
    onPrint(modal){
        let cpk = this.props.form.getFormItemsValue(this.config.formId,'pk_marsaleclass').value;
        if(cpk === null){
            return;
        }
                ...
                <NCDiv  areaCode={NCDiv.config.HEADER} className="header">
                    ...
                    <span className="showOff">
                        <NCCheckbox
                            ...
                            size="lg"
                        >
                            {this.state.json['10140MSCLG-000023']/* 国际化处理： 显示停用*/}
                        </NCCheckbox>
			        </span>
                    {/* 按钮组 btn-group*/}
                    ...
                    <DragWidthCom
                        ...
                        rightDom = {
                            ...
                            {this.config.nodeType != 'ORG_NODE' ? <EmptyAreaTip
                                type="btn"
                                desc={this.state.json['10140MSCLG-000024']}
                                onClick={this.addClickCall}
                                show={cardEmpty} /> : <EmptyAreaTip
                                    desc={this.state.json['10140MSCLG-000025']}
                                    show={cardEmpty} />}
