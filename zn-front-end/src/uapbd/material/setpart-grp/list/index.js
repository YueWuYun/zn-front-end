...
import { createPage,ajax,base,toast,cacheTools,cardCache} from 'nc-lightapp-front';
...
let dataSource = "setpart-grp";

let config = {
	nodeName: '10141486-000033',
    nodetype: 'group',
    pageCode: '10141486_list',
    pageCodeCard: '10141486_card',
		...
		function (data) {console.log('inittemplage');console.log(data);
			if (data) {
				if (data.template) {
                    ...
                    searchVal =props.search.getAllSearchData(config.searchId);
    			...
    			return (
    				<span>
                        ...
                        <span
    						...
    						onClick={() => {
                                let searchVal = props.search.getAllSearchData(config.searchId);
                                cacheTools.set(config.searchId, searchVal);
                                props.pushTo('/card', {
                                    pagecode:props.config.pageCodeCard,
    								status: 'browse',
    ...
    function tableButtonClick(props, id, text, record, index){return;
        ...
        switch(id){
            case 'tableedit':
                ...
                break;
            case 'tabledel':console.log(props);
...
// export default <Setpart_grp {...{config:config}}/>;
