...
var config = {
    title: '10140TAXCB-000009',/* 国际化处理： 税种-组织*/
    nodetype: 'org',
    tableid : 'taxkind',
    appid : '0001Z310000000000O2E',
    pagecode : '10140TAXCO_1014001',
    pkorg : '',
    defaultOrg:''
};
...
const initTemplate = (props, callback)=>{
	...
	(data)=>{
		if(config.nodetype=='org'&&data.context.pk_org&&data.context.org_Name){
...
config.initTemplate = initTemplate;



var Taxkind_Org = createPage({
    billinfo:{
        billtype: 'grid',
        pagecode: config.pagecode,
...
function modifierMeta(props,meta) {
	...
	let event = {
		...
		render(text, record, index) {
			...
			if ((isSys ==='1' || isGlobe === 'GLOBLE00000000000000')){
				btnArray=[];
			}
...
ReactDOM.render(<Taxkind_Org {...{config:config}}/>, document.querySelector('#app'));
