...
class AccChartContent extends Component{
    constructor(props) {
        super(props);
        this.state = {
            treeData: [],
    ...
    begTreeData() {//ajax请求树信息
        ...
        ajax({
            loading: true,
            url: "/nccloud/uapbd/rbac/AccChartDataPermAction.do",
            data: {...paramObj},
            // pk_group
            success: function (res) {
                ...
                if (res.success) {
                    if(!res.data){
                        ...
                        that.setState({treeData: res.data});
                    }
        ...
        const loop = (data, keys, callback) => {
            data && data.map( (item, index, arr) => {
                if(keys.includes(item.id)) {
                    const keyIndex = Utils.indexof(keys, item.id);
    ...
    render() {
        const loop = data => data.map( (item) => {
            if (item.children && item.children.length) {
                return <NCTreeNode key={item.id} title={item.pid=='root'?item.name:item.code+' '+item.name}>{loop(item.children)}</NCTreeNode>;
            }
            return <NCTreeNode key={item.id} title={item.pid=='root'?item.name:item.code+' '+item.name} />;
        })
...
AccChartContent = createPage({
    
})(AccChartContent);

module.exports = AccChartContent;
