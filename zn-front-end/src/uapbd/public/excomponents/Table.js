...
class Table extends Component {
    ...
    constructor(props) {
                    ...
                    var dataIndex = col.dataIndex || function(){},
                        render = (text, record, index) => {
                            var value = BaseUtils.isFunction(dataIndex) ? dataIndex(record, index) : record.getData()[dataIndex],
                                ...
                                if(  !(mode==='edit' && editer )  ){
                                    return (<NCTooltip {...tooltipcfg} overlay={text}>
                                        <span fieldid={col.dataIndex}>{col.render ? col.render(value, record, index) : value}</span>
                                    </NCTooltip>
                                    );
                                }
                                return <span fieldid={col.dataIndex}>{editer(record, index, this)}</span>;  
                        };
        ...
        this.tableConfig = {
            ...
            useFixedHeader:true,
            bodyStyle:{height: this.state.tableHight + 'px' },
            scroll: this.config.scroll || { x: true, y: this.state.tableHight },
            adaptionHeight: this.props.adaptionHeight || false
        };
    }
    ...
    calcHeight = () => {
        let tableBody = this.myComponent.querySelectorAll('.u-table-body')[0];
        const paddingBottom = Number(this.getStyle(document.body, 'paddingBottom').replace('px', ''));
        let headerHeight = this.myComponent.querySelectorAll('.u-table-fixed')[0].offsetHeight;
        headerHeight = headerHeight > 300 ? 36 : headerHeight;
        if (tableBody) {
            console.log(document.body.offsetHeight, this.myComponent.getBoundingClientRect(), tableBody.offsetHeight)
            let tableHight = document.body.offsetHeight -this.myComponent.getBoundingClientRect().bottom + tableBody.offsetHeight - paddingBottom;
            tableBody = null;
            this.setState({tableHight:tableHight});
        }else {
            const  tableHight = document.body.offsetHeight -this.myComponent.getBoundingClientRect().bottom - paddingBottom;
            tableBody = null;
            this.setState({tableHight:tableHight});
        }
    }
            ...
            <NCDiv ref={component => this.myComponent = findDOMNode(component)} fieldid={this.tableConfig.fieldid || "common"} areaCode={NCDiv.config.TableCom}>
               <NCTable
                   ...
                   data={this.state.mode === 'browse' ? this.state.records : this.state.dirtyRecords.filter(e => e.getStatus() !== 'del') }
                    {...this.tableConfig}
                   bodyStyle={{height: this.state.tableHight + 'px' }}
                scroll={ {x: true, y: this.state.tableHight }}
