...
function createTree({
	...
	otherConfig={}
		...
		const loop = data => data.map((item) => {
			...
			if (item.children) {
				return (
					<NCTreeNode key={item.key} title={searchValue=="" ? iconTitle : title} >
						{loop(item.children)}
					</NCTreeNode>
				);
			}

			return <NCTreeNode key={item.key}  title={searchValue=="" ? iconTitle : title} disabled = { fixed == treeId ? isDisable(item): false}/>;
		});
