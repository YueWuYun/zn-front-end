...
class ResultGrid extends Component {
    ...
    handlePageCheck =()=>{
        ...
        this.props.pageprops.table.getCheckedRows(this.state.gridId).map((e)=>{
            this.state.selectedPKs.push(e.data.values.pk_customer.value);
        })
        this.setState(this.state);
    }
            ...
            return (
               <div>
                   <div style={{display: this.props.showHeadButtons === false ? 'none' : ''}}>
                       <NCButton fieldid = 'chooseall' style={{backgroundColor:'#e4e4e4'}} shape="border" colors="accent" onClick={ this.handleAllCheck.bind(this) }>{this.props.json['resultgrid-000000']}</NCButton>{/* 国际化处理： 全部选择*/}
                       <NCButton fieldid = 'cancelall' style={{backgroundColor:'#e4e4e4'}} shape="border" colors="accent" onClick={ this.handleAllCancel.bind(this) }>{this.props.json['resultgrid-000001']}</NCButton>{/* 国际化处理： 全部取消*/}
                       <NCButton fieldid = 'pagechoose' style={{backgroundColor:'#e4e4e4'}} shape="border" colors="accent" onClick={ this.handlePageCheck.bind(this) }>{this.props.json['resultgrid-000002']}</NCButton>{/* 国际化处理： 本页选择*/}
                       <NCButton fieldid = 'pagecancel' style={{backgroundColor:'#e4e4e4'}} shape="border" colors="accent" onClick={ this.handlePageCancel.bind(this) }>{this.props.json['resultgrid-000003']}</NCButton>{/* 国际化处理： 本页取消*/}
                   </div>
