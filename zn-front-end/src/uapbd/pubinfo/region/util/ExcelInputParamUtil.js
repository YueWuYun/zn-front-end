//FzIXzf1TQ28b1Z4cWNt4XGzxgjq+nk+MGbdytvDZmN+6MTkxxayAZMRL7KOqFe/0d4DFyvmQdlCK
//S38GTknI6A==

import UpDownProgress from '../upDownProgress';
...
const ProcessArr = (props) => {
    const {upParams, sysTs, regionProps,importOtherParams} = props;
    return (
        <div key={sysTs}>
	 		<UpDownProgress initProNum={1} listUrl=""  url="/nccloud/uapdr/trade/excelimportproduce.do" 
	 		paramObj={{...upParams, importSign: sysTs.toString()}} regionProps={regionProps} importOtherParams={importOtherParams}/>
	 	</div>
    )
    ...
    return getImportConfig(moduleName,billType, toastArr.length, importUrl, importOtherParams, (resultinfo, isSuccess) => {
        ...
        if(resultinfo.upStaus == 'beginUpload') {
            ...
            props.modal.show('importModal', {
                content: <ProcessArr sysTs={resultinfo.sysTs} upParams={{"moduleName":moduleName,"billType":billType}} regionProps={props} importOtherParams={importOtherParams}/>
            });
		}else {
            let ErrorList;
            if((Object.prototype.toString.call(resultinfo) ==='[object Array]' && resultinfo.length) || Object.prototype.toString.call(resultinfo) ==='[object Object]'){
                ErrorList = getErrorList(resultinfo);
                if(isSuccess){
                    // toast({
                    //     duration: 500,
