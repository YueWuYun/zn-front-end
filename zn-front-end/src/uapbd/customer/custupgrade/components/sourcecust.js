...
const pageId = '1317CUME_base';//页面id
const appid = '0001Z010000000000SJE';//注册按钮的id
...
class Sourcecust extends Component {
    ...
    //查询数据
    getData =() =>{
        if(this.config.pk_source != null){
            let data = { pk: this.config.pk_source, pageid: this.pageId };
            let that = this;
            ajax({
                url: '/nccloud/uapbd/custupgrade/querysourcecustomer.do',
                data: data,
                success: (res) => {
