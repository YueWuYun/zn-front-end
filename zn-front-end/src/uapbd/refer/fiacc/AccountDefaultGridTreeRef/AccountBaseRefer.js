//2Oyzk7HRBoz2TmvbH0WDT3oltQ/Rphv+uexyZh1OUY7WDHptD4C+ieUkcPqKFlYZ
...
import AccAssItemTable from '../../../public/excomponents/Table';
import Utils,{BaseUtils} from "../../../public/utils/index";
import { base, high ,ajax } from 'nc-lightapp-front';
import PropTypes from 'prop-types';
const {NCCheckbox : Checkbox,NCPopover : Popover,NCLoading : Loading,NCCollapse,NCButton,NCTable,NCMenu : Menu, NCTooltip} =base;
...
class AccountBaseRefer extends PopRefer {
    ...
    // 复写原型方法：点击参照三个点的事件
    show = () => {
        let {disabled,isTreelazyLoad, idKey ,refType, rootNode,isAccountRefer, pageSize,searchWithKeywordOutsideWhenOpen } = this.props;
        if(disabled){return false;}
        let trueValue = this.getTrueValue(),
		    hasValue = Array.isArray(trueValue) ? (trueValue[0]&&trueValue[0].refpk?trueValue.length:0) : trueValue && trueValue.refpk,
		    needSearch = searchWithKeywordOutsideWhenOpen && !hasValue;
        ...
        this.setState({isShow: true,isFirstShow: false,dropDownShow: false,includeChildren:false,isTreeSearch:false,formData:{}},()=>{
            ...
            let param = this.__getParam({pid: isTreelazyLoad ? rootNode[idKey] : '',keyword:needSearch?this.state.referVal:'',pageInfo: {pageSize,pageIndex: -1}});
            ...
            this.loadAndSetTreeData(param, rootNode, this.state.accexpandedKeys,()=>{
                ...
                this.setState({
                    treeSearchVal:needSearch?this.state.referVal:null,//null
                    ...
                    if(needSearch&&this.state.referVal){
                        this.TreeComp&&this.TreeComp.onTreeSearchChange(this.state.referVal);
                    }
                });
            });
        });
    ...
    loadTableData = async (param) => {
        ...
        return await new Promise((resolve) => {
            this.setState({loading : true},() => {
                    ...
                    ajax({url : queryGridUrl,data : param,loading : false,
                        success: (res) => {
    ...
    loadTreeData = async (param) => {
        return await new Promise((resolve) => {
            this.setState({loading: true},() => {
                    ...
                    ajax({url : queryTreeUrl,data : param,loading : false,
                        success: (res) => {
                            if(res.success&&res.data&&res.data.rows){
                                this.setState({loading: false},()=>{
                                    ...
                                    var cloopNode = (nodes) => {
                                        nodes.forEach( node => {
                                            node.nodeData.isleaf = node.nodeData.endflag === 'Y' ? true : false;
                                            node.isleaf = node.nodeData.isleaf;
                                            if(node.children  && node.children.length == 0 ){
                                            return;
                                            }
                                            cloopNode(node.children || []);
                                        });
                                    };
                                    cloopNode(res.data.rows.treeData);
    ...
    renderPopoverRight = () => {
        return (
            <div style={{height:"100%"}} >
                <div className="nc-theme-area-split-bc" style={{width:'100%','border-bottom':'1px solid #DEDEDE'}}>
                    {this.renderPopoverSearchArea()}
                </div>
                <ReferForm formData={this.state.formData} multiLang={this.props.multiLang} fieldid={this.props.fieldid || ''}/>
                <div className='ncc-hr-table nc-theme-area-split-bc' style={{borderTop:'1px solid #DEDEDE'}}>
                <AccAssItemTable
...
// 复写原型方法：渲染弹出层搜索区域
    renderPopoverSearchArea = () => {
        ...
        return [
            ...
            // 设为常用
            isShowUsual && (
                <Button fieldid='usual' className="set-to-usual" onClick={this.usualAction.bind(this, 'save')}>
    ...
    getMoreButtonDOM = () => {
        ...
        const menu = (
            <Menu 
                ...
                vertical
                keyboard
                onClick={({ item, key, keyPath }) => {this.state.hasData && this.setState({isMoreButtonShow: false},()=>{
                    this.moreButton && this.moreButton.focus();
                    switch (key) {
                        case 'expandAll':
                        case 'unExpandAll':
                            this.setState({isMoreButtonShow:false, defaultExpandAll: key=='expandAll', autoExpandParent: key=='expandAll', accexpandedKeys: key=='expandAll'?this.getAllTreeId():['root']});
                            break;
                        default:
                ...
                <Item key="expandAll" fieldid='expandAll'>{this.props.multiLang['refer-001033']/* 国际化处理： 全部展开*/}</Item>
                <Item key="unExpandAll" fieldid='unExpandAll'>{this.props.multiLang['refer-001034']/* 国际化处理： 全部收起*/}</Item>
                <SubMenu key="3" fieldid='levelExpansion' title={this.props.multiLang['refer-001014']/* 国际化处理： 级次展开*/}>{this.getMenuItem()}</SubMenu>
                {isMultiSelectedEnabled && (<SubMenu key="4" fieldid='levelSelection' title={this.props.multiLang['refer-001015']/* 国际化处理： 级次选择*/}>{this.getMenuItem(true)}</SubMenu>)}
            </Menu>
        );
        return (
            <Popover
    ...
    getMenuItem = (flag)=>{
        ...
        for(let i = 1 ;i<=maxlayNumber;i++){
            res.push(<Item key={`select${i}`} fieldid={`select${i}`}>{flag?multiLang['refer-001036']:multiLang['refer-001038']}{i}{multiLang['refer-001037']/* 国际化处理： 选择,级*/}</Item>)
        }
        ...
        return (
            ...
            <span className="ncc-hr-refer-searcharea" style={{width:'50px'}}>
                <span
                    ...
                    className="ncc-hr-font-size nc-theme-common-font-c"
                    style={{'margin-left':'10px',fontFamily:'uf'}}
    ...
    renderPopoverBottom = () => {
        ...
        selectColumns.push({
			// 操作
			title: (<div fieldid='opr'>{this.state.jsonInsideRefer['containers-refer-0025']}</div>),
			key: 'operate',
			width: 100,
        ...
        return [
            isMultiSelectedEnabled && (
                <div className="refer-selected" key="1">
                    <NCButton fieldid='selected' onClick={() => {this.setState({selectedShow: !selectedShow});}}>
            ...
            <div className="buttons" key="3">
                <NCTooltip trigger="hover" placement="top" inverse={true} className='model-helper-overlay' overlay={(<div>{`${this.props.multiLang['refer-001010']}(Alt+Y)`}</div>)}>
                    <NCButton colors="primary" disabled={!this.state.hasData}
                        onClick={this.onReferSureClick} fieldid='sure'>
                        {this.props.multiLang['refer-001010']/* 国际化处理： 确定*/}(<span class='text-decoration-underline'>Y</span>)
                    </NCButton>
                </NCTooltip>
                <NCTooltip trigger="hover" placement="top" inverse={true} className='model-helper-overlay' overlay={(<div>{`${this.props.multiLang['refer-001011']}(Alt+N)`}</div>)}>
                    <NCButton style={{marginLeft: '9px'}}
                        onClick={this.cancel} fieldid='cancel'>
                    {this.props.multiLang['refer-001011']/* 国际化处理： 取消*/}(<span class='text-decoration-underline'>N</span>)
                    </NCButton>
                </NCTooltip>
            </div>
        ];
    ...
    onTreeNodeDblClick(checkedKeys,e){
        ...
        if(!isMultiSelectedEnabled){
            ...
            !this.state.checkedKeys.includes(eventKey) && this.state.checkedKeys.push(eventKey);
            
        }

        // let selectedKeys = [...selectedValues.keys()];
        // !selectedKeys.includes(eventKey) && selectedKeys.push(eventKey);

        let result = this.getResultData(this.state.treeData,isMultiSelectedEnabled?this.state.checkedKeys:selectedKeys);
        // if(isMultiSelectedEnabled){
    ...
    onSelectTreeNodeLoadTreeData = (selectedKeys,callback,obj,checkedKeys)=>{
        ...
        if(!this.checkIsRootOrFirstSub(selectNode.node)){//不是根和第一级子节点
            ...
            // 选中树节点与checkedKeys没关系，checkedKeys不在这里处理
            if(this.props.isMultiSelectedEnabled){//多选->多选模式
                this.state.checkedKeys = this.state.multiMode?[...selectedMap.keys()]:(selectNode.node.disableCheckbox?this.state.checkedKeys:[selectNode.node.refpk]);
            } 
