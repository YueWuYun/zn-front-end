...
class CustSupplierFlexGridTreeRef extends ReferWithUnit {

	constructor(props) {
		super(props);
		this.state = {
			...this.state, // 继承state
			radioValue: this.props.defaultCheck =='pk_custclass'?'pk_custclass' :'pk_supplierclass',
			group_pk: 'firstGroup',
			currentReffer: []
		};
	}
	...
	renderPopoverLeftHeader = () => {
		return (
			<div id="radioGroup" style={{ 'margin-bottom': '5px','margin-left':'10px' }}>
				<NCRadio.NCRadioGroup
					name="role"
					radioValue={this.state.radioValue}
					selectedValue={this.state.radioValue}
					onChange={this.handleRadioChange.bind(this)}>
					<NCRadio className="my-radio" value="pk_custclass"  >{this.props.multiLang['refer-000540']}</NCRadio>
					<NCRadio className="my-radio" value="pk_supplierclass" >{this.props.multiLang['refer-000541']}</NCRadio>
				</NCRadio.NCRadioGroup>
			</div>
		);
	};
...
const CustSupplierWrapper = MultiLangWrapper(CustSupplierFlexGridTreeRef)
export default CustSupplierWrapper;
