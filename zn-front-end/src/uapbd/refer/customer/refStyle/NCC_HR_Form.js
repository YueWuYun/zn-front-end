...
class NCC_HR_Form extends Component{
     ...
     * @param needTransForm @type boolean
     **************************************************/
    setFormData(formId,data,needTransForm){
        
        if(this.state.formData.hasOwnProperty(formId)) {
            this.state.formData[formId] = {};
        }

        this.state.formData[formId] = needTransForm?this.transFormData(formId,this.state.formMeta,data):data;
        this.setState(this.state);
    }

    /**************************************************
     * 转换表单数据
     * 当数据不是走平台的toForm方法时，需要这个方法来转换
     * @param formId
     * @param meta
     * @param data
     **************************************************/
    transFormData(formId,meta,data){
        var dataObj = data;
        ...
        if(!meta[formId]){
            toast({title:"提示",content:"该表单编码获取不到对应的元数据",color:'warning',duration:'10'});
            throw new Error("该表单编码["+formId+"]获取不到对应的元数据");
        }

        let curFormData = new Object();
        meta[formId].items.map((item)=>{
            if(item.attrcode == 'cashtype'){
                let cashTypeWapper = {
                    0:'其它',
                    1:'现金科目',
                    2:'银行科目',
                ...
                /*******************************************
                 * 0=其它，1=现金科目，2=银行科目，3=现金等价物
                 *******************************************/
                curFormData[item.attrcode] = {
                    display:cashTypeWapper[dataObj[item.attrcode]],
                    value:dataObj[item.attrcode]
                }
            }else{
                switch(item.itemtype){
                    case 'checkbox':
                        curFormData[item.attrcode] = {
                            display:!!dataObj[item.attrcode],
                            value:!!dataObj[item.attrcode],
                        }
                        break;
                    ...
                    default:
                        ...
                        break;
                    /*********************************************
                     * 会计科目参照的右卡信息只有input和checkbox两种类型，
                     * 所以只写了两种，如果还有别的，可以在这里添加
                     *********************************************/
    ...
    render(){
        ...
        const renderForm = (flag,code,items)=>{
            if(flag)
                return (<NCRow>{renderFormItems(code,items)}</NCRow>);
            return null;
        }
        /************************************************
         * 创建表单
        ...
        const createForms = (ids)=>(ids.map((id)=>{
            ...
            return relationMetas.map((meta)=>{
                ...
                let usualItems = !!besidesTextAreaItems && besidesTextAreaItems.length>0;
                let hasTextArea = !!textAreaItems && textAreaItems.length>0;
                return (
                    <div className="group-form-wrapper">
                        ...
                        <div id={meta.code} className={`ncc-hr-form-style group-form-item ${groupFormClass}`}>
                            <NCForm showSubmit={false} checkFormNow={false} useRow={true} className="lightapp-component-form"
                                    submitCallBack={() => {}}>
                                {/*浏览态: 只渲染值  编辑态：渲染组件*/}
                                {renderForm(usualItems,areacode,besidesTextAreaItems)}
