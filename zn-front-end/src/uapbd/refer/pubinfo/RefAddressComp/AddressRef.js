//YD9Qt/TFx2z49Wqsp1SSOfP8B+O6IFK52F5l92fd25reSCLg17PiKh9hDsGZooQ6
 ...
 * zhenmx 地址簿 卡片界面
 * add by liupzhc
 * 目前地址簿参照统一使用 pubinfo/AddressRef/index
 * ****************当前地址簿已经废弃****************
 * 1、当前地址簿参照需要有配置
 * 2、当前地址簿参照界面UI不好
 * 3、当前地址簿参照需要有模板
...
class AddressRef extends PopRefer{
    ...
    show = () => {
        ...
        param.queryCondition = Object.assign({
                ...
                pk_address:this.props.foolValue || this.props.foolValue.value
    ...
    onSaveAddress = (callback)=>{
        ...
        //如果什么都不输入点击确定不忘地址表里面插数据，不生成pk_address
...
const AddressRefWrapper = MultiLangWrapper(AddressRef)
export default AddressRefWrapper;
