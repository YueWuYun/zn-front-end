//3NjpynbnhBndWrGd+tWMV1Rdxb6g0eiiuqMk3gnXEuJO5diuG028TN93Tw9zTm9b
...
class ClassTree extends Component{
     ...
     * @param key
     */
    onMouseEnterEveCallBack(key){
        if(BaseUtils.isFunction(this.config.onMouseEnterEve)){
            this.config.onMouseEnterEve(key);
        }
    }
    ...
    *节点选中事件
    */
    onTreeExpandCallBack(expandedKeys,info){
        this.state.selectedNode = this.props.syncTree.getSyncTreeValue(this.config.treeId,info.node.props.refpk);
        this.setState(this.state,()=>{
            if(BaseUtils.isFunction(this.config.onTreeExpand)){
                this.config.onTreeExpand(expandedKeys,info);
            }  
        });       
      
    }

    onDoubleClick(expandedKeys,info){
        this.state.selectedNode = this.props.syncTree.getSyncTreeValue(this.config.treeId,info.node.props.refpk);
        this.setState(this.state,()=>{
            if(BaseUtils.isFunction(this.config.onDoubleClick)){
                this.config.onDoubleClick(expandedKeys,info);
            }  
        });       
        ...
        return(
            <div className="tree-area">
                {createSyncTree({
                    ...
                    disabledSearch:this.config.disabledSearch || false,//这个用来控制搜索框是否可用；
                    onTreeExpand:this.onTreeExpandCallBack.bind(this),//
                    searchType:'filtration',//树节点过滤方式修改
                    onDoubleClickEve:this.onDoubleClick.bind(this),
                })}
            </div>
        )
