...
const urls = {
	...
	print: '/nccloud/uapbd/incomeperiod/printIncomePeriod.do'
};
...
class SingleTable extends Component {
				...
				return tableStatus == 'browse' || tableStatus == undefined ? (
					<div className="table-opr">
						<NCPopconfirm
							trigger="click"
							placement="top"
							content={this.state.json['10140SBAE-000001']/* 国际化处理： 确认删除?*/}
							onClose={() => {
								if(props.editTable.getStatus(tableid) === 'edit'){//编辑状态
									...
									let delObj = {
										...
										values: {
											ts: {
												display: this.state.json['10140SBAE-000002'],/* 国际化处理： 时间戳*/
												value: record.values.ts.value
											}
										}
									};
									delObj.values[primaryKey] = {
										display: this.state.json['10140SBAE-000003'],/* 国际化处理： 主键*/
										value: record.values[primaryKey].value
									}
									let indexArr=[];
									indexArr.push(index);
									let data = {
										pageid: "10140INCPO_list",
										model: {
											areaType: 'table',
											pageinfo: null,
											rows: [ delObj ]
									...
									ajax({
										...
										success: function(res) {
											...
											if (success) {
												...
												toast({content:this.state.json['10140SBAE-000004'],color:'success'});/* 国际化处理： 删除成功*/
											}
										}.bind(this)
									});
								}
						...
						>
							<a className='operator'>{this.state.json['10140SBAE-000026']/* 国际化处理： 删除*/}</a>
						</NCPopconfirm>
					</div>
				):(
					<div className="table-opr">
						...
						>{this.state.json['10140SBAE-000026']/* 国际化处理： 删除*/}</a>
					</div>
				);
			}
		};
			...
			success: (res) => {
				...
				if (success) {
					data[tableid].rows.forEach(function(item, index, array){
						item.values['ispredata'].disabled = 'on';
					});
					...
					if(!searchVal.trim()=='') {
						...
						for(var row of allData.rows){
							if(row.values['code'].value.indexOf(searchVal)>-1 || row.values['name'].value.indexOf(searchVal)>-1){
								rows.push(row);
							}
						}
						allData.rows = rows;
		...
		if(key === 'enablestate'){  // && data.values['doclevel'] === '0'){
			...
			if(!this.canOperate(pk_org,pk_group,isPreData,this.props.nodetype)) {
				if(this.props.nodetype && this.props.nodetype === 'org')
					toast({content: this.state.json['10140SBAE-000005'],color: 'warning'})/* 国际化处理： 组织节点只能维护当前节点有权限组织的数据！*/
				else
					toast({content: this.state.json['10140SBAE-000006'],color: 'warning'})/* 国际化处理： 集团节点只能维护当前登录集团的数据！*/
			...
			this.enableContent = !rowData.values['enablestate'].value ? this.state.json['10140SBAE-000007']: this.state.json['10140SBAE-000008']/* 国际化处理： 您确定要停用所选数据吗？,您确定要启用所选数据吗*/
	...
	updateButtonStatus(){
		...
		if(this.props.nodetype == 'org' && (!this.state.curOrg || (this.state.curOrg && !this.state.curOrg.refpk))) {
			...
			if(length <= 0){//未选择数据
				...
				buttonEnableState.Print = true
				buttonEnableState.Output = true
			}
			else{//选择数据
	...
	addRow(isFocus = true) {
		if(this.props.nodetype == 'org' && this.state.curOrg.refpk == null) {
			toast({content: this.state.json['10140SBAE-000009'],color: 'warning'})/* 国际化处理： 请先选择业务单元！*/
			return
		}
	...
	onButtonClick(props,id) {
		switch (id) {
			...
			case 'Cancel':
				...
				promptBox({
					color: 'warning',               // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
					title: this.state.json['10140SBAE-000010'],                // 提示标题, 默认不同类别下分别为："已成功"/"帮助信息"/"请注意"/"出错啦",非必输/* 国际化处理： 确认取消*/
					content: this.state.json['10140SBAE-000011'],             // 提示内容,非必输/* 国际化处理： 是否确认要取消？*/
					noFooter: false,                // 是否显示底部按钮(确定、取消),默认显示(false),非必输
					noCancelBtn: false,             // 是否显示取消按钮,，默认显示(false),非必输
					beSureBtnName: this.state.json['10140SBAE-000012'],          // 确定按钮名称, 默认为"确定",非必输/* 国际化处理： 确定*/
					cancelBtnName: this.state.json['10140SBAE-000013'],           // 取消按钮名称, 默认为"取消",非必输/* 国际化处理： 取消*/
			...
			case 'Save':
				setTimeout(() => {
					...
					if(tableData === undefined) {
						...
						toast({title: this.state.json['10140SBAE-000014'], color:'success'})/* 国际化处理： 保存成功！*/
					...
					allTableData.rows.forEach((row, index) => {
						if((row.values.code.value == null || row.values.code.value == '') && !hasError) {
							let content = [this.state.json['10140SBAE-000029']]
							toast({content:this.props.MutiInit.getIntl("10140INCPG").get('10140SBAE-000028',{count:index + 1, content}), color: 'warning'})/* 国际化处理： 已成功！,查询成功，共,条。*/
							hasError = true
						}
						else if((row.values.name.value == null || row.values.name.value == '')  && !hasError) {
							let content = [this.state.json['10140SBAE-000030']]
							toast({content:this.props.MutiInit.getIntl("10140INCPG").get('10140SBAE-000028',{count:index + 1, content}), color: 'warning'})/* 国际化处理： 已成功！,查询成功，共,条。*/
							hasError = true
						}
					})
					...
					let data = {
						...
						//pk_org: this.state.curOrg.refpk && this.state.curOrg.refpk !== '' ? this.state.curOrg.refpk : ''
					};
			...
			case 'Delete':
				...
				if(selectedData.length==0){
					toast({content:this.state.json['10140SBAE-000015'],color:'warning'});/* 国际化处理： 请选择要删除的数据*/
					return 
				}
				...
				if(this.props.editTable.getStatus(tableid) === 'edit'){//编辑状态
					...
					promptBox({
						color: 'warning',               // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
						title: this.state.json['10140SBAE-000016'],                // 提示标题, 默认不同类别下分别为："已成功"/"帮助信息"/"请注意"/"出错啦",非必输/* 国际化处理： 确认删除*/
						content: this.state.json['10140SBAE-000017'],             // 提示内容,非必输/* 国际化处理： 您确认删除所选数据？*/
						noFooter: false,                // 是否显示底部按钮(确定、取消),默认显示(false),非必输
						noCancelBtn: false,             // 是否显示取消按钮,，默认显示(false),非必输
						beSureBtnName: this.state.json['10140SBAE-000012'],          // 确定按钮名称, 默认为"确定",非必输/* 国际化处理： 确定*/
						cancelBtnName: this.state.json['10140SBAE-000013'],           // 取消按钮名称, 默认为"取消",非必输/* 国际化处理： 取消*/
			...
			case 'Refresh':
				...
				this.getData(this.state.isShowOff, () => {
					toast({title:this.state.json['10140SBAE-000018'],color:'success'});/* 国际化处理： 刷新成功！*/
				})
				break
			case 'Print':
				this.output('print')
			...
			case 'Output':
				...
				allData.forEach(item => {
					pks.push(item.values.pk_incomeperiod.value)
				})
				this.setState({
					pks: pks
				},() => {
	...
	output(type=''){
        ...
        allData.rows.forEach((row)=>{
            pks.push(row.values.pk_incomeperiod.value);
        });
        ...
        if(type!=''){
            ...
            print(
                ...
                urls.print,
                {
                    funcode:/*this.props.config.funcode*/'10140INCPG',     //功能节点编码
                    nodekey:'incomeperiod',     //模板节点标识
                    oids:pks,
                    outputType:type
                }
            )
	...
	onSelectMoreButton({ key }) {
		toast({content:this.state.json['10140SBAE-000019'],color:'warning'});/* 国际化处理： 努力开发中......*/
	 
	}

	onRowClick(props, moduleId, record, index) {
		//控制字段是否可编辑，只有在编辑态的时候可以进行
		if(this.props.editTable.getStatus(tableid) === 'edit') {
			...
			// if(curOrg && curOrg !== '' && record.values.pk_org.value !=='' && curOrg !== record.values.pk_org.value) {
			// 	this.allEditableColumns.forEach((item) => {
			// 		props.editTable.setEditableByKey(tableid,record.rowid,item,false)
			// 	})
			// }
		...
		ajax({
			...
			success: (res) => {     //此处使用箭头函数，如果不使用箭头函数，一定要bind(this)
				...
				if (success) {
					...
					this.props.editTable.setTableData(tableid,allD);
					setDefData('tableData', this.props.dataSource, allD)
					toast({title:this.state.json['10140SBAE-000020'],color:'success'});/* 国际化处理： 删除成功！*/
				}
			}
		});
	}
	...
	onOrgChange(val) {
		...
		//如果是下面的写法：getData函数当中的this指针是未定义的
		//setTimeout(() => {this.getData()})
		//setTimeout(this.getData.bind(this))
	...
	render() {
		...
		return (
			<div className="nc-single-table" fieldid='tableId'>
				<NCDiv  areaCode={NCDiv.config.HEADER} className="nc-singleTable-header-area" style={{borderBottom: 'none'}}>
					{/* 标题 title */}
					<div className="header-title-search-area">
						{createBillHeadInfo({
								title:(this.props.nodetype == 'group' ? this.state.json['10140SBAE-000021'] : this.state.json['10140SBAE-000022']),
						...
						<div className="title-search-detail">
							<NCFormControl
								placeholder={this.state.json['10140SBAE-000023']/* 国际化处理： 请输入编码或名称筛选*/}
						...
						<div className="title-search-detail">
							{isShowOffEnable?(
								<span className="showOff">
									...
									>{this.state.json['10140SBAE-000027']/* 国际化处理： 显示停用*/}</NCCheckbox>
								</span>
							):('')}
						</div>
					</div>
				...
				{/* 删除前确认模态框 */}
				{createModal('modal',{
					title : this.state.json['10140SBAE-000016'],										//标题/* 国际化处理： 确认删除*/
					content : this.state.json['10140SBAE-000024'],							//内容/* 国际化处理： 确认删除所选数据？*/
					beSureBtnClick : this.onDelForBrowse.bind(this),		//确定按钮事件回调  								//右侧按钮名称
				})}
				{createModal(('cancel'),{
					title: this.state.json['10140SBAE-000010'],/* 国际化处理： 确认取消*/
					content: this.state.json['10140SBAE-000011'],/* 国际化处理： 是否确认要取消？*/
					beSureBtnClick: this.onCancelSureEvent.bind(this)
				})}

				{createModal('enable',{
					title: this.state.json['10140SBAE-000025'],/* 国际化处理： 询问*/
