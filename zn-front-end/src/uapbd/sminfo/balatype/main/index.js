...
const tableid = 'balatype';
const pagecode = '10140BALT_balatype';
const nodetype = 'grp';
const appid = '0001Z010000000000O25';
const appcode = '10140BALT';
const urls = {
	save : '/nccloud/uapbd/balatype/balatypesave.do',
	query : '/nccloud/uapbd/balatype/balatypequery.do',
	queryTemplet : '/nccloud/platform/templet/querypage.do',
	print: '/nccloud/uapbd/balatype/balatypeprint.do'
};
const isShowOffEnable = true;			//是否启用“显示停用”功能
let hasData = false; // 列表是否有数据
let allTableData = {};
const keys = ['pk_org','netbankflag','defaultflag','cash','directincome','consignpay','istickcred','isretail','enablestate'];  //过来空行时，忽略的字段
const keysSwitch = ['netbankflag','defaultflag','cash','directincome','consignpay','istickcred','isretail'];  //过来空行时，忽略的字段
...
let initTemplate = (props) => {
	...
	(data)=>{
		...
		props.button.setPopContent('DelLine',props.MutiInit.getIntl("10140BALT")&&props.MutiInit.getIntl("10140BALT").get('10140BALT-000000')) /* 设置操作列上删除按钮的弹窗提示 */
...
function modifierMeta(props,meta) {
	...
	//添加表格操作列
	let event = {
		label: props.MutiInit.getIntl("10140BALT")&&props.MutiInit.getIntl("10140BALT").get('10140BALT-000001'),/* 国际化处理： 操作*/
		attrcode: 'opr',
		key: 'opr',
		itemtype: 'customer',
		visible:true,
	...
	meta['balatype'].items.map((item) => {
		if (item.attrcode == 'fundtype') {
				...
				ajax({
					...
					success: function(res) {
						...
						if (success) {
							...
							props.editTable.setTableData(tableid,allD);
							allTableData = allD;
							toast({content:props.MutiInit.getIntl("10140BALT")&&props.MutiInit.getIntl("10140BALT").get('10140BALT-000002'),color:'success'});/* 国际化处理： 删除成功*/
						}
					}.bind(this)
				});
			}
...
class SingleTable extends Component {
	...
	componentDidUpdate(){
		let l_formstatus = this.props.form.getFormStatus(tableid);
				...
				if (success) {
					Utils.handleTableReData({
						...
						notEmpty : (data)=>{	//数据不为空时执行回调方法
							...
							// data[tableid].rows.forEach((item,index) => {
							// 	for (var i in keysSwitch) {
							// 		if(!item.values[keysSwitch[i]])
							// 			item.values[keysSwitch[i]] = {value:false};
							// 	}
							// });
	...
	onAfterEvent(props, moduleId , key, changerows, value, index, data) {
		...
		// if(key === 'enablestate' || key === 'netbankflag'){
			
        if(keysSwitch.indexOf(key) > -1 ){
			...
			// 网银支付
			if (key === 'netbankflag' && rowData.values['netbankflag'].value) {
				
				props.editTable.setValByKeyAndIndex(moduleId, index, 'istickcred', {value: false});
			}
			// 业务逻辑的互斥处理
			if (key === 'directincome' || key === 'consignpay') {
				if (rowData.values[key].value) {
					// rowData.values['cash'].value = false;
					// rowData.values['istickcred'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'cash', {value: false});
					props.editTable.setValByKeyAndIndex(moduleId, index, 'istickcred', {value: false});
				}
			}
		    // 现金
			if (key === 'cash' && rowData.values['cash'].value) {
				// 直接借记
				if (rowData.values['directincome'].value) {
					// rowData.values['directincome'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'directincome', {value: false});
				}
				// 贷记转账
				if (rowData.values['consignpay'].value) {
					// rowData.values['consignpay'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'consignpay', {value: false});
				}
				// 信用证
				if (rowData.values['istickcred'].value) {
					// rowData.values['istickcred'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'istickcred', {value: false});
				}
			}
			// 信用证
			if (key === 'istickcred' && rowData.values['istickcred'].value) {
				if (rowData.values['cash'].value) {
					// rowData.values['cash'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'cash', {value: false});
				}
				if (rowData.values['consignpay'].value) {
				...
				if (rowData.values['netbankflag'].value) {
					// rowData.values['netbankflag'].value = false;
					props.editTable.setValByKeyAndIndex(moduleId, index, 'netbankflag', {value: false});
				}
			}
		}
		// 启用状态
	...
	updateButtonStatus(){
		...
		if(this.props.editTable.getStatus(tableid) === 'edit'){//编辑状态
			...
			this.props.button.setPopContent('DelLine',this.state.json['10140BALT-000000']);/* 国际化处理： 确认要删除该信息吗？*/
		}
	}
	...
	onButtonClick(props,id) {
		switch (id) {
			case 'Add':
				...
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'pk_org', {value: 'GLOBLE00000000000000',display:this.state.json['10140BALT-000003']});//设置组织默认值/* 国际化处理： 全局*/
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'netbankflag', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'defaultflag', {value: false});//设置默认值
				// this.props.editTable.setValByKeyAndIndex(tableid, num, 'fundtype', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'cash', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'directincome', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'consignpay', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'istickcred', {value: false});//设置默认值
				this.props.editTable.setValByKeyAndIndex(tableid, num, 'isretail', {value: false});//设置默认值
			...
			case 'Cancel':
				...
				promptBox({
					...
					title:this.state.json['10140BALT-000004'],/* 国际化处理： 确认取消*/
					content:this.state.json['10140BALT-000005'],/* 国际化处理： 是否确认要取消？*/
			...
			case 'Save':
				setTimeout(() => {
					...
					if(!tableData || tableData.length === 0) {
						...
						this.props.editTable.cancelEdit(tableid, this.updateButtonStatus.bind(this));
						toast({content: this.state.json['10140BALT-000006'], color:'success'});/* 国际化处理： 保存成功*/
						return
					}
						...
						ajax({
							...
							success: function (res)  {     //此处使用箭头函数，如果不使用箭头函数，一定要bind(this)
								...
								if (success) {
									...
									toast({title: this.state.json['10140BALT-000007'], color:'success'});							/* 国际化处理： 保存成功！*/
								}
							}.bind(this)
						});
					},{[tableid]: "editTable"});
			...
			case "Delete":
				...
				if(selectedData.length==0){
					toast({content:this.state.json['10140BALT-000008'],color:'warning'});/* 国际化处理： 请选择要删除的数据*/
					return 
				}
				if(this.props.editTable.getStatus(tableid) === 'edit'){//编辑状态
					let indexArr=[];
					...
					promptBox({
						color: 'warning', // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
						title:this.state.json['10140BALT-000009'],/* 国际化处理： 删除提醒*/
						// content:'确定要删除数据吗？',
						content:this.state.json['10140BALT-000010'],/* 国际化处理： 删除基础数据要做业务引用检查，非常耗时，建议使用封存功能，是否继续？*/
			...
			case "Refresh":
				...
				this.getData();
				toast({title: this.state.json['10140BALT-000011'], color:'success'});/* 国际化处理： 刷新成功！*/
				break;
			case 'Print':
				this.output('print');
				break;
    ...
    output(type=''){
        ...
        allData.rows.forEach((item)=>{
            pks.push(item.values['pk_balatype'].value);
        });
        if(type === "print"){
            //打印
			print(
	...
	onSelectMoreButton({ key }) {
		toast({content:this.state.json['10140BALT-000012'],color:'warning'});/* 国际化处理： 努力开发中......*/
	...
	onDelForBrowse(){
		...
		selectedData.forEach((val) => {
			...
			// delObj.values.pk_balatype.value=val.data.values.pk_balatype.value;
			dataArr.push(delObj);
			indexArr.push(val.index);
		});
		let data = {
		...
		ajax({
			...
			success: function(res)  {     //此处使用箭头函数，如果不使用箭头函数，一定要bind(this)
				...
				if (success) {
					...
					this.props.editTable.setTableData(tableid,allD);
					allTableData = allD;
					toast({title:this.state.json['10140BALT-000013'],color:'success'});/* 国际化处理： 删除成功！*/
				}
			}.bind(this)
		});
	}
	...
	addRowAutoCallback() {
		...
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'pk_org', {value: 'GLOBLE00000000000000',display:this.state.json['10140BALT-000003']});//设置组织默认值/* 国际化处理： 全局*/
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'netbankflag', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'defaultflag', {value: false});//设置默认值
		// this.props.editTable.setValByKeyAndIndex(tableid, num, 'fundtype', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'cash', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'directincome', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'consignpay', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'istickcred', {value: false});//设置默认值
		this.props.editTable.setValByKeyAndIndex(tableid, num-1, 'isretail', {value: false});//设置默认值
		...
		let moreButton = (
			<Menu
                  onSelect={this.onSelectMoreButton.bind(this)}>
					  ...
					  <Item key="3">this.state.json['10140BALT-000018']-this.state.json['10140BALT-000020']</Item> */}/* 国际化处理： 自定义档案维护,业务单元*/
                </Menu>
		);
			...
			<div className="nc-single-table">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-singleTable-header-area" style={{borderBottom:'none'}}>
					...
					<div className="header-title-search-area">
						...
						{/* {createPageIcon()} */}
						{/* 国际化处理： 结算方式*/}
						{/* <h2 className="title-search-detail">{this.state.json['10140BALT-000021']}</h2> */}

						{createBillHeadInfo(
							{
								title: this.state.json['10140BALT-000021'],//标题
						...
						<div className="title-search-detail">
							<NCFormControl
								placeholder={this.state.json['10140BALT-000014']/* 国际化处理： 请输入编码或名称筛选*/}
						...
						<div className="title-search-detail">
							{isShowOffEnable?(
								<span className="showOff">
									...
									>{this.state.json['10140BALT-000022']/* 国际化处理： 显示停用*/}</NCCheckbox>
							 	</span>
							):('')}
						</div>
					</div>
