...
import { base,high,ajax,toast, getGizp,getMultiLang} from 'nc-lightapp-front';
import {component} from '../../../public/platwapper/index.js';

let {getWholeUrl, setGlobalStore, getGlobalStore} = ViewModel;
const {NCForm,NCFormControl,NCRadio,NCIcon,NCCheckbox,NCSwitch,NCTextArea,NCNumber,NCRow,NCCol,NCUpload} = base;
...
class FileUploadForm extends Component{
    constructor(props){
        ...
        this.config = Object.assign({
            ...
            urls:{
                // 上传附件接口
                uploadAttachment: '/nccloud/uapbd/asslinenum/upload.do',
                // 上传url接口
        ...
        if(Array.isArray(me.config.items)){
            ...
            if(!isContainFile || isContainFile.length == 0)
                me.config.items.push({
                    itemType:'file',
                    label:this.state.json['10140CBCN-000000'],/* 国际化处理： 导入文件*/
    ...
    beforeUpload(file, fileList) {
        ...
        console.log('ff',filetype.indexOf(type))
       if(filetype.indexOf(type) <0){
            toast({content:this.state.json['10140CBCN-000001'],color:'warning'});/* 国际化处理： 只支持导入xlsx和xls格式的文件*/
            return false;
       }
        // if (!isLt5M) {
        //    // Message.create({content: '上传大小大于5M！', color: 'warning'});
        //    toast({content:'上传大小大于5M！',color:'warning'});
        //     return false;
        // }
        if(this.isExit.call(this,file)){
            toast({content:this.state.json['10140CBCN-000002'],color:'danger'});/* 国际化处理： 已存在同名文件！*/
...
console.log('response',response)
    ...
    createItems(){
        ...
        let formDOM = me.config.items.map((item, i)=>{
            let formitem;
            let initValue = item.initialValue ? item.initialValue.key : '' ||
            ...
            switch (item.itemType || 'input') {
                case 'input':
                    formitem = <NCFormControl
                        fieldid="uploadFormControl_item"
                ...
                case 'number':
                    ...
                    if (item.operateType === 'between') {
                        formitem = (
                            <NCNumber
                                fieldid="uploadNCNumber_item1"
                                start={{
                                    scale: scale ? scale : item.scale,
                                    placeholder: this.state.json['10140CBCN-000003']/* 国际化处理： 起始*/
                                }}
                                end={{
                                    scale: scale ? scale : item.scale,
                                    placeholder: this.state.json['10140CBCN-000004']/* 国际化处理： 结束*/
                                }}
                                value={initValue}
                ...
                case 'select':
                    formitem = (
                        <div className='asslinenum-self-option'>
                            <NCSelect
                                fieldid="selectItem"
                                labelInValue={false}
                                name={item.name}
                                ...
                                disabled={!!item.disabled}
                                placeholder={item.placeholder||this.state.json['10140CBCN-000005']}/* 国际化处理： --请选择--*/
                                onChange = {me.onChange.bind(this,{name:item.name,type:item.itemType})}
                                dropdownClassName={item.attrcode + '-' + 'select'}
                            >
                                {item.options &&
                                item.options.map((e, i) => (
                                    <NCOption value={String(e.value)}>
                                        {e.display}
                                    </NCOption>
                                ))}
                            </NCSelect>
                ...
                case 'file':
                    formitem =
                            <NCUpload
                                ...
                                accept={me.config.accept || 'xls,xlsx'}
                                size={item.size || 5*1024*1024}
                                NcUploadOnChange={
                                    
                                    me.handleChange.bind(this)
                                }
                                onRemove={me.onRemove.bind(this)}
                                beforeUpload={me.beforeUpload.bind(this)} >
                                <NCButton fieldid="button_upload" colors="primary" shape="border">
    ...
    onChange(name,value){
        ...
        switch(name.type.toLowerCase()){
            ...
            case 'select':
                ...
                realValue = typeof value != 'object' ? {value:value} : {'value':value.key ? value.key : value.value,'display':value.label};
                break;
        }
        me.combinedItemValue(name.name ? name.name :name,realValue);
    }
    /**
     * 组装表单数据
     * @param name
    ...
    combinedItemValue(name,value){
        if(!name){
            throw new Error(`${this.state.json['10140CBCN-000006']}${name}${this.state.json['10140CBCN-000007']}`);/* 国际化处理： 未找到,对应的表单项*/
        }
