...
export default function buttonClick(props, id) {
	...
	let data = props.form.getAllFormValue("accountingbookcard");
	let status = props.getUrlParam('status');
	let pk_accountingbook = data.rows[0].values.pk_accountingbook.value;
	//let pk_accbook = data.rows[0].values.pk_accbook.value;
	let user_code = data.rows[0].values.code.value;
	let user_name = data.rows[0].values.name.value;
	...
	switch (id) {
		...
		case 'accedit':
				...
				if(pk_accountingbook!=null&&""!=pk_accountingbook){
					props.form.setFormStatus('accountingbookcard', 'edit');
				...
				if(pk_accountingbook!=null&&""!=pk_accountingbook){
					...
					promptBox({
						...
						beSureBtnClick: () => {
							...
							let padata = {
								'model':data,
								'pageid':'101001CB_accbookcard'
								
							}
							ajax({
								url: '/nccloud/uapbd/accbook/accSave.do',
								data:{padata:padata,
									accpk:data.rows[0].values.pk_accountingbook.value,
								...
								success: function (res) {
									...
									if (res.success) {
										if(data.error!=null){
											...
											that.props.form.setAllFormValue({'101001CB_accbookcard':data.accountingbookcard});
											if('add'==that.props.getUrlParam('status')){
												data.data.accountingbookcard.rows[0].values.showflag = {value:res.data.isShow};
												addCache(data.accountingbookcard.rows[0].values.pk_accountingbook.value,data.accountingbookcard,'accountingbookcard','uapbd.org.accbook.acccache',data.accountingbookcard.rows[0].values);
												that.props.addUrlParam({id:data.accountingbookcard.rows[0].values.pk_accountingbook.value,status:'browse'})
											}else if('edit'==that.props.getUrlParam('status')){
												updateCache('pk_accountingbook',data.accountingbookcard.rows[0].values.pk_accountingbook.value,data.accountingbookcard,'accountingbookcard','uapbd.org.accbook.acccache',data.accountingbookcard.rows[0].values);
												that.props.setUrlParam({status:'browse'})
											}
											that.props.form.setFormStatus('data.accountingbookcard', 'browse');
											that.props.button.setButtonsVisible({
												add:true,
												edit:true,
												del:true,
		...
		case 'copy':
			...
			break;
		case 'accsave':
			if(props.form.isCheckNow('accountingbookcard')){
				let that = this;
				let data = props.form.getAllFormValue("accountingbookcard");
				...
				// toast({color: "warning", content: "灰度环境不允许保存!"});
				// return ;
				ajax({
					data:{padata:padata,
						accpk:data.rows[0].values.pk_accountingbook.value,
					...
					success: function (res) {
						...
						if (res.success) {
							if(data.error!=null){
								...
								that.props.form.setAllFormValue({'accountingbookcard':data.returnForm.accountingbookcard});
								if('add'==that.props.getUrlParam('status')){
									data.returnForm.accountingbookcard.rows[0].values.showflag = {value:res.data.isShow};
									addCache(data.returnForm.accountingbookcard.rows[0].values.pk_accountingbook.value,data.returnForm.accountingbookcard,'accountingbookcard','uapbd.org.accbook.acccache',data.returnForm.accountingbookcard.rows[0].values);
									that.props.addUrlParam({id:data.returnForm.accountingbookcard.rows[0].values.pk_accountingbook.value,status:'browse'})
								}else if('edit'==that.props.getUrlParam('status')){
									updateCache('pk_accountingbook',data.returnForm.accountingbookcard.rows[0].values.pk_accountingbook.value,data.returnForm.accountingbookcard,'accountingbookcard','uapbd.org.accbook.acccache',data.returnForm.accountingbookcard.rows[0].values);
									that.props.setUrlParam({status:'browse'})
								}
								that.props.form.setFormStatus('accountingbookcard', 'browse');
								that.props.button.setButtonsVisible({
									add:true,
									edit:true,
									del:true,
			...
			promptBox({
				...
				beSureBtnClick:() => {
					...
					if(pk_accountingbook!=null){
						let data = { pk: pk_accountingbook, pageid: '101001CB_accbookcard' ,bookproperties :'1'};
						...
						ajax({
							...
							success: function (res) {
								...
								if (res.success) {
									...
									that.props.form.setAllFormValue({ ["accountingbookcard"]: res.data.form.accountingbookcard});
					...
					that.props.form.cancel('accountingbookcard');
					that.props.form.setFormStatus('accountingbookcard', 'browse');
