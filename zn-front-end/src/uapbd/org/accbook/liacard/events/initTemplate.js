...
export default function (props) {
	let callback = (json, status, inlt) => { // json 多语json格式参数； status： 是否请求到json多语，可用来判断多语资源是否请求到并进行一些别的操作； inlt： 可用来进行占位符的一些操作
		 if (status) {
			props.createUIDom(
				{
					pagecode: '101001CB_libbookcard',//页面id
					// appcode:'10120USRM'
				},
				function (data) {
					if (data) {
						if (data.template) {
							...
							let accdata = { pk: id, pageid: '101001CB_liabookcard' ,bookproperties :'2'};
							ajax({
								url:  '/nccloud/uapbd/accbook/queryBypk.do',
								data: accdata,
								success: (res) => {
									if (res.data) {
										if(res.data.form.liabilitybook_card){ 
											props.form.setAllFormValue({ ["liabilitybook_card"]: res.data.form.liabilitybook_card});
											let enable = res.data.form.liabilitybook_card.rows[0].values.enablestate.value;
											if(status == 'edit'){
												props.form.setFormItemsDisabled('liabilitybook_card',{'pk_relorg':true})
												//props.form.setFormItemsDisabled('liabilitybook_card',{'pk_exratescheme':enable==2,'pk_setofbook':enable==2})
											}
										...
										props.form.EmptyAllFormValue("liabilitybook_card");
									}
									//callback&&callback();
									//toggleShow(status);
							...
							props.form.setFormStatus('liabilitybook_card', props.getUrlParam('status'));
						}
						if (data.button) {
