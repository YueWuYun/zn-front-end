//Qe7Mkbc6d37bjqj+gdxe/Qq2Ugxulxvz/PNDrQie6uQBgBYdx98W4r/NHaRp1Drh
...
function setByStatus(props) {
    ...
    props.form.setFormItemsValue(CARD.form_id,{'pk_ccstructure':{value:props.getUrlParam('id')}});
    props.form.setFormItemsValue(CARD.form_id,{'vno':{value:props.getUrlParam('version').vno}});
    props.form.setFormItemsValue(CARD.form_id,{'vname':{value:props.getUrlParam('version').vname}});
    props.form.setFormItemsValue(CARD.form_id,{'vstartdate':{value:props.getUrlParam('version').vstartdate}});
    props.form.setFormItemsValue(CARD.form_id,{'venddate':{value:props.getUrlParam('version').venddate}});
    if (status !== 'add') {
...
export function dealTreeData(data){
...
export function loadTree(props){
    let root = {
        "isleaf": false,
        "key":"~",
    ...
    ajax({
        ...
        data:{ type:"tree",pk:props.getUrlParam('id') },//{pkorg:this.state.curOrg.pk_org},
        success:(result)=>{
            if(result.success){
                let data = [Object.assign( {...root} , {children : result.data} )];
                //同步树  加载全部数据
                props.syncTree.setSyncTreeData("costCenterTree" , dealTreeData(data));
                //展开节点  设置默认展开项
                props.syncTree.openNodeByPk("costCenterTree", root.refpk);
                props.syncTree.setNodeSelected("costCenterTree",root.refpk);
            }
        }
    });
};
        ...
        success: (res) => {
            ...
            if (success) {
                if (data && data.head) {
                    ...
                    // props.form.setFormItemsValue(CARD.form_id,{'rootpk':{value:props.getUrlParam('id')}});
                }
                if (data && data.bodys) {
