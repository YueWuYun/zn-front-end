...
let templateCode = '101001PROPS_list';//动态加载模板，及显示公式
 ...
 */
 ...
 let pageCode = {
    mainPageCode: "10100PROPS_productionplanstru",
...
let ajaxUrl={
    ...
    sysLoadUrl:"/nccloud/uapbd/org/ProductionPlanStruQueryAction.do",
    sysDeleteUrl:"/nccloud/uapbd/org/ProductionPlanStruDeleteAction.do",
    sysEnableUrl:"/nccloud/uapbd/org/ProductionPlanStruEnableAction.do",
    sysDisableUrl:"/nccloud/uapbd/org/ProductionPlanStruDisableAction.do",
    sysQueryByPkUrl:"/nccloud/uapbd/org/ProductionPlanStruQueryByPkAction.do",
    sysSaveUrl:"/nccloud/uapbd/org/ProductionPlanStruSaveAction.do",
    sysCopySaveUrl:'/nccloud/uapbd/org/ProductionPlanStruCopySaveAction.do',
    memberLoadUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberQueryActon.do",
    memberSaveUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberSaveAction.do",
    memberQueryByPkUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberQueryByPk.do",
    memberDeleteUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberDeleteAction.do",
    memberAddUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberAddAction.do",
    importDataQryUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberImportQueryAction.do",
    importSaveUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberImportSaveAction.do",
    memberSortQryUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberSortQueryAction.do",
    memberSetOrderUrl:"/nccloud/uapbd/org/ProductionPlanStruMemberSetOrderAction.do",
    printUrl:"/nccloud/uapbd/org/ProductionPlanStruPrintAction.do",
    orgTreeLoadUrl:'/nccloud/uapbd/org/ProductionPlanStruQuerySourceOrgAction.do',
    createCreditUrl:'/nccloud/uapbd/org/ProductionPlanStruCreateFromOrgAction.do',
    batchSaveUrl:'/nccloud/uapbd/org/ProductionPlanStruBatchSaveAction.do'
}
...
class OrgSysPanel extends Component {
    ...
    loadLeftTreeData(showDisable, isRefreshAction=false,flag=true,flag2=true){
        this.props.button.setButtonsVisible({BatchAdd:false});
        ajax({
    ...
    onAdd(){
        sysSaveType = 'add';
        // this.openSysModal('','新增生产计划体系');
        this.openSysModal('',this.state.json['101001PPS-000061']);
    ...
    onCopy(){
        sysSaveType = 'copy';
        // this.openSysModal(this.props.syncTree.getSelectNode(leftTree).id, '复制生产计划体系');
        this.openSysModal(this.props.syncTree.getSelectNode(leftTree).id, this.state.json['101001PPS-000062']);
    }
    //启用
    onEnable(){
        let selectNode = this.props.syncTree.getSelectNode(leftTree);
    ...
    onEditSys(selectedTreeNode){
        sysSaveType = 'edit';
		this.openSysModal(selectedTreeNode.id, this.state.json['101001PPS-000061']);/* 国际化处理：生产计划体系*/
    }
    //体系编辑窗口
    openSysModal =(pk, title) => {
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    //弹出体系编辑窗口
                     that.props.modal.show(sysModalId, {title}, ()=>{
                         setTimeout(()=>{
                            that.props.form.setFormItemFocus(formId, 'code');
                         },1000)
                     });
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    if(sysSaveType==='add'){
                        toast({ color: 'success', title: that.state.json['101001PPS-000005'] });/* 国际化处理： 新增成功！*/
        ...
        ajax({
            ...
            success: (res) => {
                if (res.success) {
                    ...
                    let pk_pps = headData.pk_prod.value;
                    ...
                    this.props.modal.show(memberEditModalId,{title: '新增生产计划体系成员'}, ()=>{
                        setTimeout(()=>{
                            this.props.form.setFormItemFocus(memberFormId,'pk_fathermember');
                        }, 1000);
                    });
                    this.windowCloseListener('add');
    ...
    onImportDataLoad(showDisableOrg){
        ...
        ajax({
            ...
            method:"post",
            data:{pk_group:pk_importGroup,showDisable:showDisableOrg,pk_system:pk,pk_orgtypes:pk_orgtypes,pk_svid:sysVRefPk},
            success:function(res){
                if(res.success){
                     //弹出引入窗口
                    ...
                    <Col md={4} xs={4} sm={4}>
                        {OrgTypeGridRef({}=this.createCfg("OrgTypeGridRef",{
                                ...
                                return {
                                    ...
                                    GridRefActionExt:'nccloud.web.org.productionplanstru.action.ProductionPlanStruMemberImportSqlBuilder'
                                };
                            }.bind(this)
                        }))}
                    </Col>
            ...
            case 'Print':
                ...
                print(
                    ...
                    ajaxUrl.printUrl, 
                    {
                        ...
                        nodekey:'10100PROPS',     //模板节点标识(NC段默认模板配置)
    ...
    initTemplate = (props)=> {
        createUIDom(props)(
            ...
            {
                moduleId: "10100PROPS",domainName: 'uapbd'
            },
            (data,langData)=>{
                if(langData){
    ...
    modifierMeta=(props, meta)=> {
        meta[memberFormId].items.map((obj)=>{
            if(obj.attrcode == 'pk_fathermember'){
                ...
                obj.refcode = 'uapbd/refer/org/ProductionPlanStruMemberTreeRef/index.js';
            }
        });
        //添加操作列
    ...
    tableButtonClick=(props, id, text, record, index)=>{
        let pk = record.values.pk_prodmember.value;
        switch(id){
            case 'Editline':
            memberSaveType = 'edit';
            ajax({
                ...
                success: (res) => {
                    if (res.success) {
                        ...
                        props.modal.show(memberEditModalId,{title: this.state.json['101001PPS-000063']}, ()=>{
                            setTimeout(()=>{
                                props.form.setFormItemFocus(memberFormId, 'pk_fathermember')
                            },1000);
                        });
                        window.onbeforeunload = () => {//编辑态关闭页签或浏览器的提示
                            return '';
...
let OrgSysPage =  createPage({
    ...
    billinfo:[{
        billtype: 'form', 
        pagecode: '10100PROPS_productionplanstru', 
        headcode: 'sysForm',
    },{
        billtype: 'form', 
        pagecode: '10100PROPS_productionplanstru', 
