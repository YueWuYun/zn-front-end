...
import { loginContext, getContext, loginContextKeys } from '../../common/loginContext';
		...
		function (data) {
			if (data) {
				...
				if (data.button) {
					...
					//给行按钮上的删除附加气泡功能
					props.button.setPopContent('Delete_inner', that.state.json['38200CA-000002']);/*删除信息提示框*//* 国际化处理： 确定要删除吗？*/
				}
				if (data.template) {						
...
function modifierMeta(props, meta, lineButton, that) {
	...
	meta[that.tableId].items.push({
		label: that.state.json['38200CA-000001'],/* 国际化处理： 操作*/
		itemtype: 'customer',
		attrcode: 'opr',
		width: '180px',
		visible: true,
