...
export default function headafterEvent(props, moduleId, key, value, changedrows, i, s, g) {
    ...
    if (moduleId == this.formId) {
        ...
        switch (key) {
            ...
            //         promptBox({
            //             color: 'warning',
            //             title: this.state.json['38200CA-000012'] /* 国际化处理： 确认修改*/,
            //             content: this.state.json['38200CA-000013'] /* 国际化处理： 确定​修改组织，这样会清空您录入的信息?*/,
...
export function headFieldAfterRequest(requestData, key, changedrows) {
	ajax({
		url: '/nccloud/cm/inprocomstuff/cardheadafteredit.do',
		data: requestData,
		async: false,
		success: (res) => {
            //渲染数据
