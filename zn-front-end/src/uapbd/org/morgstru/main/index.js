...
const {NCDatePicker, NCButton ,NCPanel,NCCheckbox,NCPopconfirm,NCRadio,NCDiv,  NCCol:Col, NCRow:Row,NCFormControl} = base;
import './index.less';
...
let templateCode = '10100MOS_list';//动态加载模板，及显示公式
let templetid = '';

let config = {
    nodeType: 'GROPE_NODE',
    appId:'1002Z01000000000WUWW',
    appCode:'10100MOS',
    pageCode: '10100MOS_list'
}
 ...
 */
let ajaxUrl={
    sysLoadUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruQueryAction.do",
    sysDeleteUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruDeleteAction.do",
    sysEnableUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruEnableAction.do",
    sysDisableUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruDisableAction.do",
    sysQueryByPkUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruQueryByPkAction.do",
    sysSaveUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruSaveAction.do",
    sysCopySaveUrl:'/nccloud/uapbd/morgstru/MaintainOrgStruCopySaveAction.do',
    sysVersionQueryUrl:'/nccloud/uapbd/morgstru/MaintainOrgStruVersionQueryAction.do',
    sysVersionSaveUrl:'/nccloud/uapbd/morgstru/MaintainOrgStruVersionSaveAction.do',
    memberLoadUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberQueryActon.do",
    memberVersionLoadUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberVersionQueryActon.do",
    memberSaveUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberSaveAction.do",
    memberQueryByPkUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberQueryByPk.do",
    memberDeleteUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberDelAction.do",
    memberAddUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberAddAction.do",
    importDataQryUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberImportQueryAction.do",
    importSaveUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberImportSaveAction.do",
    memberSortQryUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberSortQueryAction.do",
    memberSetOrderUrl:"/nccloud/uapbd/morgstru/MOrgStruMemberSetOrderAction.do",
    printUrl:"/nccloud/uapbd/morgstru/MaintainOrgStruPrintAction.do"
}
...
class OrgSysPanel extends Component {
            ...
            success:(res)=>{
                if(res.success){
                    if(res.data.tree){
                        ...
                        if(res.data.tree[0].refpk!=='root'){
                            this.onSelectEve( res.data.tree[0].refpk,null,true);
                        }
                    ...
                    if(isRefreshAction){
                        toast({ color: 'success', title: this.state.json['10100MOS-000001'] });/* 国际化处理： 刷新成功！*/
                    }
                    this.toggleShow();
    ...
    onDeleteSysEve(selectedTreeNode){
        ...
        promptBox({
            ...
            title:  this.state.json['10100MOS-000018'],
            content:this.state.json['10100MOS-000002'],/* 国际化处理： 删除基础数据要做业务引用检查，非常耗时，建议使用封存功能，是否继续？*/
    ...
    onDeleteSys(){
        ...
        ajax({
            ...
            success:function(res){
                ...
                toast({ color: 'success', title: that.state.json['10100MOS-000003'] });/* 国际化处理： 删除成功！*/
            }
        });
    }
    //选择左树加载右表数据 控制启用停用按钮
	...
	toggleShow (){
        ...
        if(!allData||allData.length === 0){//左树无数据时只有新增和刷新可用
            this.props.button.setButtonDisabled(['Copy','Enable','Disable','Import','AddVirtual','Version','SortManage','Print','Output'],true);
            return;
        }else{
    ...
    onMouseEnterEve( key ) {
        if(key === this.state.json['10100MOS-000004']){/* 国际化处理： 体系--全局*/
            let obj = {
                delIcon:false,
                editIcon:false,
                addIcon:true
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    toast({ color: 'success', title: that.state.json['10100MOS-000044'] });/* 国际化处理： 启用成功！*/
                    // that.toggleShow();
                }
            }
        });
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    toast({ color: 'success', title: that.state.json['10100MOS-000045'] });/* 国际化处理： 停用成功！*/
                }
            }
        });
    }
    ...
    openSysModal(pk){
        ...
        ajax({
            ...
            method:"post",
            data:{pk_system:pk,nodeType:that.props.nodeType,queryType:sysSaveType,templateCode:templateCode,templetid:templetid},
            success:function(res){
                if(res.success){
    ...
    onSaveSys(){
        ...
        let validateData = {"model" :formdata,"pageid" :config.pageCode};
    ...
    systemSave(formdata){
        let requestParam = {
            model: formdata,
            pageid: config.pageCode//pageid : 我们导出的模板的名字  也就是 json数据的最外层的code值
        };
        let that = this;
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    if(sysSaveType=='add'){
                        toast({ color: 'success', title: that.state.json['10100MOS-000005'] });/* 国际化处理： 保存成功！*/
                    }else{
                        toast({ color: 'success', title: that.state.json['10100MOS-000005'] });/* 国际化处理： 保存成功！*/
                    }
    ...
    onEditMember(id,text, record, index){
        memberSaveType = 'edit';
        let pk_system =  record.values.pk_morgstru.value;
        ajax({
            url:ajaxUrl.memberQueryByPkUrl,  
            data: {pk_member:record.key,pk_system:pk_system},
            success: (res) => {
                if (res.success) {
                    ...
                    if(res.data.ismanageorg){
                        ...
                        this.props.form.setFormItemsDisabled(memberFormId,{'plan_flag':true})
                        props.form.setFormItemsDisabled(memberFormId,{'pk_entityorg':false})
                    }else{
                        this.props.form.setFormItemsDisabled(memberFormId,{'plan_flag':false})
     ...
     onAddMember(){
        ...
        if((allMember&&allMember.length>0)&&((!selectMember)||selectMember.length<1)){
            toast({ color: 'warning', content: this.state.json['10100MOS-000006'] });/* 国际化处理： 保存成功！*/
            return;
        }
        ...
        ajax({
            ...
            success: (res) => {
                if (res.success) {
                    ...
                    let pk_morgstru = headData.pk_morgstru.value;
                    ...
                    let cardData = {rows:res.data.memberForm.memberForm.rows};
                    let cardvalue = {'memberForm':cardData}
                    Utils.filterEmptyData(cardData.rows[0].values);
                    this.props.form.setAllFormValue(cardvalue);

                    let orgData = {rows:res.data.orgForm.orgForm.rows};
                    let orgvalue = {'orgForm':orgData}
                    Utils.filterEmptyData(orgData.rows[0].values);
                    this.props.form.setAllFormValue(orgvalue);
                    //新增的对应组织不可编辑
    ...
    onSaveMember(){
        ...
        let validateData = {"model" :orgdata,"pageid" :config.pageCode};
        this.props.validateToSave( validateData ,()=>{
    ...
    memberSaveFun(orgdata,){
        ...
        let requestParam = {
            model: formdata,
            pageid: config.pageCodee//pageid : 我们导出的模板的名字  也就是 json数据的最外层的code值
        };
        let orgRequest = {
            model:orgdata,
    ...
    onImportClick(){
        ...
        let selectMember = this.props.treeTableManyCol.getSelectedRow(rightTreeTable);
        if((allMember&&allMember.length>0)&&((!selectMember)||selectMember.length<1)){
            toast({content : this.state.json['10100MOS-000006'],color : 'warning'});/* 国际化处理： 组织体系内的成员树只能有一个根节点，请选择一个成员节点后再引入组织数据*/
            return;
        }
        ...
        ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    // that.orgTransfer.setRootTitle( groupname+'-' + that.state.json['10100MOS-000007'])/* 国际化处理： 资产*/
    ...
    onImportSave(){
        ...
        if(!(orgs&&orgs.length>0)){
            toast({content : this.state.json['10100MOS-000008'],color : 'warning'});/* 国际化处理： 您没有选择任何数据！*/
            return;
        }
    ...
    checkBeforeMove(nodes){
        ...
        nodes.forEach((item,index)=>{
            if(item.nodeData.isMember){
                ...
                toast({content : item.refname + this.state.json['10100MOS-000009'],color : 'warning'});
                l2rBefor = false;
                return;
                // returnResult = false;
                // return false;
    ...
    getImportDialog(){
        return (
			<div id="org_transfer" className="steps-content" style={{height:'450px'}} fieldid = 'morgstru_import_area'>
                <Row  style={{marginBottom:'5px'}}>
                    ...
                    <Col md={4} xs={4} sm={4}>
                        {OrgTypeGridRef({}=this.createCfg("OrgTypeGridRef",{
                                ...
                                return {
                                    //此处可以添加参数
                                    GridRefActionExt:'nccloud.web.uapbd.morgstru.action.MOrgMemberImportSqlBuilder'
                                };
                            }.bind(this)
                        }))}
                    </Col>
                    <Col>
                         <NCCheckbox id = 'orgShowDisable' 
                            ...
                            >{this.state.json['10100MOS-000031']}</NCCheckbox>{/* 国际化处理： 显示停用数据*/}
                    </Col>
                </Row>             
                <Transfer 
                ...
                <NCRadio.NCRadioGroup 
                    ...
                    <NCRadio value="0" >{this.state.json['10100MOS-000032']}</NCRadio>{/* 国际化处理： 包含所有下级*/}
                    <NCRadio value="1" >{this.state.json['10100MOS-000033']}</NCRadio>{/* 国际化处理： 仅自己*/}
                    <NCRadio value="2" >{this.state.json['10100MOS-000034']}</NCRadio>{/* 国际化处理： 仅直接下级*/}
                    <NCRadio value="3" >{this.state.json['10100MOS-000035']}</NCRadio>{/* 国际化处理： 仅末级*/}
                </NCRadio.NCRadioGroup>
			</div>
    ...
    setOrder(orderType){
        ...
        if(!selected){
            /* 国际化处理： 请选择要操作的数据！*/
            toast({content : this.state.json['10100MOS-000010'],color : 'warning'});
            return;
        }
    ...
    onClickButton(props, id){
        switch (id) {
            ...
            case 'Enable':
                promptBox({
                    color: 'warning',               // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
                    title:this.state.json['10100MOS-000011'],/* 国际化处理： 提示*/
                    content:this.state.json['10100MOS-000012'],/* 国际化处理： 确定要启用该数据吗？*/
            ...
            case 'Disable':
                ...
                promptBox({
                    color: 'warning',               // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
                    title:this.state.json['10100MOS-000011'],/* 国际化处理： 提示*/
                    content:this.state.json['10100MOS-000013'],/* 国际化处理： 确定要停用该数据吗？*/
            ...
            case 'Print':
                ...
                print(
                    ...
                    ajaxUrl.printUrl, 
                    {
                        ...
                        nodekey:'morgstruprint',     //模板节点标识(NC段默认模板配置)
    ...
    doubleClick = (record, index, e)=>{
        ...
        let pkvid = record.values.pk_vid.value;//singletable
        let that = this;
        ajax({
            url:ajaxUrl.memberVersionLoadUrl,
            method:"post",
            data:{pk_vid: pkvid},
            success:function(res){
                if(res.success){
                    if(res.data){
                        //后台返回的是表格的数据  需要构造成树状表的数据
                        let datas =  that.props.treeTableManyCol.createNewData(res.data.versionTreeTable.rows);
                        //根据树状表的数据构造树表
                        that.props.treeTableManyCol.initTreeTableData("versionTreeTable",datas,'refpk');
                    }else{
                        // let datas =  that.props.treeTableManyCol.createNewData({rows: []});
                        // that.props.treeTableManyCol.initTreeTableData(rightTreeTable,datas,'refpk')
                        that.props.treeTableManyCol.emptyAllData("versionTreeTable");
                    }
                    that.props.modal.show("versionTreeTable");
                }
            }
        });
    ...
    render() {
        ...
        return (
            <div className="nc-bill-tree-table">
                {createModal('warning',{
                     title:this.state.json['10100MOS-000014'],/* 国际化处理： 关闭提醒*/
                     content:this.state.json['10100MOS-000015'],/* 国际化处理： 是否确定要关闭？*/
                })}
                {/*体系 编辑模态框   this.state.json['10100MOS-000000'] 放到 createModal 前面是因为创建模态框的时候多语还没有加载，导致模态框没有标题，放到前面作用是没有多语则不加载模态框*/}
                { this.state.json['10100MOS-000000']&&createModal(sysModalId,{
                    title:this.state.json['10100MOS-000000'],/* 国际化处理： 维修组织体系*/
                    ...
                    cancelBtnClick: ()=>{//无法控制是否关闭模态框
                        ...
                        promptBox({
                            color: 'warning',               // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
                            title:this.state.json['10100MOS-000016'],/* 国际化处理： 确认取消*/
                            content:this.state.json['10100MOS-000017'],/* 国际化处理： 是否确认要取消？*/
                            beSureBtnClick:()=>{
                                this.props.modal.close(sysModalId);
                                this.toggleShow();
                                this.windowCloseListener('browse')
                 ...
                 {/*体系 删除模态框*/}
                {this.state.json['10100MOS-000018']&&createModal(sysModalDelId,{
                     title:this.state.json['10100MOS-000018'],/* 国际化处理： 删除提醒*/
                     content:this.state.json['10100MOS-000019'],/* 国际化处理： 确定要删除数据吗？*/
                     userControl:true,//自己控制什么时候关闭窗口
                     beSureBtnClick:this.onDeleteSys.bind(this),
                     cancelBtnClick: ()=>{this.props.modal.close(sysModalDelId)}
                })}
                 {/*体系成员- 编辑模态框*/}
                 { this.state.json['10100MOS-000020']&&createModal(memberEditModalId,{
                    title:this.state.json['10100MOS-000020'],/* 国际化处理： 维修组织体系成员*/
                    content:function(){
                        return(
                            <div>
                                <div>
                                     ...
                                     {/* <div> */}
                                        {/* <hr/> */}
                                        {this.state.json['10100MOS-000036']}:{/* 国际化处理： 组织信息*/}
                                     {/* </div> */}
                 ...
                 {/* 维修组织体系版本化模态框 */}
                 { this.state.json['10100MOS-000046']&&createModal(sysVersionModalId,{
                    title:this.state.json['10100MOS-000046'],/* 国际化处理： 预算组织体系版本化*/
                    content:function(){
                        return (
                            <div>
                                <div style={{display:'flex',justifyContent:'space-around',marginBottom: 10}}>
                                    <div style={{display: 'flex'}}>
                                        <div style={{width:95,marginTop: 3}}>
                                            {this.state.json['10100MOS-000047']}：
                                        </div>
                                        <NCFormControl fieldid='nextvno' type='text' id = 'nextvno' disabled value ={this.state.nextvno }  style={{width:'160px'}}/>{/* 国际化处理： 新版本号*/}
                                    </div>
                                    <div style={{display: 'flex'}}>
                                        <div style={{width:55,marginTop: 2}}>
                                            {this.state.json['10100MOS-000048']}：
                                        </div>
                                        <NCFormControl fieldid='newVersionName' type='text' id='newVersionName' name = 'newVersionName' value={this.state.versionName} required onChange={(value)=>{this.state.versionName = value;this.setState(this.state);}}  style={{width:'160px'}} />   { /* 国际化处理： 说明*/}
                                    </div>
                                    <div style={{display: 'flex'}}>
                                        {/* 生效日期<NCDatePicker  id = 'vstartdate' disabled  value ={this.state.vstartdate} style={{width:'120px'}}/>  */}
                                        <div style={{width:95,marginTop: 3}}>
                                            {this.state.json['10100MOS-000049']}：
                                        </div>
                                        <NCFormControl fieldid='vstartdate' type='text'  id = 'vstartdate' disabled  value ={this.state.vstartdate} style={{width:'160px'}}/> {/* 国际化处理： 生效日期*/}
                                    </div>
                                </div>
                                <div>
                ...
                {/*根据版本查看成员模态框*/}
                {this.state.json['10100MOS-000050']&& createModal("versionTreeTable",{
                    title:this.state.json['10100MOS-000050'],/* 国际化处理： 成员版本*/
                    content:function(){
                        return(
                            <div className="versionTreeTableCol">
                                { treeTableCol( "versionTreeTable",{
                 ...
                 {/*体系成员- 引入操作模态框*/}
                 { this.state.json['10100MOS-000021']&&createModal(importModalId,{
                    title:this.state.json['10100MOS-000021'],/* 国际化处理： 请选择要引入的组织*/
                ...
                {/*体系成员- 结构调整模态框*/}
                { this.state.json['10100MOS-000022']&&createModal(memberSortModalId,{
                    title:this.state.json['10100MOS-000022'],/* 国际化处理： 结构调整*/
                    content:function(){
                        return(
                            ...
                            <div class="ncc-hr-right-operate nc-theme-gray-area-bgc">
                                <div class="ncc-hr-padding">
                                    <div className="opr-botton" style={{marginBottom:'5px'}}>
                                        <NCButton  fieldid = 'upTop'  onClick= {this.setOrder.bind(this,'upTop')}>{this.state.json['10100MOS-000037']}</NCButton>{/* 国际化处理： 置于顶层*/}
                                    </div>
                                    <div className="opr-botton" style={{marginBottom:'5px'}}>
                                        <NCButton fieldid = 'upOne'  onClick= {this.setOrder.bind(this,'upOne')}>{this.state.json['10100MOS-000038']}</NCButton>{/* 国际化处理： 向上一层*/}
                                    </div>
                                    <div className="opr-botton" style={{marginBottom:'5px'}}>
                                        <NCButton  fieldid = 'downOne' onClick= {this.setOrder.bind(this,'downOne')}>{this.state.json['10100MOS-000039']}</NCButton>{/* 国际化处理： 向下一层*/}
                                    </div>
                                    <div className="opr-botton" style={{marginBottom:'5px'}}>
                                        <NCButton  fieldid = 'downBottom' onClick= {this.setOrder.bind(this,'downBottom')}>{this.state.json['10100MOS-000040']}</NCButton>{/* 国际化处理： 置于底层*/}
                                    </div>
                                    <div className="opr-botton" style={{marginBottom:'5px'}}>
                                        <NCButton fieldid = 'upgrade'  onClick= {this.setOrder.bind(this,'upgrade')}>{this.state.json['10100MOS-000041']}</NCButton>{/* 国际化处理： 升级*/}
                                    </div>
                                    <div className="opr-botton" style={{marginBottom:'10px'}}>
                                        <NCButton  fieldid = 'degrade' onClick= {this.setOrder.bind(this,'degrade')}>{this.state.json['10100MOS-000042']}</NCButton>{/* 国际化处理： 降级*/}
                                    </div>
                ...
                <NCDiv  areaCode={NCDiv.config.HEADER}  className="header">
                    {/*页面大图标*/}
                    {/* {createPageIcon()} */}
                    {/* <h2 className="title">{this.state.json['10100MOS-000000']}</h2>国际化处理： 维修组织体系 */}
                    {createBillHeadInfo({
                        title:this.state.json['10100MOS-000000']/* 国际化处理： 维修组织体系*/,
                    ...
                    <span className="showOff">
                        ...
                        >
                            {this.state.json['10100MOS-000043']}{/* 国际化处理： 显示停用*/}
                        </NCCheckbox>
			        </span>
                    {/* 按钮区  btn-group */}
                            ...
                            <div className="treeTableCol">
                                 ...
                                 <Row style={{marginBottom: 10,marginTop: 10,marginLeft:10}}>
                                    ...
                                    <NCDiv areaCode={NCDiv.config.SEARCH}> 
                                        <NCFormControl
                                            placeholder={this.state.json['10100MOS-000052'] }
    ...
    initTemplate = (props)=> {
        createUIDom(props)(
            ...
            {
                moduleId: "10100MOS",domainName: 'uapbd'
            },
            (data,langData)=>{
                if(langData){
                    this.state.json = langData
                    orgTypeName = this.state.json['10100MOS-000051'];//引入组织类型名称/* 国际化处理： 业务单元*/
                }
                if(data){
                    ...
                    if(data.button){
                        ...
                        props.button.setPopContent('Delline',this.state.json['10100MOS-000029']) /* 10100MOS-000029-'确认要删除该条吗？ 设置操作列上删除按钮的弹窗提示 */
                    }
                    // callback && callback();
                }
        });
    ...
    modifierMeta=(props, meta)=> {
        ...
        meta[rightTreeTable].items.push({
            attrcode: 'opr',
            label: this.state.json['10100MOS-000030'],/* 国际化处理： 操作*/
    ...
    tableButtonClick=(props, id, text, record, index)=>{
        let pk = record.values.pk_morgstrumember.value;
        let pk_system =  record.values.pk_morgstru.value;
        switch(id){
            case 'Editline':
            memberSaveType = 'edit';
            ajax({
                url:ajaxUrl.memberQueryByPkUrl,  
                data: {pk_member:pk,pk_system:pk_system},
                success: (res) => {
                    if (res.success) {
                        let headData = res.data.memberForm.memberForm.rows[0].values;
                        let pk_morgstru =  headData.pk_morgstru.value;
            ...
            case 'Delline':
                ajax({
                    ...
                    data: {pk_member:pk,ts:record.values.ts.value,pk_system:pk_system},
                    success: (res) => {
                        if (res.success) {
                            toast({ color: 'success', title: this.state.json['10100MOS-000003'] });/* 国际化处理： 删除成功！*/
