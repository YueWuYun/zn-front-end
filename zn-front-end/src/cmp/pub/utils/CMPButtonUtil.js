/*iubfDWcQXRoz8QfHqOsQt6slRO7BMf7nUlKMatNi7vA0eKBApWDTIxnG/FZ7elLx*/
...
export const BatchDelLine = function (props, areacode, callback) {
    ...
    if (selectRows == null || selectRows.length == 0) {
        toast({
            'color': 'warning',
            'content': loadMultiLang(props, '36070APM--000118')/*'未选中要删除的行'*/
        });
        return false;
    }
    let selectIndexs = [];
...
export const BatchCopy = function (props, areacode, index) {
    ...
    if (selectRows == null || selectRows.length == 0) {
        toast({
            'color': 'warning',
            'content': loadMultiLang(props, '36070APM--000119')/*'未选中要复制的行'*/
        });
        return false;
    }
    let selectIndexs = [];
    ...
    if (Object.keys(pkMapTs).length > 0) {
        ajax({
            ...
            success: (res) => {
                ...
                succContent = succCount > 0 ? succContent + loadMultiLang(props, '36070APM--000120')/*'项'*/ + actionname + loadMultiLang(props, '36070APM--000121')/*'成功'*/ : '';
                let content = (succContent == '' ? '' : succContent + '，') + (failCount > 0 ? errContent + loadMultiLang(props, '36070APM--000120')/*'项'*/  + actionname + loadMultiLang(props, '36070APM--000122')/*'失败'*/ : '');
                //全部成功
                if (status == 0 && errMsgArr.length == 0) {
                    toast({ color: 'success', content: actionname + loadMultiLang(props, '36070APM--000123')/*"成功，共"*/ + succCount + loadMultiLang(props, '36070APM--000124')/*'条'*/ });
                }
                //全部失败/部分失败
                    ...
                    toast({
                        ...
                        color: 'danger',
                        content,
                        TextArr: [loadMultiLang(props, '36070APM--000125')/*'展开'*/, loadMultiLang(props, '36070APM--000126')/*'收起'*/, loadMultiLang(props, '36070APM--000127')/*'我知道了'*/],
                        groupOperation: true,
...
export const cardOperator = function (props, pageCode, headCode, bodyCodeMapbodyPKName, pkName, url, actionname, datasource, callback, showTBB, extParam) {
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (callback && (typeof callback == 'function')) {
                //如果是缴存或者支取
                if(pageCode=='36070DC_C01' || pageCode=='36070WC_C01' || pageCode=='36070FCE_C01'){
                    callback(data);
                }else{
                    callback(props, data);
                }
