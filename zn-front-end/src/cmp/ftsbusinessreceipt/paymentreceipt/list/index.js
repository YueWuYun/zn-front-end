...
import { createPage, ajax, base, getMultiLang, toast, createPageIcon, deepClone, cardCache } from 'nc-lightapp-front';
...
class List extends Component {
	...
	DoubleClick = (record, index, props, e) => {
		this.props.pushTo('/card', {
			status: 'browse',
			id: record.pk_paymentreceipt_h.value,
		});
	}
	/**
	 * 表格选择框有变动的钩子函数
	...
	selectedChangeFn = (props, moduleId, newVal, oldVal) => {
		...
		if(this.props.getUrlParam('scene') == 'linksce'){
			...
			if(checkRows && checkRows.length > 0){
				...
				this.props.button.setButtonDisabled(['formalprint', 'supplyprint', 'print', "printout"], false);
			}else{
			this.props.button.setButtonDisabled(['formalprint', 'supplyprint', 'print', "printout"], true);
		}
	}
							...
							{createBillHeadInfo(
                                {
									title: this.props.MutiInit.getIntl("36300REC") && 
										this.props.MutiInit.getIntl("36300REC").get('36300REC-000000'),//标题
