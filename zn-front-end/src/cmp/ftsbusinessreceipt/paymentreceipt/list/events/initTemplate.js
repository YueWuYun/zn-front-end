...
function modifierMeta(that, props, meta) {
	...
	meta[searchId].items.find((e) => e.attrcode === 'pk_fundpayorg').isMultiSelectedEnabled = true;
	...
	item.isShowDisabledData = true;
		...
		if (item.attrcode == 'pk_fundpayorg') {
			item.queryCondition = () => {
				let pk_fundpayorg = props.search.getSearchValByField(searchId, 'pk_fundpayorg').value.firstvalue; // 调用相应组件的取值API
				return { pk_fundpayorg: pk_fundpayorg }; 
			};
			//支持跨集团
