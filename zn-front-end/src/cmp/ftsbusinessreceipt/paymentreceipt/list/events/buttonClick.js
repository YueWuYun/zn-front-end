        ...
        */
		case 'print':
			...
			if (printdata.length == 0) {
				...
				printdata.forEach((vale) => {
					pks.push(vale.data.values.pk_paymentreceipt_h.value);
				});
			}
			print(
		...
		case 'printout':
			...
			outputBtnData.forEach((item) => {
				oids.push(item.data.values.pk_paymentreceipt_h.value);
			});
			output({
				url: Print_URL,
				data: {
		...
		case 'link':
			...
			// 	return;
			// }
			if(record && record.pk_srcbill && record.pk_srcbill.value){
				let pk_srcbill = record.pk_srcbill.value;
				// linkQueryData.forEach((vale) => {
				// 	pk_srcbill = vale.data.values.pk_srcbill.value;
				// });
				let sbExtParam = {
					status: 'browse',
		...
		case 'formalprint':
			...
			elecSignListPrint(props, {
				...
				getOrgFunc: (selectData) => {
					let pk_fundpayorg = selectData && selectData.data && selectData.data.values && selectData.data.values['pk_fundpayorg'] && selectData.data.values['pk_fundpayorg'].value;
					if(pk_fundpayorg){
						return pk_fundpayorg;
					}
					else{
