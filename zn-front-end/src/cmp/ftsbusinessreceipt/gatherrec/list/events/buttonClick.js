	...
	switch (id) {
		...
		case 'print':
			...
			if (printdata.length == 0) {
				...
				printdata.forEach((vale) => {
					pks.push(vale.data.values.pk_gatheringreceipt_h.value);
				});
			}
			print(
		...
		case 'printout':
			...
			outputBtnData.forEach((item) => {
				oids.push(item.data.values.pk_gatheringreceipt_h.value);
			});
			output({
				url: Print_URL,
				data: {
		...
		case 'formalprint':
			...
			elecSignListPrint(props, {
				...
				getOrgFunc: (selectData) => {
					let pk_fundgetorg = selectData && selectData.data && selectData.data.values && selectData.data.values['pk_fundgetorg'] && selectData.data.values['pk_fundgetorg'].value;
					if(pk_fundgetorg){
