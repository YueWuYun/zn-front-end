...
function modifierMeta(that, props, meta) {
	...
	meta[searchId].items.find((e) => e.attrcode === 'pk_fundgetorg').isMultiSelectedEnabled = true;
	...
	item.isShowDisabledData = true;
		...
		if (item.attrcode == 'pk_fundgetorg') {
			item.queryCondition = () => {
				let pk_fundgetorg = props.search.getSearchValByField(searchId, 'pk_fundgetorg').value.firstvalue; // 调用相应组件的取值API
				return { pk_fundgetorg: pk_fundgetorg }; // 根据pk_org_r_v过滤
			};
			//支持跨集团
	...
	meta[tableId].items.push({
		attrcode: 'opr',
		label:
			props.MutiInit.getIntl('36300REC') &&
			props.MutiInit.getIntl('36300REC').get('36300REC-000016') /* 国际化处理： 操作*/,
		width: 160,
		itemtype: 'customer',
		className: 'table-opr',
		fixed: 'right',
