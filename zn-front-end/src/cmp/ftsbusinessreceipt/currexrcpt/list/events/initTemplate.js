	...
	meta[searchId].items.map((item) => {
		...
		if (item.attrcode == 'pk_fundorg') {
			item.queryCondition = () => {
				...
				return { pk_fundorg: pk_fundorg }; // 根据pk_org_r_v过滤
			};
			//支持跨集团
		...
		// 买入内部账户
		if (item.attrcode == 'pk_inneraccount_in') {
		
			item.queryCondition = () => {
				let pk_currtype = props.search.getSearchValByField(searchId, 'pk_incurrtype').value.firstvalue;
				return {
		...
		// 卖出内部账户
		if (item.attrcode == 'pk_inneraccount_out') {
			item.queryCondition = () => {
				let pk_currtype = props.search.getSearchValByField(searchId, 'pk_outcurrtype').value.firstvalue;
				return {
					pk_currtype : pk_currtype
				};
			};
