	...
	getData = () => {
		...
		if (this.props.getUrlParam('status') == 'browse') {
			...
			if (pk != 'null') {
				let pks = [];
				pks.push(pk);
				let data = { pks: pks, pageCode: pageCodeCard, extParam };
				ajax({
					url: Query_BY_PK_URL,
					data: data,
					success: (res) => {
							...
							<div className="header-title-search-area">
								{
									createBillHeadInfo(
										{
											title: this.props.MutiInit.getIntl("36300REC") && this.props.MutiInit.getIntl("36300REC").get('36300REC-000007'),  //标题
