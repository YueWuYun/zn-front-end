        ...
        */
		case 'print':
			...
			if (printdata.length == 0) {
				...
				printdata.forEach((vale) => {
					pks.push(vale.data.values.pk_intranrcpt_h.value);
				});
			}
			print(
		...
		case 'printout':
			...
			outputBtnData.forEach((item) => {
				oids.push(item.data.values.pk_intranrcpt_h.value);
			});
			output({
				url: Print_URL,
				data: {
		...
		case 'formalprint':
			...
			elecSignListPrint(props, {
				...
				getOrgFunc: (selectData) => {
					let pk_fundorg = selectData && selectData.data && selectData.data.values && selectData.data.values['pk_fundorg'] && selectData.data.values['pk_fundorg'].value;
					if(pk_fundorg){
						return pk_fundorg;
					}
					else{
