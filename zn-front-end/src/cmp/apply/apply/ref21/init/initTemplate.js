	...
	props.createUIDom(
		{
			...
			// appcode: REF21_CONST.appcode //应用编码
		},
		(data) => {
			if (data) {
				if (data.template) {
...
export const loadSearchCache = function (props) {
    //从缓存中获取查询区域条件
    let searchData = cardCache.getDefData(REF21_CONST.searchId, REF21_CONST.Ref21DataSource);
    //更新查询区域
    if (searchData) {
        props.search.setSearchValue(REF21_CONST.searchId, searchData.querycondition);
    }
}
