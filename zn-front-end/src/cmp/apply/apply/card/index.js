...
import { buttonClick, initTemplate, afterEvent, beforeEvent ,bodyBeforeEvent,pageInfoClick, buttonVisible, bodyBtnVisible } from './events';
...
import { loadData2Card, qryDataByPK, loadNeedDealDataByPK, loadCopyData, repaintView, save ,getNewRow} from "../util/index";
//引入附件组件，审批详情,联查预算
...
import {savecommit} from "../card/events/buttonClick";
//加载小应用基础部件
...
class Card extends Component {
	...
	componentDidMount() {
		...
		if(id21){			
			if(status ==SHOW_MODE.ADD){
				...
				cacheTools.set('21TO36D1Pks', null);
			}
		}
		if(id22){			
			if(status ==SHOW_MODE.ADD){
				...
				this.props.form.setFormItemsDisabled(CARD_PAGE_INFO.HEAD_CODE, { 'pk_supplier': true });
				cacheTools.set('Z2TO36D1Pks', null);
			}
		}
	...
	render() {		
		...
		let { showUploader, billID, billNO, tradeType,showInnerAccInfo, pk_inneracc, showApproveDetail, showNtbDetail, ntbdata, showBankAccBalance, bankAccBalanceParam ,tpflag ,showbilltrack,showbilltracktype,showbilltrackpk, assignData, assignShow,assignType } = this.state;
		const that = this;
		const { createTransferList } = transferTable;
		let ishowback = this.props.getUrlParam("status") === "browse"; 	
		//单据抽取	
		let islink = this.props.getUrlParam('scene')== "zycx" || this.props.getUrlParam('scene')== SCENE.LINK;
				...
				<div className="nc-bill-top-area">
					...
					<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area"> 
						...
						<div className="header-button-area"> 
							...
							<div className="button-app-wrapper">
								{status == SHOW_MODE.BROWSER && !islink && (
									<Refer
										...
										clickContainer={<NCButton>{loadMultiLang(this.props, '36070APM--000019')}</NCButton>}/* 国际化处理： 付款交易类型*/
									/>
								)}
							</div>
							<div>								
...
Card = createPage({
	...
	orderOfHotKey: ["mainform_apply_01", "table_apply_01"]
})(Card);
