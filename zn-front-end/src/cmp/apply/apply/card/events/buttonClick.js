...
//引入公共api
import { AddLine, BatchDelLine, DelLine, Open, CopyLine, BatchCopy, cardOperator ,showTBBMsg} from '../../../../pub/utils/CMPButtonUtil';
//影像
import { imageScan, imageView } from 'sscrp/rppub/components/image';
//引入内转api
import { qryDataByPK,emptyView, loadNeedDealDataByPK, repaintView, loadData2Card, save ,edit ,addLineAction,delLineAction,addNewRow,InsertLine} from "../../util/index";
// ca
...
export const buttonClick = function (props, key, text, record, index) {
  ...
  switch (key) {
    //头部 刷新
    case 'Refresh':
      qryDataByPK(props, pk,()=>{toast({ color: 'success', content: loadMultiLang(props, '36070APM--000132') }); });      
      break;
    case 'Copy':
      copy(props);
      break;
    ...
    //提交
    case 'Commit':
      cardOperator(props, CARD_PAGE_INFO.PAGE_CODE, CARD_PAGE_INFO.HEAD_CODE, bodyCodeMapBodyPKName, ITEM_INFO.PK, URL_INFO.COMMON.COMMIT, loadMultiLang(props, '36070APM--000009'),  APP_INFO.DATA_SOURCE, commitAssign.bind(this),true);      
      break;
    //收回
    case 'Uncommit':
      cardOperator(props, CARD_PAGE_INFO.PAGE_CODE, CARD_PAGE_INFO.HEAD_CODE, bodyCodeMapBodyPKName, ITEM_INFO.PK, URL_INFO.COMMON.UNCOMMIT, loadMultiLang(props, '36070APM--000010'), APP_INFO.DATA_SOURCE, repaintView.bind(this, props),true);
      break;
    //头部 附件
    case 'Filedoc':
      this.setState({
...
/**保存提交 */
export const savecommit = function (props, assign) {
  saveCommit(props, {
      //页面编码
      pageCode: CARD_PAGE_INFO.PAGE_CODE,
      //表头区域编码
      ...
      updateViewFunc: (res) => {
          saveUpdateView(props, res, () => {
              toast({ color: 'success', content: loadMultiLang(props, '36070APM--000137')/* 国际化处理： 提交成功！*/ });
          });
      },
      //保存的校验（无需考虑验证公式，本身包含）
      saveValidate: () => {
