  ...
  ajax({
    ...
    success: (res) => {
      if (res.success) {
        toast({
          color: 'success',
          content: this.props.MutiInit.getIntl("36070WC") && this.props.MutiInit.getIntl("36070WC").get('36070WC-000000') /* 国际化处理： 保存成功*/
        });
    ...
    success: (res) => {
      ...
      if(success){
        if(data){
          ...
          if(billform){
            let id = billform[formcode1].rows[0].values.pk_cashdraw.value
            if (status === 'save' || status === 'copy') {
              addCache(id, billform, formcode1, this.cacheDataSource, billform[formcode1].rows[0].values);
            }
            if (status === 'edit') {
              updateCache(this.pkname, id, billform, formcode1, this.cacheDataSource, billform[formcode1].rows[0].values);
            }
          ...
          if(appointmap){
            ...
            toast({
              color: 'success',
              content: this.props.MutiInit.getIntl("36070WC") && this.props.MutiInit.getIntl("36070WC").get('36070WC-000001') /* 国际化处理： 提交成功*/
            });
          }
        }
      }
