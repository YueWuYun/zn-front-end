...
export default function tableButtonClick(props, key, text, record, index) {
	...
	let that = this;
	//缓存相关
	let { deleteCacheId, addCacheId } = props.table;
	...
	function getrequestdata() {
		...
		let indexmap = {};
		//处理选择数据
		let pk = record.pk_cashdraw.value;
		pksArr.push(pk); //主键数组
		...
		that.pk = record.pk_cashdraw.value;
		that.index = index;
	...
	switch (key) {
		...
		case 'unsubmitinBtn':
			...
			// 修改
		case 'editinBtn':
			let editid = record.pk_cashdraw.value;
		...
		case 'makebillBtn':
			//处理选择数据
			let pk_cashdraw = record.pk_cashdraw.value;
			let tableName = ITEM_INFO.tableName;
			let pkfieldName = ITEM_INFO.PK;
			MakeBillApp(this.props, constant.appcode, pk_cashdraw, constant.billtype,tableName,pkfieldName);
			break;
	}
