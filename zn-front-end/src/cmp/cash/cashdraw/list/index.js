...
class List extends Component {
    ...
    voucherLinkBill = () => {
        ...
        if(checkedData && checkedData.length > 0){
            ...
            ajax({
				...
				if (success) {
					if (data) {
						cardCache.setDefData(constant.fipscene_key, this.cacheDataSource, true);
						...
						if(data.model){
							if(data.model.rows.length>1){
								...
								this.linkGoToCard(data.model.rows[0].values.pk_cashdraw.value);
							}
                            // this.props.table.setAllTableData(this.tableId, data.model);
                        } else {
					...
					if (success) {
						if (data) {
							...
							if(allnum > 0){
								toast({
									color: 'success',
									content: this.state.json['36070WC-000051']
								});
							}else{
								toast({
									color: 'warning',
									content: this.state.json['36070WC-000047']
								});
							}
	...
	delConfirm = () => {
		...
		if (selectdata.length == 0) {
			toast({
				color: 'warning',
				content: this.state.json['36070WC-000027'] /* 国际化处理： 请选择数据*/
			});
			return;
		}
			 ...
			 {createListWebSocket(this.props, {
						...
						{createBillHeadInfo(
							{
								...
								// loadMultiLang(this.props, '36070WC-000019'),//国际化处理： 现金支取
				...
				{!isfiplink&&<div className="tab-definInfo-area">
					<NCTabs activeKey={this.state.tabInfo} onChange={(v) => {this.navChangeFun.call(this, v);}}>
                        <NCTabPane key={'0'} tab={
							<span>{this.state.json['36070WC-000032']+ '('}
								<span>{this.state.tobesubmittab || 0}</span>
								{')'}
							</span>
                            //待提交
							// this.state.json['36070WC-000032']+ '(' +  (this.state.tobesubmittab || 0)+ ')'
                        } />
                        <NCTabPane key={'1'} tab={
							<span>{this.state.json['36070WC-000033']+ '('}
								<span>{this.state.approvingtab || 0}</span>
								{')'}
							</span>
                            //审批中
							// this.state.json['36070WC-000033']+ '(' +  (this.state.approvingtab || 0)+ ')'
                        } />                        
                        <NCTabPane key={'2'} tab={
							<span>{this.state.json['36070WC-000034']+ '('}
								<span>{this.state.tobesettletab || 0}</span>
								{')'}
							</span>
                            //待结算
                            // this.state.json['36070WC-000034']+ '(' +  (this.state.tobesettletab || 0)+ ')'
                        } />
						<NCTabPane key={'3'} tab={
                            //全部
                            this.state.json['36070WC-000035']
                        } />
                    </NCTabs>
				</div>}
