...
import { FormBeforeEvent, CardTableBeforeEvent } from '../../util/BXTableRefFilter';
...
export default function beforeEvent(props, moduleId, key, value, index, record) {
	let pk_org = props.form.getFormItemsValue(FORM_BBM_BX01, 'pk_org');
	...
	if (moduleId == TABLE_BBM_BX01) {
		switch (key) {
			default:
				CardTableBeforeEvent.call(this,props, moduleId, key, value, index, record);
			return true; //默认单元格都可操作
			break;
		}
	}
