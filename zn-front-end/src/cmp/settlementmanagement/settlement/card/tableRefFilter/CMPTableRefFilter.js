...
import { bodyChildBeforeEvent } from "./CMPEditTableRefFilter.js";
import { DirectEcdsEdit } from "../buttonClick/EditControlUtil.js";
import { NotetopeTablebeforeEvent } from "../../util/CMPChangeRefEvent.js";//票据号参照编辑前
/**
...
function bodyBeforeEvent(props, moduleId, key, value, index, record) {
    ...
    //侧拉框吧编辑前事件
    bodyChildBeforeEvent.call(this, props, "detail_edit", key, value, index, record);
    ...
    meta[moduleId].items.map((item) => {
        ...
        if (attrcode == key) {
            switch (attrcode) {
                ...
                let Pk_balatype = meta['detail_edit'].items.find((e) => e.attrcode === 'pk_balatype');
                Pk_balatype.queryCondition = () => {
                        let dataObject = {
