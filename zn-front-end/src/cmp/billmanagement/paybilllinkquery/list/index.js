...
const { NCBreadcrumb, NCTabsControl } = base;
...
class List extends Component {
	...
	modalContent() {
		return <div className="addModal">{this.props.form.createForm('form1')}</div>;
	}
	...
	render() {
		...
		console.log('数据信息', getTablePageInfo(this.tableId));
		return (
			<div className="nc-bill-list">
				<div className="nc-bill-header-area">
					<div className="header-title-search-area">
						<h2 className="title-search-detail">{'付款结算单'}</h2>
						{/* multiLang && multiLang.get('36300TP-0003') */}
					</div>
				...
				<div className="tab-definInfo-area">
					<NCTabsControl defaultKey={1}>
						<div key={0} clickFun={this.navChangeFun.bind(this, '0')}>
							保存<span>{(numvalues && numvalues.SAVED) || 0}</span>
						</div>
						<div key={1} clickFun={this.navChangeFun.bind(this, '1')}>
							待审批<span>{(numvalues && numvalues.FORCOMMIT) || 0}</span>
						</div>
						<div key={2} clickFun={this.navChangeFun.bind(this, '2')}>
							审批中<span>{(numvalues && numvalues.APPRING) || 0}</span>
						</div>
						<div key={4} clickFun={this.navChangeFun.bind(this, '3')}>
							全部<span>{(numvalues && numvalues.ALL) || 0}</span>
						</div>
					</NCTabsControl>
				</div>
				<div>
					{createModal('reverse', {
						...
						content: multiLang && multiLang.get('确认删除？'),
