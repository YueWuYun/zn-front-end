...
export const addbodyBtn = function () {
    ...
    if (org_val && org_display) {
        ...
        // clsRowno(this.props, this.tableId);
        //根据表头数据给新增表体数据赋值
        let keyArr = ['pk_currtype', 'pk_balatype', 'pk_account', 'rec_primal', 'note_type',
            'mon_account', 'note_no', 'mon_account', 'local_rate','group_rate','global_rate', 
            'rec_local', 'objecttype',
            'pk_customer', 'pk_org', 'pk_pcorg', 'pk_busiman', 'pk_dept', 'pk_group', 'creator',
            'creationtime', 'bill_date', 'pk_recbill', 'pk_oppaccount'];
        keyArr.forEach((val) => {
            let key = val;
            if (this.props.form.getFormItemsValue(this.formId, key)) {
                let value = this.props.form.getFormItemsValue(this.formId, key).value;//值
                let dly = this.props.form.getFormItemsValue(this.formId, key).display;//显示值
                if (value) {
                    ...
                    if (key == 'local_rate'||key == 'group_rate'||key == 'global_rate') {
                        ...
                        if (!isEdit) {
                            ...
                            this.props.cardTable.setEditableByIndex(this.tableId, rownum - 1, 'group_rate', true);//可以编辑
                            this.props.cardTable.setEditableByIndex(this.tableId, rownum - 1, 'global_rate', true);//可以编辑
                        }
 ...
 * @param {*} tableId 
 */
export const clsRowno = function (props, tableId) {
    let allTableData = props.cardTable.getAllRows(tableId);
    if (allTableData) {
        let index = 0;
        allTableData.forEach((val) => {
            console.log('index:', index);
            let rowno = (parseInt((index) * 10) + parseInt(10));
            val.values.rowno.value = String(rowno);
            index++;
        });
    }
}
