/*af9rYXJZK6Rs8ko3MReObjFamt36bMs0sg/PqmkWYt4oA36wSvLjVdkEY5W9+XKw*/
import { ajax, viewModel } from 'nc-lightapp-front';
import { setSourceFlag } from '../../util/setSourceFlag.js';//设置来源
import { formRateEditinfo, tableRateEditinfo } from "../events/judgeCurrtype.js";
...
export const linkLoadQueryData = function () {
            ...
            ajax({
                ...
                success: (res) => {
                    if (res.data) {
                        ...
                        if(this.props.getUrlParam('scene')&&this.props.getUrlParam('scene')=='bz'){
                            this.toggleShow();//我的报账-->正常显示按钮
                        }else{
                            this.linkToggleShow();//切换被联查页面状态<只显示附件联查打印>
                        }
                    } else {
                        //清空数据
    ...
    //报账中心-->点击菜单--->新增--->需要添加交易类型
     if (this.props.getUrlParam('status') === 'add') {
        ...
        //处理session数据
        //缓存数据
        let tradetype = getGlobalStorage('sessionStorage', 'sessionTP');//交易类型编码
        let tradename = getGlobalStorage('sessionStorage', 'sessionName');//类型名称
        let tradepk = getGlobalStorage('sessionStorage', 'sessionpk');//类型pk
