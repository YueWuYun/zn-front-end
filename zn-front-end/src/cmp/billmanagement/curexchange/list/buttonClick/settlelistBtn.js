/*1798F34pL+hnpdYS/rai9LoHvhUpDLNqHo8bYNz4dTe2WdP8wCSU5sioVu95v0xB*/
 ...
 * @param {*} props 
 */
export const settlelistBtn = function () {
    let settleData = this.props.table.getCheckedRows(this.tableId);
    //数据校验
    if (settleData.length == 0) {
        toast({
            ...
            content: this.props.MutiInit.getIntl("36070FCE") && this.props.MutiInit.getIntl("36070FCE").get('36070FCE-000051')   // 提示内容,非必输/* 国际化处理： 请选择数据,进行结算!*/
        })
        return
    }
    let indexArr4 = [];
    let dataArr4 = [];
    let settlelistTsmap = [];//ts的list类型
    //处理选择数据
    settleData.forEach((val) => {
        ...
        settlelistTsmap.push(settletsmap);
    });
    //自定义请求数据
    let data4 = {
        'pks': dataArr4,
        'listTsmap': settlelistTsmap,
        'pageid': this.pageCode
    };

    ajax({
        url: Templatedata.buttonclick_settle,
        data: data4,
        success: (res) => {
