...
class List extends Component {
				...
				success: (res) => {
					...
					if (success) {
						if (data && data.model.rows.length > 0) {
							...
							let message =
								this.state.json['36070AB-000046'] +
								data.model.allpks.length +
								this.state.json['36070AB-000049'];
							toast({
							...
							toast({
								color: 'warning',
								content: this.state.json['36070AB-000047'] //{/* 国际化处理： 未查询出符合条件的数据！*/}
							});
	...
	render() {
		...
		// let ISLINK = this.props.getUrlParam('scene');
		let ISLINK = getDefData(constant.fipscene_key, this.cacheDataSource);
			...
			<div className="nc-bill-list">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
					<div className="header-title-search-area">
						{/* {createPageIcon()}
						<h2 className="title-search-detail">{this.state.json['36070AB-000019']}</h2>国际化处理： 账户汇兑损益单 */}
						{createBillHeadInfo({
