...
import { toast, ajax, cacheTools, deepClone } from 'nc-lightapp-front';
...
export const BatchDelLine = function (props, areacode) {
    ...
    if (selectRows == null || selectRows.length == 0) {
        toast({
            'color': 'warning',
            'content': this.state.json['36070-000023']//'未选中要删除的行'
        });
        return false;
    }
    let selectIndexs = [];
...
export const BatchCopy = function (props, areacode, index) {
    ...
    if (selectRows == null || selectRows.length == 0) {
        toast({
            'color': 'warning',
            'content': this.state.json['36070-000024']//'未选中要复制的行'
        });
        return false;
    }
    let selectIndexs = [];
    let selectRowCopy = JSON.parse(JSON.stringify(selectRows));
    //deepClone(selectRows);
...
export const listMultiOperator = function (props, pageCode, tableCode, pkName, url, successMess, refreshFunc) {
    ...
    if (selectDatas == null || selectDatas.length == 0) {
        toast({
            color: 'warning', content: this.state.json['36070-000025']//'未选中行！' 
        });
        return;
    }
    let pkMapTs = {};
