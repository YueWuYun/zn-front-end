...
export function state4headBtns() {
  ...
  return [
    ...
    {
      key: "Commit",
      visible: (isNoState || isNoPass) && isPayFor, // 已承付, 未提交 
      primary: isNoState, // 未提交 主要按钮
    },
    {
      key: "Uncommit", // 收回 审批状态为未通过、通过、提交 单据状态为审批中、未执行 
      visible:
        (isNoPass || isPassing || isCommit) && !isVoucher,
    },
    {
      key: "Save",
      visible: isAdd || isEdit , // 新增和编辑态
      primary: isAdd || isEdit 
    },
    {
      key: "NoteRegister",//发票登记
      visible: !isInvoice , // 发票已到
    },
    {
      key: "Honour",//承付
      visible: isNoState && !isPayFor, // 自由+未承付
      primary: isNoState && !isPayFor, // 自由+未承付 主要按钮
    },
    {
    ...
    {
      key: "SaveAdd",
      // visible: (isAdd || isEdit) && !isPull // 新增和编辑态
      visible: false,
    },
    {
      key: "SaveCommit",
    ...
    },
    {
      ...
      visible: isBrowse && isPassing && !isRefuse, // 浏览态
    },
    {
    ...
    {
      key: "Voucher", // 联查凭证
      visible: isBrowse && isVoucher, // 浏览态
    },
    {
      key: "Print", // 打印
      visible: isBrowse, // 浏览态
    ...
    {
      key: "LinkOperate", // 关联操作
      visible: isBrowse && isPassing, // 浏览态
    },
    {
      key: "PushPayRegister", // 付款登记
      visible: isBrowse && isPassing && !isRefuse && !isPay, // 浏览态
    },
    {
      key: "PushDocumentaryApply", // 押汇申请
      visible: isBrowse && isPassing && !isRefuse && !isdocubills, // 浏览态
    },
    {
...
export function state4tableBtns() {
  let { isEdit, isAdd,isPayFor} = btnRule.call(
    this
  );
  return [
    {
      key: "AddRow", // 增行
      visible: (isAdd || isEdit) && !isPayFor ,
...
function btnRule() {
  ...
  // 编辑或复制态
  let isEdit = (status === "edit" || status === "honour") && id;
  // 新增态
  ...
  let commitdate = formUtil.getFormItemsValue(
    FormConfig.formId,
    "commitdate"
  );
  // let isPayFor = commitdate.value!==null;
  let isPayFor = status === "honour" || commitdate.value!==null;
  ...
  // 是否已被下游付款单拉单
  let isPay = formUtil.getFormItemsValue(
    FormConfig.formId,
    "ispay"
  );
  isPay = isPay && isPay.value  ;
  // 是否已被下游押汇单拉单
  let isdocubills = formUtil.getFormItemsValue(
    FormConfig.formId,
    "isdocubills"
  );
  isdocubills = isdocubills && isdocubills.value  ;
