...
class List extends Component {
  ...
  multiLangCallback = (multiLang, status, inlt) => {
    if (status) {
      ...
      console.log(
        this.state.multiLang["361701OQ-000006"]
      ); /* 国际化处理： 未加载到本页面多语资源 */
    }
  };
  render() {
    let { multiLang } = this.state;
    let pageTitle = multiLang["361701OQ-000000"]; /* 国际化处理： 开证查询 */
    let tabs = [
      {
        key: "nocommit",
        num: "(0)",
        status: "-1,0",
        content: multiLang["361701OQ-000001"] /* 国际化处理： 待提交*/,
      },
      {
        key: "approving",
        num: "(0)",
        status: "2,3",
        content: multiLang["361701OQ-000002"] /* 国际化处理： 审批中*/,
      },
      {
        key: "all",
        num: "",
        status: "5",
        content: multiLang["361701OQ-000005"] /* 国际化处理： 全部*/,
      },
    ]; // tab页签
    return (
      <BaseList
