...
export function bodyButtonClick(props, key, text, record, index) {
  let { TableConfig, cardTable: cardTableUtil, FormConfig, form: formUtil} = this.props;
  // 当前页签
  ...
  switch (key) {
    ...
    // 行内 插行
    case "InsertRow":
      if (curTabKey === "contractinfo") {
        // // xx子表
        // cardTableUtil.addRow(TableConfig.tableCode, tempIndex);
         // 信用证币种、货物分类、货品（劳务）
         cardTableUtil.addRow(TableConfig.tableCode, tempIndex, {
          pk_contcurrtype: formUtil.getFormItemsValue(FormConfig.formId, "pk_lccurrtype"), // 信用证币种
          pk_marbasclass: formUtil.getFormItemsValue(FormConfig.formId, "pk_marbasclass"), // 货物分类
          pk_material: formUtil.getFormItemsValue(FormConfig.formId, "pk_material") // 货品/劳务
        })
        card.cardTableDeleteRowTriggerAfterEvent.call(this,"pk_material");
      }
      break;
       //肩部 复制行
       case "CopyRow":
