...
class Card extends Component {
  ...
  multiLangCallback(multiLang, status, inlt) {
    if (status) {
      ...
      console.log(
        this.state.multiLang["361703DBP-000006"]
      ); /* 国际化处理： 未加载到本页面多语资源 */
    }
  }
  ...
  handleExtension() {
    ...
    this.props.form.setFormItemsValue(FormConfig.formId, {
      versionres: { value: "EXTEND", },
    });
    // 切换到 展期信息 页签
    this.props.cardTable.setCurrTabKey("repayplan");

    // let extbegindate;
    // let unpaymny = formUtil.getFormItemsValue(FormConfig.formId, "unpaymny");
    // let olcunpaymny = formUtil.getFormItemsValue(
    //   FormConfig.formId,
    //   "olcunpaymny"
    // );

    // let tableValues = cardTableUtil.getTabData("repayplan").rows;
    // let len = (tableValues && tableValues.length) || 0;
    // if (len) {
    //   // 如果展期信息有值 下次展期开始日期为上次展期结束日期
    //   extbegindate = tableValues[len - 1].values.extensionenddate;
    // } else {
    //   // 如果是第一次进行展期 展期开始日期为放款结束日期
    //   extbegindate = formUtil.getFormItemsValue(
    //     FormConfig.formId,
    //     "contenddate"
    //   );
    // }
    ...
    // 新增一行 带出默认值
    // cardTableUtil.addRow(TableConfig.tableCode, len, {
    //   extensionmny: unpaymny, // 展期金额
    //   olcextensionmny: olcunpaymny, //  展期本币金额
    //   extbegindate, // 展期开始日期
    // });
  }
  ...
  render() {
    ...
    let pageTitle = multiLang["361703DBP-000000"]; /* 国际化处理： 押汇放款 */
    ...
    return (
      ...
      //  _beforeEvent={beforeEvent} // 主表编辑前函数
        ...
        _handleExtension={this.handleExtension} // 变更时主子表事件处理函数
        {...this.props}
      />
    );
  }
