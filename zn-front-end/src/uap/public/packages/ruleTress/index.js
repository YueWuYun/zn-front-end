...
import "./index.less";
const { NCButton, NCTree, NCSelect, NCRow, NCCol, NCDiv } = base;
const {
  getNodeByKey,
  delTreeNodeByKey,
  validateSubmitInfo,
...
class RuleTree extends Component {
  ...
  componentDidMount() {
    console.log("roleTreeJson",this.props.roleTreeJson)
    const treeInitData =
      this.props.roleTreeJson &&
      !Utils.isEmptyObj(this.props.roleTreeJson.querycondition)
        ? deepClone(this.props.roleTreeJson.querycondition)
        : {};
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      !Utils.isEmptyObj(nextProps.transTreeNodeInfo) &&
      nextProps.transTreeNodeInfo.nodeTitleId !=
        this.props.transTreeNodeInfo.nodeTitleId
    ) {
      ...
      const transmitObj = {
        extraParaVO: {
          ...
          functions: nextProps.transTreeNodeInfo.functions || []
        },
  ...
  getDragDropObj(key) {
    ...
    const originalTreeData = [...this.state.treeData];
    let obj = {};
    if (Utils.checkNumber(key)) {
      //全部由数字组成意味着拖拽/目标节点是选择性父节点
  ...
  moveTreeNode(dragKey, dropKey) {
    ...
    if(dragObj.extraParaVO.isApproveResult || dropObj.extraParaVO.isApproveResult) return ;
    //删除被拖拽节点
    this.delNode(originalTreeData, dragKey);

    //数据新增合成
    if (Utils.checkNumber(dropKey)) {
      //目标节点是选择性父节点
  ...
  render() {
    ...
    const loop = data =>
      data.map((item, index) => {
        if (
          ...
          !Utils.isEmptyArr(item.conditions)
        ) {
          if (item.logic && item.conditions.length) {
            parKey += 1;
            return (
              <NCTreeNode
                liAttr={{fieldid: item.extraParaVO.nodeTitleId || item.extraParaVO.refpk}}
                titleClass={"refernode"}
                key={parKey}
                title={
    ...
    if(this.state.treeData.length > 0 &&
      this.state.treeData[0].conditions.length != 0) {
        treeInfo = 
        <NCDiv fieldid="rule" areaCode={NCDiv.config.TreeCom}>
          <NCTree
            className="myCls"
            showLine
            draggable
    ...
    return (
      <div>{/**fieldid="rule_tree-contain" */}
        {treeInfo}
        <NCDiv areaCode={NCDiv.config.BOTTOM}>
          <div className="rule-tree-btn-group">
            <NCButton className="pull-right" fieldid="ensure" onClick={this.cancelInfo.bind(this)}>
              {this.state.json["1880000025-000002"]}
            </NCButton>
            <NCButton fieldid="cancel"
              className="pull-right"
              colors="primary"
              onClick={this.ensureInfo.bind(this)}
            >
              {this.state.json["1880000025-000003"]}
            </NCButton>
          </div>
        </NCDiv>
