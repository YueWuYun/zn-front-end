...
const generateDisplayZh = (oprtype, json) => {
  const displayArr = [
    { display: json["1880000025-000006"], value: ">" },
    { display: json["1880000025-000007"], value: "<" },
    { display: json["1880000025-000008"], value: "><" },
    { display: json["1880000025-000009"], value: "<>" },
    { display: json["1880000025-000010"], value: ">=" },
    { display: json["1880000025-000011"], value: "<=" },
    { display: json["1880000025-000013"], value: "like" },
    { display: json["1880000025-000014"], value: "left like" },
    { display: json["1880000025-000015"], value: "right like" }
  ];
  let msg = json["1880000025-000012"];
  displayArr.map(item => {
    if (oprtype == item.value) {
      msg = item.display;
    }
  });
  return msg;
...
const getNodeByKey = (tree, key, node) => {
  if (!node) {
    tree &&
      tree.filter((item, index) => {
        if (
          item.hasOwnProperty("extraParaVO") &&
          item.extraParaVO.nodeTitleId === key
        ) {
          node = item;
 ...
 * 生成table数据显示用display
 * **/
const generateDisplayTreeInfo = (data, json, needRefcode=false) => {
  let transDisplay = "";
  let transLogic = "and";
        ...
        if (item.hasOwnProperty("conditions") && item.conditions.length >= 1) {
          ...
          if (item.value.firstvalue !== "") {
            if (item.value.secondvalue) {
              transDisplay += `${item.extraParaVO.refname} ${
                json["1880000025-000008"]
              } ${item.value.firstvalue} ~ ${item.value.secondvalue} ${
                index < arr.length - 1
                  ? generateZhCondition(transLogic, json) + " "
                  : ""
              }`;
            } else {
              transDisplay += `${item.extraParaVO.refname} ${generateDisplayZh(
                item.oprtype,
                json
              )} ${item.display.includes(",") ? "(" : ""} ${needRefcode && item.extraParaVO.itemtype === "refer" && item.refVO.refpk ? item.refVO.refCode : item.display} ${
  ...
  if(data && !data[0].conditions.length) {//xuxwb处删除所有数据
    transDisplay = "";
  }else {
    displayLoop(data);
  }
...
const findEmptyNode = data => {
  let emptyObj;
  if(data && !data[0].conditions.length) return false;//xuxwb处删除所有数据
