...
export const refreshTip = (param = {leavTip: "", refreshTip: ""}) => {
  window.addEventListener("beforeunload", function (e) {
    var  n  =  window.event.screenX  -  window.screenLeft; 
      var  b  =  n  >  document.documentElement.scrollWidth-20; 
    if(b  &&  window.event.clientY  <  0  ||  window.event.altKey) {//关闭
      //不是所有浏览器都支持提示信息的修改
      var confirmationMessage = param.leavTip || "系统可能不会保存您所做的更改";
      e.returnValue = confirmationMessage;
      return confirmationMessage;
    }else {
      //不是所有浏览器都支持提示信息的修改
      var confirmationMessage = param.refreshTip || "系统可能不会保存您所做的更改";
      e.returnValue = confirmationMessage;
      return confirmationMessage;
    }
  });
