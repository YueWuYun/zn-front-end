...
const getSessionId = (newProps, queryUrl, queryData, searchTemplate, json) => {
  ajax({
    ...
    success: res => {
      ...
      if (
        ncclouPrintSetting &&
        ncclouPrintSetting.printTemplateID &&
        ncclouPrintSetting.nopreview
      ) {
        printClient(newProps, queryUrl, {
          ...transData,
          printTemplateID: searchTemplate
            ? ncclouPrintSetting.printTemplateID
            : queryData.printTemplateID
        }, json);
      } else {
...
const previewPrint = (newProps, queryUrl, queryData, searchTemplate, json) => {
  printPreview(newProps, queryUrl, queryData, searchTemplate, info => {
  ...
  if (websocket != null && websocket.readyState == 1) {
    ...
    if ("WebSocket" in window) {
      ...
      let ws = new WebSocket(
        `ws://${queryData.socketIp || "127.0.0.1"}:9999/websocket`
      );
      ...
      toast({ color: "warning", content: "不支持websocket" });
    }
  }
};
