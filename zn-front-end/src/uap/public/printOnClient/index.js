...
const getSessionId = (newProps, queryUrl, queryData, searchTemplate, json) => {
  ajax({
    ...
    data: {
      ...
      num: queryData.type == 3 ? queryData.oids.length.toString() : "1"
    },
    success: res => {
      ...
      if(ncclouPrintSetting && ncclouPrintSetting.printTemplateID) {
        printClient(newProps, queryUrl, {
          ...transData,
          printTemplateID: ncclouPrintSetting.printTemplateID
        }, json)
      }else if(searchTemplate) {
        queryTemplateAndPrint(newProps, queryUrl, transData, json);
      }else {
        printClient(newProps, queryUrl, transData, json);
      } 
    },
    error: res => {
...
//模板查询并打印
const queryTemplateAndPrint = (newProps, queryUrl, queryData, json) => {
      ...
      let timer = setInterval(function() {
        if (ws.readyState == WebSocket.OPEN) {
          ...
          data = Object.assign({}, queryData, {
            ...
            download: queryData.download || null,
            suffx: queryData.suffx || null,
            beginUrl,
            aeskey,
            rockin,
          });
