...
export default function (props, key) {
    ...
    switch (key) {
        ...
        //联查
        case 'link':
            let pk_srcbill = this.props.form.getFormItemsValue(formId, 'pk_sourcebillid').value;
            let sbExtParam = {
                status: 'browse',
            ...
            elecSignCardPrint(props, {
                ...
                getOrgFunc: () => {
                    let pk_org_p = props.form.getFormItemsValue(formId, 'pk_org_p') && props.form.getFormItemsValue(formId, 'pk_org_p').value;
                    if (pk_org_p) {
                        return pk_org_p;
                    }
                    else {
