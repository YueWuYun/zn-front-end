...
export const buttonVisible = function (that) {
    ...
    if (status == 'edit') {//编辑态
        ...
        //转账成功且审批通过
            ...
            //已制证
            if (ismakevoucher) {
                that.props.button.setButtonVisible(['Add', 'Copy', 'CancelVoucher', 'Linkquerygroup', 'AttachManage', 'Print', 'Refresh'], true);
                that.props.button.setButtonVisible(['ViewApprove', 'SendBill', 'AccountBalance', 'QueryVoucher', 'NtbPlan'], true);
            }
            //未制证
            else {
                that.props.button.setButtonVisible(['Add', 'Copy', 'UnCommit', 'Voucher', 'Linkquerygroup', 'AttachManage', 'Print', 'Refresh'], true);
