...
export default function beforeEvent(props, moduleId, key, value, i, record, status) {
	...
	if(key == ("olcrate"||"glcrate"||"gllcrate")){
		...
		ajax({
			url: base_url+'spegathercardbeforeedit.do',
			data: olcratedata,
			async: false,
			success: (res) => {
				if (res) {					
					props.cardTable.setEditableByIndex(card_table_id, i, 'olcrate', !res.data);
					props.cardTable.setEditableByIndex(card_table_id, i, 'glcrate', !res.data);
					props.cardTable.setEditableByIndex(card_table_id, i, 'gllcrate', !res.data);
				}
			}
		});
	}
