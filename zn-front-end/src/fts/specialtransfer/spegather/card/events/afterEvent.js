...
import { buildLightBodyAfterEditData ,bodyRateEditOnAfterEdit} from '../../../../../tmpub/pub/util/index.js';
...
export default function afterEvent(props, moduleId, key, value, changedrows, i, s, g, isAutoSet) {
	...
	switch (key) {
		...
		case 'pk_org':
			...
			//有变更，老数据和新数据的值不同，则进行提示
				...
				props.modal.show('changeOrg', {
					title: this.state.json['36300STG-000000'], // 弹框表头信息/* 国际化处理： 确认修改*/
					content: this.state.json['36300STG-000001'], //弹框内容，可以是字符串或dom/* 国际化处理： 是否修改组织，这样会清空您录入的信息?*/
					userControl: false, // 点确定按钮后，是否自动关闭弹出框.true:手动关。false:自动关
					noFooter: false, //是否需要底部按钮,默认true
					rightBtnName: this.state.json['36300STG-000002'], //左侧按钮名称,默认关闭/* 国际化处理： 取消*/
					leftBtnName: this.state.json['36300STG-000003'], //右侧按钮名称， 默认确认/* 国际化处理： 确认*/
		...
		case 'pk_payfinanceorg':
			...
			if (!newvalue || !newvalue.value) {
				props.cardTable.setValByKeyAndIndex(card_table_id, i, 'pk_payfinanceorg', { value: null, display: null });
				props.cardTable.setValByKeyAndIndex(card_table_id, i, 'pk_accid', { value: null, display: null });
			}
			//有变更 发起请求进行编辑后事件处理
			else if (!oldvalue || !oldvalue.value || newvalue.value != oldvalue.value) {
				handleSpegatherBodyAfterEdit(props, moduleId, key, value, changedrows, i, eventData);
			}
			break;
		//表体内部账户
		case 'pk_accid':
 ...
 * @param {*} callback 
 */
const handleSpegatherBodyAfterEdit = function (props, moduleId, key, value, changedrows, i, eventData, callback) {
	let status = props.getUrlParam('status');
	let extParam = { 'uiState': status };
	ajax({
		url: base_url + 'spegathercardafteredit.do',
		async: false,
		data: { 'eventPosition': 'lightBody', 'eventType': 'card', 'eventData': JSON.stringify(eventData) },
		success: (res) => {
			let { success, data } = res;
			if (success) {
				let { grid, extParam, headProp, bodyProps} = data;
				// let {head } = grid
				//更新表单数据
