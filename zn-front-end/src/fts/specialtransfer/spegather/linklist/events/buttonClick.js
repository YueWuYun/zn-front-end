...
import { base_url, list_table_id, list_page_id, billtype, print_nodekey, print_templateid, funcode, viewmod_deal, sourceModel, card_page_url, list_page_url, pkName, dataSource, linklist_page_id } from '../../cons/constant.js';
...
export default function buttonClick(props, key, text, record, index) {
	...
	switch (key) {
		...
		//头部 提交
		case 'BatchCommit':
			listMultiOperator(props, linklist_page_id, list_table_id, pkName, base_url + 'spegathercommit.do', this.state.json['36300STG-000007'], dataSource, false, null, (props, data) => {/* 国际化处理： 提交*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
		...
		//头部 收回
		case 'BatchUncommit':
			listMultiOperator(props, list_linklist_page_idpage_id, list_table_id, pkName, base_url + 'spegatheruncommit.do', this.state.json['36300STG-000008'], dataSource);/* 国际化处理： 收回*/
			break;
		//头部 打印
		case 'Print':
		...
		//表体 提交
		case 'Commit':
			listSingleOperator(props, linklist_page_id, list_table_id, base_url + 'spegathercommit.do', record, pkName, index, this.state.json['36300STG-000007'], dataSource, true, null, (props, data) => {/* 国际化处理： 提交*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
				if (workflow && workflow == 'approveflow' || workflow == 'workflow') {
					this.setState({
		...
		//表体 收回
		case 'Uncommit':
			listSingleOperator(props, linklist_page_id, list_table_id, base_url + 'spegatheruncommit.do', record, pkName, index, this.state.json['36300STG-000008'], dataSource);/* 国际化处理： 收回*/
			break;
		//表体 制证
		case 'Premit':
			listSingleOperator(props, linklist_page_id, list_table_id, base_url + 'spegathervoucher.do', record, pkName, index, this.state.json['36300STG-000009'], dataSource);/* 国际化处理： 制证*/
			break;
		//表体 取消制证
		case 'UnPremit':
			listSingleOperator(props, linklist_page_id, list_table_id, base_url + 'spegathercancelvoucher.do', record, pkName, index, this.state.json['36300STG-000010'], dataSource);/* 国际化处理： 取消制证*/
			break;
		//表体 经办
		case 'Decide':
			props.pushTo("/card", {
...
const delConfirm = function (props) {
	listMultiOperator(props, linklist_page_id, list_table_id, pkName, base_url + 'spegatherdelete.do', this.state.json['36300STG-000005'], dataSource, true);/* 国际化处理： 删除*/
}
...
export const listReBack = function (props, record, index, reason) {
	...
	let extParam = { reason };
	listSingleOperator(props, linklist_page_id, list_table_id, base_url + 'spegatherback.do', record, 'pk_spegather_h', index, this.state.json['36300STG-000020'], dataSource, false, extParam);/* 国际化处理： 退回*/
