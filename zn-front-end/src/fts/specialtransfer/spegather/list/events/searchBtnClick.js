...
import { ajax, cardCache, toast, deepClone, cacheTools } from 'nc-lightapp-front';
import {
    base_url, oid, hasQuery, list_page_id, list_search_id, list_table_id, group_needcommit, group_approving, group_all, searchArea,
    dataSource, islink, linkdata, card_from_id, card_page_id, pkName,islinkone
} from '../../cons/constant';
...
export const searchBtnClick = function clickSearchBtn(props, qryCondition, groupKey) {
    ...
    if(!qryCondition || !qryCondition.conditions || qryCondition.conditions.length == 0){
        qryCondition = getDefData(searchArea, dataSource)  
    }
    //若查询区域查询条件有值则表明是查询按钮逻辑，故将查询条件存入缓存
    else {        
        //查询区域查询条件存入缓存
        setDefData(searchArea, dataSource, qryCondition);
        setDefData(hasQuery, dataSource, 'true');
    }
    if (!qryCondition || !qryCondition.conditions || qryCondition.conditions.length == 0) {
        return;
    }
...
const getGroupCondition = function (groupKey) {
    let groupCondition;
    if(groupKey == 'simple'){
        groupKey = getDefData("selectedGroup", dataSource)
    }
    switch (groupKey) {
        //审批中
...
export const listInitData = function (props, type) {
    ...
    let extParam = {};
    //begin tm tangleic 20181030 修改列表数据初始化逻辑
    switch (type) {
        //构建预算反联查数据
        case 'tbb':
        ...
        //构建凭证反联查数据
        case 'voucher':
            let voucherParam = cacheTools.get('checkedData');
            if (!voucherParam || voucherParam.length == 0) { return; }
            extParam = { 'voucherParam': JSON.stringify(voucherParam), pageInfo };
            break;
    }
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (rows && rows.length == 1) {
                ...
                goTocard(props, { status: 'browse', id, from: 'list', scene: SCENE.LINK }, this.getState.bind(this));
            }
            //end tangleic
        }
    });
