	...
	props.createUIDom(
		...
		function (data) {
			if (data) {
				...
				if (props.getUrlParam('isCopy') || props.getUrlParam('status') != 'add') {
					that.qryData();
				}else {
					that.toggleShow();
				}
...
function modifierMeta(props, meta) {
	...
	meta[card_from_id].items.map((item) => {
		...
		//资金计划项目参照过滤
		else if (item.attrcode == 'pk_fundplanitem') {
			item.queryCondition = () => {
				return {
		...
		item.render = function (text, record, index) {
			return (
				AccidGridRef({
					queryCondition: () => {
						return {
							pk_getfinanceorg: (record.values.pk_getfinanceorg && record.values.pk_getfinanceorg.value) || 'empty',
