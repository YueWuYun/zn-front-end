...
import { base_url, list_table_id, list_page_id, billtype, print_nodekey, print_templateid, funcode, link_card_page_id, viewmod_deal, sourceModel, card_page_url, list_page_url, dataSource, pkName, link_list_page_id } from '../../cons/constant.js';
...
export default function buttonClick(props, key, text, record, index) {
	...
	switch (key) {
		...
		case 'Delete':
			promptBox({
				color: "warning",
				title:('1880000025-000005'),/* 国际化处理： 删除*/
		...
		//头部 提交
		case 'CommitList':
			listMultiOperator(props, link_list_page_id, list_table_id, 'pk_spepay_h', base_url + 'spepaycommit.do', loadMultiLang(this.props,'1880000025-000041'), dataSource, false, null, (props, data) => {/* 国际化处理： 批量提交成功！*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
		...
		//头部 收回
		case 'UnCommit':
			listMultiOperator(props, link_list_page_id, list_table_id, 'pk_spepay_h', base_url + 'spepayuncommit.do', loadMultiLang(this.props,'1880000025-000042'), dataSource);/* 国际化处理： 批量收回成功！*/
			break;
		//头部 附件管理
		case 'Attachment':
		...
		//表体 提交
		case 'BodyCommit':
			listSingleOperator(props, link_list_page_id, list_table_id, base_url + 'spepaycommit.do', record, 'pk_spepay_h', index, loadMultiLang(this.props,'1880000025-000007'), dataSource, true, null, (props, data) => {/* 国际化处理： 提交成功！*/
				let { workflow } = data;
				//有指派信息，则指派，没有则重绘界面
				if (workflow && workflow == 'approveflow' || workflow == 'workflow') {
					this.setState({
		...
		//表体 收回
		case 'BodyUnCommit':
			listSingleOperator(props, link_list_page_id, list_table_id, base_url + 'spepayuncommit.do', record, 'pk_spepay_h', index, loadMultiLang(this.props,'1880000025-000008'), dataSource);/* 国际化处理： 收回成功！*/
			break;
		//表体 制证
		case 'MakeVoucher':
			listSingleOperator(props, link_list_page_id, list_table_id, base_url + 'spepayvoucher.do', record, 'pk_spepay_h', index, loadMultiLang(this.props,'1880000025-000009'), dataSource);/* 国际化处理： 制证成功！*/
			break;
		//表体 取消制证
		case 'UnMakeVoucher':
			listSingleOperator(props, link_list_page_id, list_table_id, base_url + 'spepaycancelvoucher.do', record, 'pk_spepay_h', index, loadMultiLang(this.props,'1880000025-000010'), dataSource);/* 国际化处理： 取消制证成功！*/
			break;
		//表体 经办
		case 'Decide':
			props.pushTo("/card", {
...
const delConfirm = function (props) {
	listMultiOperator(props, link_list_page_id, list_table_id, 'pk_spepay_h', base_url + 'spepaydelete.do', loadMultiLang(this.props,'1880000025-000044'), dataSource, true);/* 国际化处理： 批量删除成功！*/
}
...
export const listReBack = function (props, record, index, reason) {
	...
	let extParam = { reason };
	listSingleOperator(props, link_list_page_id, list_table_id, base_url + 'spepayback.do', record, 'pk_spepay_h', index, loadMultiLang(this.props,'1880000025-000020'), dataSource, false, extParam);/* 国际化处理： 退回成功！*/
