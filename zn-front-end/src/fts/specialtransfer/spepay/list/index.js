...
//引入常量定义
import { module_id,module_name,module_tmpub_name,module_tmpub_id, base_url,funcode, button_limit, oid, list_page_id, list_search_id, list_table_id, group_needcommit, group_approving, group_all, billtype, dataSource, pkName } from '../cons/constant.js';
//引入退回弹框
...
import { goTocard, setListButtonUseful, autoLoadData } from '../util/spepayUtil';
...
class List extends Component {
	...
	onRowDoubleClick = (record, index, props, e) => {
		...
		ajax({
			...
			data: {pk,ts},
			success: (res) => {
				goTocard(this.props, { status: 'browse', id: record.pk_spepay_h.value, from: 'list',ts:record.ts.value }, this.getState.bind(this));
			}
		});
	...
	delConfirm = (record, index) => {
		...
		ajax({
			...
			data: data,
			success: () => {
				//tm zhangjp6 start【删除提示不受预算提示信息影响】
			...
			<div className="nc-bill-list">
				...
				{/* <div className="tab-definInfo-area"> */}
					{<NCTabs activeKey={selectedGroup} onChange={(v) => {
						...
						<NCTabPane key={group_needcommit} tab={
							<span>
								{loadMultiLang(this.props,'1880000025-000045')+ '('}
						...
						<NCTabPane key={group_approving} tab={
							<span>
								{loadMultiLang(this.props,'1880000025-000046')+ '('}
						...
						<NCTabPane key={group_all} tab={
							loadMultiLang(this.props,'1880000025-000047')/* 国际化处理： 全部*/
						} />
					</NCTabs>}
				{/* </div> */}
				{socket.connectMesg({
				...
				<div className="nc-bill-table-area">
					{createSimpleTable(list_table_id, {
						...
						onSelected: setListButtonUseful.bind(this,this.props),
						onSelectedAll: setListButtonUseful.bind(this,this.props),
						showCheck: true,
						showIndex: true,
				...
				<div>
					{assignShow && <ApprovalTrans
						...
						getResult={(value) => {
							...
							listSingleOperatorNoRecord(this.props, list_page_id, list_table_id, base_url + 'spepaycommit.do', billID, ts, index, loadMultiLang(this.props,'1880000025-000007'), dataSource, true, extParam);/* 国际化处理： 提交成功！*/
						}}
						cancel={() => {
							this.setState({ assignShow: false, assignData: null })
						}}
