...
import { base_url, card_from_id, card_table_id, billtype, print_nodekey, funcode, print_templateid, viewmod_deal, sourceModel, list_page_url, card_page_url, dataSource, pkName, card_table_pk, link_card_page_id } from '../../cons/constant';
...
import { saveMultiLangRes, loadMultiLang ,elecSignCardPrint} from "../../../../../tmpub/pub/util/index";
//引入公共api
...
export const buttonClick = function (props, key, text, record, index) {
  ...
  let billdata = props.createMasterChildData(link_card_page_id, card_from_id, card_table_id);
  let saveObj = {};
  ...
  switch (key) {
    ...
    //提交
    case 'Commit':
      cardOperator(props, link_card_page_id, card_from_id, bodyCodeMapBodyPKName, pkName, base_url + 'spepaycommit.do', loadMultiLang(this.props,'1880000025-000007'), dataSource, commitAssign.bind(this));/* 国际化处理： 提交成功！*/
      break;
    //收回
    case 'UnCommit':
      cardOperator(props, link_card_page_id, card_from_id, bodyCodeMapBodyPKName, pkName, base_url + 'spepayuncommit.do', loadMultiLang(this.props,'1880000025-000008'), dataSource, this.toggleShow.bind(this));/* 国际化处理： 收回成功！*/
      break;
    //复制
    case 'Copy':
    ...
    //头部 制证
    case 'Voucher':
      cardOperator(props, link_card_page_id, card_from_id, bodyCodeMapBodyPKName, pkName, base_url + 'spepayvoucher.do', loadMultiLang(this.props,'1880000025-000009'), dataSource, this.toggleShow.bind(this));/* 国际化处理： 制证成功！*/
      break;
    //头部 取消制证
    case 'CancelVoucher':
      cardOperator(props, link_card_page_id, card_from_id, bodyCodeMapBodyPKName, pkName, base_url + 'spepaycancelvoucher.do', loadMultiLang(this.props,'1880000025-000010'), dataSource, this.toggleShow.bind(this));/* 国际化处理： 取消制证成功！*/
      break;
    //头部 经办
    case 'Decide':
      props.pushTo("/card", {
    ...
    case 'PastLastLine':
      ...
      if (selectPast == null || selectPast.length == 0) {
        toast({
          'color': 'warning',
          'content': ('1880000025-000014')/* 国际化处理： 未选中要复制的行*/
        });
        return false;
      }
      //粘贴末行的位置index
...
const save = function (props, callback) {
  ...
  //获取主子表单据数据
  let billdata = this.props.createMasterChildData(link_card_page_id, card_from_id, card_table_id);
  let data = { data: JSON.stringify(billdata), pageCode: link_card_page_id };
  const that = this;
  if (flagForm && flagTable) {
    ajax({
      url: url,
...
const saveCommit = function (props, callback) {
  ...
  let billdata = this.props.createMasterChildData(link_card_page_id, card_from_id, card_table_id);
  let pageCode = link_card_page_id;
...
export const cardReBack = function (props, reason) {
  ...
  let extParam = { reason };
  cardOperator(props, link_card_page_id, card_from_id, bodyCodeMapBodyPKName, 'pk_spepay_h', base_url + 'spepayback.do', loadMultiLang(this.props,'1880000025-000020'), dataSource, pageChange.bind(this), false, extParam);/* 国际化处理： 退回成功！*/
  //关闭退回弹框
