...
export const buttonVisible = function (that) {
    ...
    if (status == 'edit') {//编辑态
        ...
        //联查场景
        that.props.button.setButtonVisible(['nulllinkquery','SourceBill','AccountBalance','NtbPlan','ViewApprove','SendBill','QueryVoucher'], true);
        if (islink && islink == SCENE.LINK||islink==SCENE.FIP) {
            ...
            if (billstatus == 5) {
                if (recmodul == sourceModel.ModuleCode_CMP) {
                    ...
                    that.props.button.setButtonVisible(['Add', 'Copy', 'Decide', 'Commit', 'Back', 'Linkquerygroup', 'AttachManage', 'Print', 'Refresh'], true);
                } else {
            ...
            //转账成功且审批通过
                ...
                if (ismakevoucher) {
                    ...
                    that.props.button.setButtonVisible(['ViewApprove', 'SendBill', 'AccountBalance', 'QueryVoucher', 'NtbPlan'], true);
                }
                //未制证
                else {
                    that.props.button.setButtonVisible(['Add', 'Copy', 'UnCommit', 'Voucher', 'Linkquerygroup', 'AttachManage', 'Print', 'elecsignformalPrint',
                    'elecsigninformalPrint','Refresh'], true);
                    that.props.button.setButtonVisible(['ViewApprove', 'SendBill', 'AccountBalance', 'NtbPlan'], true);
                    that.props.button.setButtonVisible(['QueryVoucher'], false);
                }
            }
        }
    }
    that.props.button.setButtonVisible(['Linkquerygroup','nulllinkquery','SourceBill','AccountBalance','NtbPlan','ViewApprove','SendBill','QueryVoucher'], true);
}
//卡片表体行级按钮数组
