...
import { cardReBack } from "../card/events/buttonClick";
...
import { list_page_id, module_id,module_name,module_tmpub_id,module_tmpub_name, base_url, button_limit, 
	link_card_page_id, card_from_id, card_table_id, viewmod_deal, billtype, funcode, print_nodekey, 
	print_templateid, list_page_url, card_page_url, pkName, dataSource, link_list_page_id ,saga_gtxid,cache} from '../cons/constant.js';
//引入附件组件
...
class Card extends Component {
	...
	qryData = () => {
		...
		if (isCopy) {
			...
			data = { pk: that.props.getUrlParam('id'), pageCode: link_card_page_id };
		}
		//经办
			...
			let pkMapTs = {};
			pkMapTs[pk] = ts;
			data = { pkMapTs, pageCode: link_card_page_id, isRet: true };
		}
		else {
			if(!this.refreshflag) {
				//异常交互
	...
	editData = () => {
		...
		let data = { pk: pk, pageCode: link_card_page_id };
		ajax({
				...
				<div>
					{assignShow && <ApprovalTrans
						...
						getResult={(value) => {
							...
							cardOperator(this.props, link_card_page_id, card_from_id, [], pkName, base_url + 'spepaycommit.do', loadMultiLang(this.props,'1880000025-000007'), dataSource, this.toggleShow.bind(this), false, extParam);/* 国际化处理： 提交成功！*/
						}}
						cancel={() => {
							this.setState({ assignShow: false, assignData: null })
						}}
