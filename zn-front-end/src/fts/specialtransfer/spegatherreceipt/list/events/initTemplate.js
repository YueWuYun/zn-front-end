		...
		function (data) {
			if (data) {
				if (data.template) {
					...
					let link = that.props.getUrlParam('scene') == 'linksce' ? true : false;
					meta = modifierMeta(that, props, meta)
					//加载模板数据
					props.meta.setMeta(meta);
					if (!link) {
						setDefOrg2AdvanceSrchArea(props, searchId, data, 'pk_org_r');
					}
					//加载模板数据
					props.meta.setMeta(meta);
					//查询赋默认组织	
					if (!link) {
						setDefOrg2ListSrchArea(props, searchId, data, 'pk_org_r');
					}
				}
				if (data.button) {
