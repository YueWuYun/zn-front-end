...
let { tableId, appcode, nodekey, formal_nodekey, supply_nodekey, Print_URL, linkkbilltype, Elecsign_Print_URL, vbillno, pkname, formId } = CONSTANTS;
        ...
        */
		case 'print':
			...
			if (printdata.length == 0) {
				...
				printdata.forEach((vale) => {
					pks.push(vale.data.values.pk_spegather_receipt_h.value);
				});
			}
			print(
		...
		case 'printout':
			...
			outputBtnData.forEach((item) => {
				oids.push(item.data.values.pk_spegather_receipt_h.value);
			});
			output({
				url: Print_URL,
				data: {
		...
		case 'link':
			...
			// break;
			let pk_srcbill =record.pk_sourcebillid && record.pk_sourcebillid.value;
            let sbExtParam = {
                status: 'browse',
                LinkBillType: linkkbilltype,
                id: pk_srcbill
