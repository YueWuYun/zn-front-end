...
export function searchBtnClick(
  ...
  if (!condition) {
    ...
    if (!condition) {
      toast({
        color: "warning",
        content: this.state.json["36620GBM-000046"]
      }); /* 国际化处理： 请选择必输的查询条件!*/
      return;
    }
  } else {
  ...
  let conditions = deepClone(condition);
  conditions.conditions.push(...groupCondition);
...
function listRender(res, isToast, isRefresh) {
  ...
  if (success && data && data.grid && data.grid[this.tableId]) {
    ...
    if (isToast) {
      let successMsg = isRefresh
        ? "刷新成功"
        : this.state.json["36620GBM-000047"]; /* 国际化处理： 查询成功!*/
      if (flag) {
        toast({
          color: "success",
          content: successMsg
        ...
        toast({
          color: "warning",
          content: this.state.json["36620GBM-000048"]
        }); /* 国际化处理： 未查询出符合条件的数据!*/
      }
    }
  } else {
