...
import { list_table_id, list_page_id, table_oid, del, checkRef, save, oprName } from '../../cons/constant.js';
import { bodyBtnOperation } from './bodyButtonClick';
import { multiToast, trimStr, searchButtonClick } from '../../../public/events'
import { listButtonVisible } from './public.js';
export default function buttonClick(props, id, grid) {
    switch (id) {
        //头部 新增
        case 'Add':
        ...
        //头部 修改
        case 'Edit':
            props.editTable.setStatus(list_table_id, 'edit');
...
function addBill(props) {
    let number = props.editTable.getNumberOfRows(list_table_id);
    props.editTable.addRow(list_table_id, number);
    listButtonVisible.call(this, props);
    // 将焦点放到新增行的code输入框上
    props.editTable.setValByKeyAndIndex(list_table_id, number, 'code', {value: '', display:'', scale:0, isEdit: true});
}
...
function oprFlag(props) {
    ...
    if( length === 0 && this.state.editDelData.model.rows.length === 0 ) { // 编辑态没有新增数据以及删除的数据时
...
function regRequired(props, saveData) {
    // 名称和编码去掉前后空格
    saveData.map( e => {
        e.values.variety_name.value = trimStr(e.values.variety_name.value);
...
function saveBill(props) {
    let saveData = props.editTable.getChangedRows(list_table_id,false);
