...
export function bodyButtonClick(props, key, record, index) {
  ...
  // 操作前事件
  let handelSendData =
    this.props._beforeOpr && this.props._beforeOpr.call(this, sendData);
  switch (key) {
    case "DelLine": //删除
    ...
    case "StartLine": //启用
      ...
      bodyBtnOperation.call(
        this,
        handelSendData || sendData,
        this.enableUrl,
        this.state.json["36010PUBLIC-000008"],
        false,
        index
      ); /* 国际化处理： 启用成功!*/
      break;
    case "StopLine": //停用
      ...
      bodyBtnOperation.call(
        ...
        handelSendData || sendData,
        this.disEnableUrl,
        this.state.json["36010PUBLIC-000009"],
        false,
        index
      ); /* 国际化处理： 停用成功!*/
      break;
      ...
      success: res => {
        if (res.success) {
          if (res.data && res.data.failNum && res.data.failNum !== "0") {
            ...
            props.editTable.deleteTableRowsByRowId(
              this.tableId,
              record.rowid,
              true
            );
