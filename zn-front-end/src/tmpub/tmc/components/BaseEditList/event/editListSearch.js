 ...
 * @param {*} props          页面内置对象
 */
export function searchButtonClick (props) {
    // 分页信息
    ...
    queryAjax.call(this, this.queryInterface, searchdata).then(data => {
        let afterGrid = this.props._afterSearch && this.props._afterSearch.call(this, data.grid);
        this.props.editTable.setTableData(this.tableId, (afterGrid && afterGrid[this.tableId]) || data.grid[this.tableId]);
        ...
        ajax({
            ...
            success: (res) => {
                ...
                if (success && data && data.grid) {
                    ...
                    // 第一次进入页面不显示toast
                    if( this.state.showToast ) { // 控制toast是否显示
...
export function handelResData(props, data) {
    // 预置数据不可操作
    let preSysRows = data.grid[this.tableId].rows.map(e => e.values[this.sysMark].value);
    let sysIndex = [];
