...
const tallyConfirm = function (props, record, index) {
	...
	ajax({
		...
		success: (res) => {
			// toast({ color: 'success', content: loadMultiLang(props, '36140FDLB--000064') });/* 国际化处理： 删除成功*/
			toast({ color: 'success', content: this.state.json['36140FDLB-000064'] });/* 国际化处理： 删除成功*/
			let updateDataArr = [{
				index: index,
...
const unTallyConfirm = function (props, record, index) {
	...
	ajax({
		...
		success: (res) => {
			// toast({ color: 'success', content: loadMultiLang(props, '36140FDLB--000065') });/* 国际化处理： 删除成功*/
			toast({ color: 'success', content: this.state.json['36140FDLB-000065'] });/* 国际化处理： 删除成功*/
			let updateDataArr = [{
				index: index,
				data: { values: res.data.head[card_from_id].rows[0].values }
				}];
