...
                	};
					...
					return {
						...
						GridRefActionExt: 'nccloud.web.fac.depositreceipt.filter.NCCAccountFilter'//自定义参照过滤条件
	                };
				}
                
            };
...
        }
	...
	if(props.getUrlParam('scene') != 'linksce' && props.getUrlParam('scene') != 'fip'){
		meta[list_table_id].items.push({
			attrcode: 'opr',
			label: that.state.json['36140FDLB-000047'],
			//label: props.MutiInit.getIntl("36140FDLB") && props.MutiInit.getIntl("36140FDLB").get('36140FDLB-000047'),/* 国际化处理： 操作*/
			width: 200,
			fixed: 'right',
			className:"table-opr",
			itemtype: 'customer',
