...
export default function buttonClick(props, id) {
    switch (id) {
        ...
        //联查台账   /nccloud/resources/ifac/facreportquery/bankregularaccountquery/main/index.html#/list/index.html
        case 'Account':
            ...
            if (link_accountData_regular.length != 1) {
                // toast({
                //     color: 'warning',
                //     content: this.state.json['36140FDLB-000039']/* 国际化处理： 请选择1条数据*/
                // });
                // return;
            }
...
const tallyHeadConfirm = function (props) {
    listMultiOperator(props, list_page_id, list_table_id, pk_name, requesturl.tally,this.state.json['36140FDLB-000064'], dataSourceTam);/* 国际化处理：记账成功*/
};

//解冻
const unTallyHeadConfirm = function (props) {
	listMultiOperator(props, list_page_id, list_table_id, pk_name, requesturl.untally, this.state.json['36140FDLB-000065'], dataSourceTam);/* 国际化处理： 取消记账成功*/
};
...
const printAll = function (props) {
	...
	if (selectDatas == null || selectDatas.length == 0) {
		toast({ color: 'warning', content: this.state.json['36140FDLB-000039'] });/* 国际化处理： 请选中一行数据！*/
		return;
	}
	let index = 0;
	let oids = [];
...
const refreshHtml = function (props) {
    ...
    if(!refreshsearchVal){
        toast({
            duration: 3,
            title: that.state.json['36140FDLB-000053'],/* 国际化处理： 刷新成功！*/
            color: 'success'
        })
...
const fileMgr = function (props) {
	...
	if (selectDatas == null || selectDatas.length == 0 || selectDatas.length > 1) {
		// toast({ color: 'warning', content: that.state.json['36140FDLB-000039']});/* 国际化处理： 请选中一行数据！,请选中一行数据*/
		// return;
	}
