...
    }
	...
	refreshcard = () => {
		...
		if (this.props.getUrlParam('status') == 'browse') {
			...
			if(!pk){
				...
				toast({	
					content: this.state.json['36340CDIR-000053'],/* 国际化处理： 刷新成功！*/
					color: 'success'
				})
			}else{
	...
	                      	processFormulamsg(this.props, res);
							if (res.data.head) {
								...
								if(res.data.head[card_from_id].rows[0].values.pk_intobj.display){
									this.props.form.setFormItemsValue(card_from_id,{
										'pk_intobj.objcode':{value:res.data.head[card_from_id].rows[0].values.pk_intobj.display,display:res.data.head[card_from_id].rows[0].values.pk_intobj.display}
									});
								}else{
									this.props.form.setFormItemsValue(card_from_id,{
										'pk_intobj.objcode':null
									});
								}
					...
					ajax({
						...
						success: (res) => {
							if (res.data) {
								...
								//处理公式
								processFormulamsg(this.props, res);
								// addCache(pk_intlist,res.data,this.formId,dataSourceTam);
								if(this.props.getUrlParam('scene') == 'linksce') {
									// addCache(pk_intlist,res.data,card_from_id,dataSourceTam,res.data.head[card_from_id].rows[0].values);
									if(getDefData(pk_intlist, dataSourceTam)){
										updateCache(card_table_id, id, card_from_id, dataSourceTam, res.data.head[card_from_id].rows[0].values);
									}else{
										// addCache(pk_intlist,res.data,card_from_id,dataSourceTam);
										setDefData(pk_intlist, dataSourceTam, res.data);
									}
				...
				success: (res) => {
					if (res.data) {
						if (res.data.head) {
							...
							this.props.form.setFormItemsDisabled(this.formId,{'pk_gaincenter':false}); //利润中心
							this.props.form.setFormItemsDisabled(this.formId,{'pk_capitalcenter':false}); //成本中心
							let vbillno = res.data.head[this.formId].rows[0].values.vbillno.value;
...
                      	processFormulamsg(this.props, res);				
	...
	beSureOrgBtnClick = () => {
		...
		if (!pk_org) {
			...
			// this.props.form.setFormItemsValue(card_from_id,{ 'apply_type': { value:'1',display:this.state.json['36340CDIR-000021']} }); /* 国际化处理： 销户*/
			// this.props.form.setFormItemsValue(card_from_id,{ 'vbillstatus': { value:'-1',display:this.state.json['36340CDIR-000022']} });/* 国际化处理： 自由*/
	...
	//卡片返回按钮
	handleClick = () => {
		let scene = (this.props.getUrlParam("scene")==='fip'||this.props.getUrlParam("scene")==='linksce')?'linksce':null;
		let islinkquery=(this.props.getUrlParam("scene")==='fip'||this.props.getUrlParam("scene")==='linksce')?true:null;
			...
			<div className="nc-bill-card">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
						...
						<div className="header-title-search-area">
							{createBillHeadInfo({
								title: this.state.json['36340CDIR-000034'], //标题/* 国际化处理： 内部账户计息设置*/
				...
				>
					<NCModal.Header closeButton >
						<NCModal.Title>{this.state.json['36340CDIR-000034']}</NCModal.Title>
					</NCModal.Header>
