...
export default function (props, pks) {
    ...
    if(cardData){
        ...
        this.setState({ billno: vbillno,vbillno: vbillno });
        ...
        ajax({
            ...
            success: (res) => {
                if (res.data) {
                    ...
                    this.setState({ billno: res.data.head[this.formId].rows[0].values.vbillno.value,vbillno: res.data.head[this.formId].rows[0].values.vbillno.value });
                    //处理公式
