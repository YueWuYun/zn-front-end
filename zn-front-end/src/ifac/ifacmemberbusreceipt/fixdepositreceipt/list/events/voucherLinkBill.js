...
import { baseReqUrl, javaUrl, list, card} from '../../cons/constant';
...
export const voucherLinkBill = function (props, pageCode, tableId) {
    ...
    if (checkedData && checkedData.length > 0) {
        ajax({
            url: `${baseReqUrl}${javaUrl.voucherlink}.do`,
            data: {
                ...
                if (success) {
                    if (data) {
                        if(data[tableId].rows.length == 1){
                            let srcid = data[tableId].rows[0].values.pk_fixdepositreceipt.value;
                            setTimeout(() => {
                                props.pushTo("/card", {
                                    status: "browse",
                                    scene: SCENE.LINK,
