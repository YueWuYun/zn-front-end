...
export const listMultiOperator = function (props, pageCode, tableCode, pkName, url, actionname, code,datasource, date,extParam, callback) {
    ...
    if (selectDatas == null || selectDatas.length == 0) {
        toast({ color: 'warning', content: that.state.json['36340CDIC-000001']/**国际化处理：未选中行 */ });
        return;
    }
    //单笔数据选中，走单笔操作
    if (selectDatas.length == 1) {
        ...
        listSingleOperator.call(this,props, pageCode, tableCode, url, record, pkName, selectDatas[0].index, actionname, datasource,date, extParam, callback);
        return;
    }
    let pkMapTs = {};
    let pkMapVbillno = {};
    ...
    if (Object.keys(pkMapTs).length > 0) {
        ajax({
            ...
            success: (res) => {
                ...
                succContent = succCount > 0 ? succContent + that.state.json['36340CDIC-000002']/**国际化处理：项 */ + actionname + that.state.json['36340CDIC-000003']/**国际化处理：成功 */ : '';
                let content = (succContent == '' ? '' : succContent + '，') + (failCount > 0 ? errContent + that.state.json['36340CDIC-000002']/**国际化处理：项 */  + actionname + that.state.json['36340CDIC-000004']/**国际化处理：失败 */ : '');
                //全部成功
                //if (status == 0 && errMsgArr.length == 0) {
                if (errMsgArr.length == 0) {
                    toast({ color: 'success', content: actionname + that.state.json['36340CDIC-000005']/**国际化处理：成功，共 */ + succCount + that.state.json['36340CDIC-000006']/**国际化处理：条 */ });
                }
                //全部失败/部分失败
                    ...
                    toast({
                        ...
                        color: 'danger',
                        content,
                        TextArr: [that.state.json['36340CDIC-000007']/**国际化处理：展开*/, that.state.json['36340CDIC-000008']/**国际化处理：收起 */, that.state.json['36340CDIC-000009']/**国际化处理：我知道了 */],
                        groupOperation: true,
 ...
 * @param {*} callback 
 */
export const listSingleOperatorNoRecord = function (props, pageCode, tableCode, url, pk, ts, pk_intobj,rowIndex, actionname, datasource,  date,extParam, callback) {
    let that = this;
    let pkMapTs = {};
    if(!ts){
        ts = 0;
    ...
    ajax({
        ...
        data: {
            //主键pk与时间戳ts的映射
            pkMapTs, pageCode, extParam,date
        },
        success: (res) => {
            let { data } = res;
            let hasTbbMsg = false;
            if (data) {
                if (data && data.head && data.head[tableCode] && data.head[tableCode].rows && data.head[tableCode].rows.length == 1) {
                    ...
                    props.table.updateDataByIndexs(tableCode, updateDataArr);
                    if (!hasTbbMsg) {
                        toast({ color: 'success', content: actionname + that.state.json['36340CDIC-000003']/**国际化处理：成功 */  });
                    }
                }
            } else {
                //删除缓存数据
 ...
 * @param {*} callback 回调
 */
export const listSingleOperator = function (props, pageCode, tableCode, url, record, pkName, rowIndex, actionname, datasource,date, extParam, callback) {
    let pk = record[pkName].value;
    let ts = record['ts'].value;
    let pk_intobj = record['pk_intobj'].value;
    
    listSingleOperatorNoRecord.call(this,props, pageCode, tableCode, url, pk, ts,pk_intobj, rowIndex, actionname, datasource,date, extParam, callback);
}

export const tryInterest = function(props, list_page_id,startdate,enddate){
    let link_Data = props.table.getCheckedRows(this.tableId);
    let pk,pk_intobj,settlemode;
    link_Data.forEach((val) => {
        pk_intobj = val.data.values.pk_intobj.value;
        pk = val.data.values.pk_demandintcal.value;
    });
