    ...
    switch (id) {
        ...
        case 'withholdingConfirm':
            if(!this.state.withholdingdate){
                toast({ color: 'danger', content: this.state.json['36340FDIC-000011']/**国际化处理：请选择日期 */});
            }else{
               if(record){
                    listSingleOperator.call(this,props, list_page_id, 'head',requesturl.preaccrued, record,'pk_depositreceipt', index, this.state.json['36340FDIC-000012']/**国际化处理：预提 */, demandintcalCacheKey,this.state.withholdingdate);
               }else{
                    listMultiOperator.call(this,props, list_page_id, 'head', 'pk_depositreceipt', requesturl.preaccrued, this.state.json['36340FDIC-000012']/**国际化处理：预提 */,'depositcode', demandintcalCacheKey,this.state.withholdingdate);
               }
            }
            break;
