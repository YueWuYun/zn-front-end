        ...
        ajax({
            ...
            success: (res) => {
                if (res.data) {
                    ...
                    updateCache('pk_interest',pks,res.data,card_from_id,dataSourceTam,res.data.head[card_from_id].rows[0].values);
                    this.setState({ billno: res.data.head[card_from_id].rows[0].values.vbillcode.value });
                    //处理公式
