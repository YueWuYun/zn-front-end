...
export const calculateLink = function(props,type,appcode,pagecode,linkpath){
	...
	if(type&&type==='fixrate'){
		if(!pk_depositrate){
			...
			toast({color:"warning",content:this.state.json['36340FDIB-000012']}) 
            return;
        }
		...
		if(!pk_aiacrate){
			...
			toast({color:"warning",content:this.state.json['36340FDIB-000013']}) 
            return;
        }
		rateLink.call(this,props,pk_aiacrate);
	}else if(type&&type==='voucher'){
        linkVoucherApp.call(this,props,pk_interest,pk_groupv,pk_orgv,'36LW',billnov);
    }else if(type&&type==='dereceipt'){
		if(!depositcode){
			//toast({color:"warning",content:'存单未空，不能联查'}) 
			toast({color:"warning",content:this.state.json['36340FDIB-000014']}) 
			return;
		 }
	...
	ajax({
		...
		success: (res) => {
			...
			if (!success || !data || !data.src || VOUCHER_INFO.LINK_VOUCHER != data.src || !data.pklist || data.pklist.length == 0) {
				toast({
					'color': 'warning',
					'content': this.state.json['36340FDIB-000015']
				});
				return;
			}
			...
			if (data.des) {//跳转到凭证界面
				...
				if (pklist.length == 1) {
					props.openTo(url, {
						...
						pagecode,
						id: pklist[0],
						n: this.state.json['36340FDIB-000001'],
						pagekey: 'link',
						backflag: 'noback'
					});
				}
