...
export default function buttonClick(props, id) {
    ...
    if ([APPROVALOPINION_BTN, DEBITCONTRACT_BTN, CONTRACT_BTN, 'Loan'].includes(id) && selectDatas.length > 1) {
        toast({ color: 'warning', content: this.state.json['36140FDLI-000021'] });/* 国际化处理： 请选中单条数据进行操作!*/
        return;
    }
    if([DELETE_BTN, COPY_BTN, EDIT_BTN, CONFIRM_BTN, UNCONFIRM_BTN,PRINT,OUTPUT_BTN,PRINT_BTN,ENCLOSURE_BTN].includes(id) && selectDatas.length<1){
        toast({ color: 'warning', content: this.state.json['36140FDLI-000020'] });/* 国际化处理： 请选中单条数据进行操作!*/
        return;
    }
    ...
    switch (id) {
        ...
        case AIACT_RATE:
            ...
            if (link_rateData.length != 1) {
                toast({
                    color: 'warning',
                    content: this.state.json['36140FDLI-000021']/* 国际化处理： 请选中单条数据进行操作*/
                });
                return;
            }
            ...
            if (!pk_aiacrate) {
                toast({ color: "warning", content: this.state.json['36140FDLI-000038'] })/* 国际化处理： 暂无数据*/
                break;
            }
        ...
        //头部  修改
        case EDIT_BTN:
            // let selectdata = props.table.getCheckedRows(list.tableCode);
            // let copyData = selectdata;
            ...
            //     copyid = val.data.values.pk_varieties.value;
            // });
            ...
            if (selectDatas.length && selectDatas.length != 1) {
                // 修改只能选择一条数据修改
                toast({ color: "warning", content: this.state.json['36140FDLI-000021'] });/* 国际化处理： 请选择数据！*/
                return;
            }
            this.props.pushTo("/card", {
                status: "edit",
        ...
        case DELETE_BTN:
            promptBox({
                color: "warning",
                title: this.state.json['36140FDLI-000005'],/* 国际化处理： 删除*/
                content: pks.length > 1 ? this.state.json['36140FDLI-000022'] : this.state.json['36140FDLI-000006'],/* 国际化处理： 确定要删除所选数据吗?,确定要删除吗?*/
                beSureBtnClick: bodyBtnOperation.bind(this, selectDatas, javaUrl.delete, null, true, null, this.state.json['36140FDLI-000005'])  /* 国际化处理： 删除*/
            });
            break;
        //头部 确认
        // case CONFIRM_BTN:
        //     bodyBtnOperation.call(this, selectDatas, javaUrl.confirm, this.state.json['36140FDLI-000001'], pks.length > 1, null, this.state.json['36140FDLI-000023']);/* 国际化处理： 提交成功!,提交,提交*/
        //     break;
        //头部 取消确认
        // case UNCONFIRM_BTN:
        //     bodyBtnOperation.call(this, selectDatas, javaUrl.unconfirm, this.state.json['36140FDLI-000010'], true, null, this.state.json['36140FDLI-000024']);/* 国际化处理： 收回成功!,收回,收回*/
        //     break;
        ...
        //头部 委托办理
        case ENTRUST_BTN:
            bodyBtnOperation.call(this, selectDatas, javaUrl.entrust, this.state.json['36140FDLI-000028'], pks.length > 1, null, this.state.json['36140FDLI-000030']);/* 国际化处理： 提交成功!,提交,提交*/
            break;
        //头部 取消委托
        case UNENTRUST_BTN:
            bodyBtnOperation.call(this, selectDatas, javaUrl.unentrust, this.state.json['36140FDLI-000029'], true, null, this.state.json['36140FDLI-000031']);/* 国际化处理： 收回成功!,收回,收回*/
            break;
        //头部 打印
        case PRINT_BTN:
            print(
...
const tallyHeadConfirm = function (props) {
    listMultiOperator(props, list.pageCode, list.tableCode, list.primaryId, `${baseReqUrl}${javaUrl.confirm}.do`, this.state.json['36140FDLI-000040'], list.listCache);/* 国际化处理：记账成功*/
};

//取消确认
const unTallyHeadConfirm = function (props) {
    listMultiOperator(props, list.pageCode, list.tableCode, list.primaryId, `${baseReqUrl}${javaUrl.unconfirm}.do`, this.state.json['36140FDLI-000041'], list.listCache);/* 国际化处理： 取消记账成功*/
};
