...
export function bodyButtonClick(props,key, record, index) {
    ...
    switch (key) {
        ...
        //     bodyBtnOperation.call(this, list, javaUrl.confirm, this.state.json['36140FDLI-000001'], false, null, this.state.json['36140FDLI-000023']);/* 国际化处理： 提交成功!,提交,提交*/
        //     break;
        // case  UNCONFIRM_INNER_BTN:  //收回
        //     bodyBtnOperation.call(this, list, javaUrl.unconfirm, this.state.json['36140FDLI-000010'], false, null, this.state.json['36140FDLI-000024']);/* 国际化处理： 收回成功!,收回,收回*/
        //     break;
        case DELETE_INNER_BTN:  //删除 
            bodyBtnOperation.call(this, list, javaUrl.delete, this.state.json['36140FDLI-000007'], false, null, this.state.json['36140FDLI-000005'])/* 国际化处理： 删除成功!*/
            break;
        default:
            break;
    }
...
const tallyConfirm = function (props, record, index) {
	...
	ajax({
		...
		success: (res) => {
			...
			toast({ color: 'success', content: this.state.json['36340FDLB-000064'] });/* 国际化处理： 删除成功*/
			let updateDataArr = [{
				index: index,
...
const unTallyConfirm = function (props, record, index) {
	...
	ajax({
		...
		success: (res) => {
			...
			toast({ color: 'success', content: this.state.json['36340FDLB-000065'] });/* 国际化处理： 删除成功*/
			let updateDataArr = [{
				index: index,
				data: { values: res.data.head[card.headCode].rows[0].values }
				}];
...
export function bodyBtnOperation(list, path, content, isBatch = false, userObj = null, opername) {
    if (isBatch && !list.length) {
        toast({
            color: 'warning', content: this.state.json['36140FDLI-000020']/* 国际化处理： 请选择至少一条数据!*/
        });
        return;
    }
    let data = {
...
export function PromptMessage(res, opername, Msg) {
    ...
    if (status == 0) {
    	...
    	let btnObj = {Copy: true, Edit: true, Delete: true, Confirm: true, UnConfirm: true, Print_group: true,DEPOSITRATE: true,File: true};
