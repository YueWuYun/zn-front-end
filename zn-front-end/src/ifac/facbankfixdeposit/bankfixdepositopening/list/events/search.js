/*w/8iNiAH+zj8WJF4+Br6+pr+60QfQgAnwRAheT8/H7QMm+zbZLxZc7EMIVWEQ++R*/
 ...
 */
export function searchBtnClick (props, condition, type, querycondition, isToast= true) {
    if (!condition) {//刷新按钮
        condition= deepClone(getDefData(this.searchKey, this.dataSource));//缓存的查询条件{需要先克隆一下}
        if (!condition) {
            toast({color: 'warning', content: this.state.json['36140FDLI-000025']});/* 国际化处理： 请选择必输的查询条件!*/
            return;
        }
    } else {//查询按钮
...
function listRender (res, isToast) {
    ...
    if (success && data && data.grid && data.grid[this.tableId]) {
        ...
        let flag= data.grid[this.tableId].allpks.length;
        if (isToast) {
            if (flag) {
                toast({color: 'success', content: this.state.json['36140FDLI-000026']+",共"+flag+"条"});/* 国际化处理： 查询成功!*/
            } else {
                toast({color: 'warning', content: this.state.json['36140FDLI-000027']});/* 国际化处理： 未查询出符合条件的数据!*/
            }
        }
    } else {
