...
export function buttonClick(props, id) {
    ...
    switch (id) {
        ...
        case COPY_BTN:
            ...
            this.props.form.setFormItemsDisabled(this.formId, { 'billmaker': true, 'billmakedate': true, 'pk_confirm': true, 'confirmdate': true, 'creator': true, 'creationtime': true, 'modifier': true, 'modifiedtime': true});
        ...
        case DELETE_BTN:
            promptBox({
                ...
                content: this.state.json['36140FDLI-000006'],/* 国际化处理： 确定要删除吗?*/
                beSureBtnClick: this.btnOperation.bind(this, javaUrl.delete, this.state.json['36140FDLI-000007'])/* 国际化处理： 删除成功!*/
            });
            break;
        // 头部 取消
        case CANCEL_BTN:
            promptBox({
                color: "warning",
                title: this.state.json['36140FDLI-000008'],/* 国际化处理： 取消*/
                content: this.state.json['36140FDLI-000009'],/* 国际化处理： 确定要取消吗?*/
        ...
        //头部 委托办理
        case ENTRUST_BTN:
            this.btnOperation(javaUrl.entrust, this.state.json['36140FDLI-000028'], pk);/* 国际化处理： 委托办理成功!*/
            break;
        //头部 取消委托
        case UNENTRUST_BTN:
            this.btnOperation(javaUrl.unentrust, this.state.json['36140FDLI-000029'], pk);/* 国际化处理： 取消委托成功!*/
            break;
        //头部 审批详情
...
const tallyConfirm = function (props) {
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (res.success) {
                toast({ color: 'success', content: that.state.json['36340FDLB-000066'] });/* 国际化处理： 保存成功*/
                if (res.data) {
...
const unTallyConfirm = function (props) {
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (res.success) {
                toast({ color: 'success', content: this.state.json['36340FDLB-000067'] });/* 国际化处理： 保存成功*/
                if (res.data) {
                    if (res.data.head && res.data.head[this.formId]) {
...
function saveOperation(props, data, type) {
    let isAdd = this.props.getUrlParam('status');
    ajax({
        ...
        success: (res) => {
            if (res.success) {
                if (res.data) {
                    ...
                    this.idTs = { id, ts };
                    toast({ color: 'success', content: this.state.json['36140FDLI-000011'] });/* 国际化处理： 保存成功*/
                    // 缓存
                    if (isAdd==='add' ||  isAdd==='copy') {
                    ...
                    if (type === 'add') {
                        ...
                        this.props.form.setAllFormValue({ [this.formId]: res.data.head[this.formId] });
                        this.btnOperation(javaUrl.commit, this.state.json['36140FDLI-000017'], res.data.head[this.formId].rows[0].values[this.primaryId].value);/* 国际化处理： 提交成功!*/
                    } else {
                        props.setUrlParam({
                            id,
                            status: 'browse'
