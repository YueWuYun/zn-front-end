...
export function getCardData (url, id, isFirst= false, isRefresh= false) {
    ...
    ajax({
        ...
        success: (res) => {
            ...
            if (success) {
            	let idNew = data.head[this.formId].rows[0].values[this.primaryId].value;//20200324xuechh复制保存后删除,单据没有刷新的问题
                if (data && data.head) {
                ...
                if (isFirst || isRefresh) {//didmount或者刷新按钮
                    //保存缓存
                	if(idNew){
                        addCache(idNew, data, this.formId, this.cache);//20200324xuechh复制保存后删除,单据没有刷新的问题
                    }
                } else {
                    // 更新缓存
