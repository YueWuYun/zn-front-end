    ...
    switch (id) {
        ...
        case DELETE_BTN:
            promptBox({
                ...
                content: this.state.json['36140DV-000006'],/* 国际化处理： 确定要删除吗?*/
                beSureBtnClick: this.btnOperation.bind(this, javaUrl.delete, this.state.json['36140DV-000007'])/* 国际化处理： 删除成功!*/
            });
            break;
        // 头部 取消
        case CANCEL_BTN:
            promptBox({
                color: "warning",
                title: this.state.json['36140DV-000008'],/* 国际化处理： 取消*/
                content: this.state.json['36140DV-000009'],/* 国际化处理： 确定要取消吗?*/
        ...
        success: (res) => {
            if (res.success) {
                if (res.data) {
                    ...
                    this.idTs = { id, ts };
                    toast({ color: 'success', content: this.state.json['36140DV-000011'] });/* 国际化处理： 保存成功*/
                    // 缓存
                    ...
                    if (type === 'add') {
                        ...
                        this.props.form.setAllFormValue({ [this.formId]: res.data.head[this.formId] });
                        this.btnOperation(javaUrl.commit, this.state.json['36140DV-000017'], res.data.head[this.formId].rows[0].values[this.primaryId].value);/* 国际化处理： 提交成功!*/
                    } else {
                        props.setUrlParam({
                            id,
                            status: 'browse'
