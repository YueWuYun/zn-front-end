...
export default function afterEvent(props, moduleId, key, value, changedrows, i) {
	...
	                content: this.state.json['36340AIAC-000028'],//弹框内容，可以是字符串或dom/* 国际化处理： 是否修改组织，这样会清空您录入的信息?*/
	...
	            })
		...
		case 'floatingpercent':
			...
			if(newvalue > 100 || newvalue < 0){
				toast({color:"warning",content:this.state.json['36340AIAC-000065']});/* 国际化处理： 账户重复，请重新输入*/
		...
		case 'pk_ratecode':
			...
			if(begdate == null){
				toast({color:"warning",content:this.state.json['36340AIAC-000064']});/* 国际化处理： 账户重复，请重新输入*/
			...
			allrows.forEach(val => {
				if(val.rowid == changerows[0].rowid){
					...
					if(val.values.pk_accid.value == value.refpk){
						toast({color:"warning",content:this.state.json['36340AIAC-000063']});/* 国际化处理： 账户重复，请重新输入*/
						flag =true;
					}
				}
