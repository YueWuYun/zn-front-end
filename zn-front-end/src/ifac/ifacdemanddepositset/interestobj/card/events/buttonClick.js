...
export default function (props, id) {
  ...
  switch (id) {
    ...
    //联查利率
    case 'Linkrate':
      let pk_ratecode = props.form.getFormItemsValue(card_from_id, 'pk_ratecode').value;
    ...
    case 'Save':
      ...
      if(saveflag){
        if(!savebodyflag){
          toast({ color: 'warning', content: this.state.json['36340AIAC-000054'] });/* 国际化处理： 账户必须有一个子户！*/
          break;
        }
        let saveobj = {};
    ...
              title: this.state.json['36340AIAC-000027'],// 弹框表头信息/* 国际化处理： 确认修改*/
              content: this.state.json['36340AIAC-000062'],//this.modalContent(), //弹框内容，可以是字符串或dom/* 国际化处理： 计息对象未计息，修改有可能导致积数调整失效，是否继续?*/
    ...
    case 'Delete':
      ...
      promptBox({
          color: 'warning', // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
          title: this.state.json['36340AIAC-000023'],
          content: this.state.json['36340AIAC-000024'],/* 国际化处理： 是否确认删除？*/
...
      })
    ...
    case 'Cancel':
      ...
      promptBox({
          color: 'warning', // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
          title: this.state.json['36340AIAC-000029'],
          content: this.state.json['36340AIAC-000014'],/* 国际化处理： 是否确认要取消？*/
          //点击确定按钮事件
				  beSureBtnClick: () => {
    ...
    case 'DeleteVersion':
      ...
      if(version == 0){
        promptBox({
            color: 'warning', // 提示类别默认"success"， "success"/"info"/"warning"/"danger",非必输
            title: this.state.json['36340AIAC-000060'],
            content: this.state.json['36340AIAC-000061'],
        })
      }else{
    ...
    case 'Disable':
      ...
      ajax({
        ...
        success: (res) => {
          ...
          toast({ color: 'success', content: props.MutiInit.getIntl("36340AIAC") && props.MutiInit.getIntl("36340AIAC").get('36340AIAC-000010') });/* 国际化处理： 操作成功*/
          props.pushTo('/card', {
            status: 'browse',
            id: pk,
            pagecode: card_page_id
    ...
    case 'DeleteRow':
      ...
      let currSelect = [];
      if(currRows === false){
        toast({color:"warning",content:this.state.json['36340AIAC-000016']})/* 国际化处理： 请选择要删除的行*/
        break;
      }
