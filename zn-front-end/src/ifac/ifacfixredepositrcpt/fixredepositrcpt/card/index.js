...
class Card extends Component {
	...
	// 浏览态数据渲染
	browseRender = (pkbill) =>{
		// let cardData,billstatus,billno,pk_fixredepositrcpt
		let cardData,billno,pk_fixredepositrcpt
		if(pkbill){
			...
			if(cardData){
				...
				pk_fixredepositrcpt = cardData[this.formId].rows[0].values[this.pkname].value;
					...
					success: (res) => {
						if (res) {
							if (res.data) {
								...
								pk_fixredepositrcpt = res.data.head[this.formId].rows[0].values[this.pkname].value;
								// billstatus = res.data[this.formId].rows[0].values.billstatus.value;

								// billno = res.data[this.formId].values.vbillno.value;
								// pk_fixredepositrcpt = res.data[this.formId].values[this.pkname].value;
								// billstatus = res.data[this.formId].rows[0].values.billstatus.value;
								if (res.data.body) {
								...
								// 	if(getDefData(pk_fixredepositrcpt, constant.cacheDataSource)){
								// 		updateCache(this.tableId, pkbill, this.formId, constant.cacheDataSource, res.data.head[this.formId].rows[0].values);
								// 	}else{
								// 		addCache(pk_fixredepositrcpt, res.data,this.formId, constant.cacheDataSource);
								// 		setDefData(pk_fixredepositrcpt, constant.cacheDataSource, res.data);
								// 	}
								// }else{
								// 	updateCache(this.tableId, pkbill, this.formId, constant.cacheDataSource, res.data.head[this.formId].rows[0].values);
								// }
			...
			<div className="nc-bill-card">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area" >
					<div className="header-title-search-area">
						{/* {createBillHeadInfo({
							title: this.state.json['36340RFDR-000019'],
