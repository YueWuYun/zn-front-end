...
let { FixedWithDrawApplyConst, pageCodeCard, base_url, formId, pkname, assignTypecon } = CONSTANTS;
/**
 * 卡片API
 */
...
export const  mysaveCommit = function (props, assign) {
    let extParam = {btncode:"SaveCommit",pagecode:"36340FDWA_C01"};
    saveCommit(this.props, {
        pageCode: pageCodeCard,//页面编码
        headCode: formId, //表头区域编码
        bodyCode: "",//表体区域编码（多表体传数组，没有表体不传）
        url: base_url+'FDWDWASaveCommitAction.do',//请求url
        assign,//指派信息
        //展示指派框的逻辑
        showAssignFunc: (res) => {
            let { data } = res; 
        ...
        updateViewFunc: (res) => {
            ...
            saveUpdateView(this,props, res, () => {
                toast({ color: 'success', content: loadMultiLang(props, '36340FDWA-000033')+loadMultiLang(props, '36340FDWA-000021')/* 国际化处理： 提交成功！*/ });
            });
        },
        //保存的校验（无需考虑验证公式，本身包含）
        saveValidate: () => {
