...
import { CurrExConst, pageCodeCard,formId, base_url, pkname,FixedWithDrawApplyConst } from '../../cons/constant';
...
export default function (props, pk) {
    ...
    let cardData = getCacheDataByPk(props, FixedWithDrawApplyConst.dataSource, pk);
    if (cardData) {
        props.setUrlParam(pk);//动态修改地址栏中的id的值
        props.form.setAllFormValue({[formId]: cardData.head[formId]});
        this.billID = cardData.head[formId].rows[0].values.pk_fwithdrawapply.value || '';
            ...
            success: (res) => {
                if (res.data) {
                    if (res.data.head) {
                        ...
                        this.billID = res.data.head[formId].rows[0].values.pk_fwithdrawapply.value || '';
