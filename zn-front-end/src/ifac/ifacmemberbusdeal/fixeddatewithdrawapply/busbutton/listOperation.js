...
let {FixedWithDrawApplyConst} = CONSTANTS;
    ...
    if (Object.keys(pkMapTs).length > 0) {
        ajax({
            ...
            success: (res) => {
                        ...
                        for(let operatorResult of operatorResults.data){
                            ...
                            if (state == 0) {
                                ...
                                if (!result||selectedGroup!='0') {
                                    if(multiOperResult.succCount>0){
                                        multiOperResult.succContent=multiOperResult.succContent+"、";
                                    }
                                    ...
                                    if (result && result[tableCode] && result[tableCode].rows && result[tableCode].rows.length > 0) {
                                        ...
                                        if (showTBB) {
                                            ...
                                            if (tbbMsg) {
                                                ...
                                                if(multiOperResult.failCount>0){
                                                    multiOperResult.errContent=multiOperResult.errContent+"、";
                                                }
                ...
                succContent = succCount > 0 ? loadMultiLang(props, '36340FDWA-000040')+succContent + loadMultiLang(props, '36340--000120')/*'项'*/ + actionname + loadMultiLang(props, '36340--000121')/*'成功'*/ : '';
                let content = (succContent == '' ? '' : succContent + '，') + (failCount > 0 ? loadMultiLang(props, '36340FDWA-000040')+errContent + loadMultiLang(props, '36340--000120')/*'项'*/  + actionname + loadMultiLang(props, '36340--000122')/*'失败'*/ : '');
                //全部成功
