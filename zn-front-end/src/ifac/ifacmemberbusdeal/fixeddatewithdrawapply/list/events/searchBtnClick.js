...
import { app_id, oid, base_url, pageCodeList, searchId, tableId, tabStatus, FixedWithDrawApplyConst, LISTGROUP } from '../../cons/constant.js';
...
export default function searchBtnClick(props, condition, type, a, isSearchButton = true, isRefresh = false) {
    ...
    //点击查询按钮，将查询条件放入缓存
    if (isSearchButton) {
        setDefData(FixedWithDrawApplyConst.dataSource, FixedWithDrawApplyConst.queryCondition, condition);
    }
    //点击刷新按钮，从缓存中获取查询条件
    // else if (isRefresh) {
    //     condition = getDefData(FixedWithDrawApplyConst.dataSource, FixedWithDrawApplyConst.queryCondition);
    // }
...
function listRender (res, isRefresh,isSearchButton) {
    ...
    if (success && data && data.grid && data.grid[tableId]) {
        ...
        if (flag!="0") {
            if (isRefresh) {
                toast({color: 'success', content: loadMultiLang(this.props, '36340FDWA-000022'),/* 国际化处理： 刷新成功！*/});
            }else if(isSearchButton){
                toast({color: 'success', content: loadMultiLang(this.props, '36340FDWA-000023'),/* 国际化处理： 查询成功！*/});
            }
        }else if(isSearchButton||isRefresh){
            toast({color: 'warning', content: loadMultiLang(this.props, '36340FDWA-000010'),/* 国际化处理： 未查询出符合条件的数据!*/});
        }
    } else {
...
const getGroupCondition = function (groupKey) {
    ...
    switch (groupKey) {
        ...
        case LISTGROUP.NEEDAPPROV:
            ...
            break;
        //待办理
        case LISTGROUP.NEEDCONSIGN:
            groupCondition = {
                field: 'billstate',
                value: {
                    firstvalue: parseInt(3),
                    secondvalue: null
                },
                oprtype: '=',
                datatype: '203',
            ...
            break;
        //办理中
        case LISTGROUP.CONSIGNING:
            groupCondition = {
                field: 'billstate',
                value: {
                    firstvalue: parseInt(4),
                    secondvalue: null
                },
                oprtype: '=',
                datatype: '203',
...
export function setStatusNumKey (tabContainer) {
    if (Object.keys(tabContainer).length) {
        ...
        setDefData(FixedWithDrawApplyConst.dataSource,'numvalues',numvalues);
    }
}
/*YuO8szH0cVixePu/Bt+mGzpFGcu+B+/4LbnQ7jXcBB3F+W7ut3Ygc4tAx8nyRZYl*/