...
export const listMultiOperator = function (props, pageCode, tableCode, pkName, url, actionname, code,datasource, date,extParam, callback) {
    ...
    if (selectDatas == null || selectDatas.length == 0) {
        toast({ color: 'warning', content: that.state.json['36340FDICC-000001']/**国际化处理：未选中行 */ });
        return;
    }
    //单笔数据选中，走单笔操作
    ...
    if (Object.keys(pkMapTs).length > 0) {
        ajax({
            ...
            success: (res) => {
                ...
                succContent = succCount > 0 ? succContent + that.state.json['36340FDICC-000002']/**国际化处理：项 */ + actionname + that.state.json['36340FDICC-000003']/**国际化处理：成功 */ : '';
                let content = (succContent == '' ? '' : succContent + '，') + (failCount > 0 ? errContent + that.state.json['36340FDICC-000002']/**国际化处理：项 */  + actionname + that.state.json['36340FDICC-000004']/**国际化处理：失败 */ : '');
                //全部成功
                if (errMsgArr.length == 0) {
                    toast({ color: 'success', content: actionname + that.state.json['36340FDICC-000005']/**国际化处理：成功，共 */ + succCount + that.state.json['36340FDICC-000006']/**国际化处理：条 */ });
                }
                //全部失败/部分失败
                    ...
                    toast({
                        ...
                        color: 'danger',
                        content,
                        TextArr: [that.state.json['36340FDICC-000007']/**国际化处理：展开*/, that.state.json['36340FDICC-000008']/**国际化处理：收起 */, that.state.json['36340FDICC-000009']/**国际化处理：我知道了 */],
                        groupOperation: true,
        ...
        success: (res) => {
            ...
            if (data) {
                if (data && data.head && data.head[tableCode] && data.head[tableCode].rows && data.head[tableCode].rows.length == 1) {
                    ...
                    props.table.updateDataByIndexs(tableCode, updateDataArr);
                    if (!hasTbbMsg) {
                        toast({ color: 'success', content: actionname + that.state.json['36340FDICC-000003']/**国际化处理：成功 */  });
                    }
                }
            } else {
                //删除缓存数据
