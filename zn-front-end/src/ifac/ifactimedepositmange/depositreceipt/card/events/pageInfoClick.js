...
export default function (props, pks) {
    ...
    if(cardData){
        ...
        let vbillcode = cardData.head[this.formId].rows[0].values.vbillcode.value;
        ...
        ajax({
            ...
            success: (res) => {
                if (res.data) {
                    ...
                    this.setState({ billno: res.data.head[this.formId].rows[0].values.vbillcode.value,vbillcode: res.data.head[this.formId].rows[0].values.vbillcode.value });
                    //处理公式
