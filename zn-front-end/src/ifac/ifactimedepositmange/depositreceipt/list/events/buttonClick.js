...
export default function buttonClick(props, id) {
    switch (id) {
        ...
        case 'Account':
            ...
            let pk_depositreceipt = link_accountData_regular[0].data.values.pk_depositreceipt ? link_accountData_regular[0].data.values.pk_depositreceipt.value : null;
            ...
            //     if (val.data.values.pk_depositreceipt && val.data.values.pk_depositreceipt.value) {
            //         pk_depositreceipt = val.data.values.pk_depositreceipt.value;
            //     }
            // });
           
            props.openTo("/ifac/ifacreportquery/centerjournal/main/index.html#/list", {
                id: pk_depositreceipt,
                status: 'browse',
                appcode: "36341FDIBS",
                pagecode: "36341FDIBS_L01",
                scene: "linksce"
            });
            break;
        //联查定期利率
        case 'RegularRate':
            ...
            if(!pk_aiacrate){
                toast({color:"warning",content:this.state.json['36340FDLB-000008']})/* 国际化处理： 暂无数据*/
                break;
            }
    ...
              	title: this.state.json['36340FDLB-000068'],
                content: this.state.json['36340FDLB-000069'],
             	beSureBtnClick: tallyHeadConfirm.bind(this,props),
                cancelBtnClick: cancelEditBtnClick.bind(this, props)
          	})
            break;
        //解冻
        case 'defrozen':
...
//取消
const cancelEditBtnClick = function (props) {
	// props.pushTo('/card', {
	// 	status: 'browse',
...
const tallyHeadConfirm = function (props) {
    listMultiOperator(props, list_page_id, list_table_id, pk_name, requesturl.tally,this.state.json['36340FDLB-000064'], dataSourceTam);/* 国际化处理：记账成功*/
};

//解冻
const unTallyHeadConfirm = function (props) {
	listMultiOperator(props, list_page_id, list_table_id, pk_name, requesturl.untally, this.state.json['36340FDLB-000065'], dataSourceTam);/* 国际化处理： 取消记账成功*/
};
...
const printAllList = function (props) {
	...
	//判断是否有选中行
	if (selectDatas == null || selectDatas.length == 0) {
		toast({ color: 'warning', content: this.state.json['36340FDLB-000039'] });/* 国际化处理： 请选中一行数据！*/
		return;
	}
	let index = 0;
	let oids = [];
...
const refreshHtml = function (props) {
    ...
    if(!refreshsearchVal){
        toast({
            duration: 3,
            title: that.state.json['36340FDLB-000053'],/* 国际化处理： 刷新成功！*/
            color: 'success'
        })
...
const fileMgr = function (props) {
	...
	// 	toast({ color: 'warning', content: that.state.json['36340FDLB-000039']});/* 国际化处理： 请选中一行数据！,请选中一行数据*/
	// 	return;
	// }
