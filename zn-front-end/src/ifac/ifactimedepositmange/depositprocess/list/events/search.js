...
export function searchBtnClick (props, condition, type, querycondition, isToast= true) {
    if (!condition) {//刷新按钮
        ...
        if (!condition) {
            toast({color: 'warning', content: this.state.json['36340FDR-000025']});/* 国际化处理： 请选择必输的查询条件!*/
            return;
        }
    } else {//查询按钮
...
function listRender (res, isToast) {
    ...
    if (success && data && data.grid && data.grid[this.tableId]) {
        ...
        if (isToast) {
            if (flag) {
                toast({color: 'success', content: this.state.json['36340FDR-000026']});/* 国际化处理： 查询成功!*/
            } else {
                toast({color: 'warning', content: this.state.json['36340FDR-000027']});/* 国际化处理： 未查询出符合条件的数据!*/
            }
        }
    } else {
