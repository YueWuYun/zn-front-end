...
class List extends Component {
	...
	componentWillMount() {
		...
		// getMultiLang({ moduleId: {'tmpub':['3601'], 'cmp':['36070', '36340ERD']}, 
		// 	callback: (lang) => {
		// 	//将多语资源数据存储到页面级缓存中
				...
				success: (res) => {
					...
					if (success) {
						if (data) {
							...
							// }
							let message = this.state.json['36340ERD-000046'] + data[this.tableId].allpks.length + this.state.json['36340ERD-000049'];
							toast({
					...
					success: (res) => {
						...
						if (success) {
							if (data) {
								...
								// }
								toast({
									content: this.state.json['36340ERD-000048'],/* 国际化处理： 刷新成功！*/
									color: 'success'
								})
								// this.props.button.setButtonDisabled(buttonDisable.querydisable, false);
							} else {
			...
			<div className="nc-bill-list">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area" >
					<div className="header-title-search-area">
						{/* {createPageIcon()}
						<h2 className="title-search-detail">{this.state.json['36340ERD-000019']}</h2>国际化处理： 账户汇兑损益单 */}
						{createBillHeadInfo(
							{
								title: this.state.json['36340ERD-000019'],//标题
