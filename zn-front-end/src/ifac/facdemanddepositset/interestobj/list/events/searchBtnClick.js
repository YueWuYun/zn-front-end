    ...
    if (searchVal) {
        ...
        ajax({
            ...
            success: (res) => {
                ...
                if (success) {
                    if (data) {
                        giveToast.call(this,props, data[list_table_id].allpks.length);
...
const giveToast = function (props, resLength) {
    if (resLength && resLength > 0) {
        toast({
            duration: 6,
            color: 'success',
            title: this.state.json['36140AIAC-000057'] + resLength + this.state.json['36140AIAC-000066']   /* 国际化处理： 查询成功！*/
        })
    }
    else {
        toast({
            duration: 6,
            color: 'warning',
            title: this.state.json['36140AIAC-000051']    /* 国际化处理： 未查询出符合条件的数据！*/
        })
    }
}
/*YuO8szH0cVixePu/Bt+mGzpFGcu+B+/4LbnQ7jXcBB3F+W7ut3Ygc4tAx8nyRZYl*/