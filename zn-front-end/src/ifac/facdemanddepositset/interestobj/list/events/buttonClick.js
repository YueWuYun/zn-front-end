...
import { pk_name,card_page_id, list_page_id, list_search_id, list_table_id,app_code,openaccountapplyCacheKey,searchArea,dataSourceTam } from '../../cons/constant.js';
...
export default function buttonClick(props, id) {
    switch (id) {
        ...
        case 'Linkrate':
            ...
            let pk_ratecode = link_rateData[0].data.values.ratecode ? link_rateData[0].data.values.ratecode.value : null;
            if (link_rateData.length != 1) {
                // toast({
                //     color: 'warning',
                //     content: this.state.json['36140AIAC-000039']/* 国际化处理： 请选择1条数据*/
                // });
                // return;
            }
            // link_rateData.forEach((val) => {
            //     if (val.data.values.ratecode && val.data.values.ratecode.value) {
        ...
        case 'Delete':
            promptBox({
                color: "warning",
                title: loadMultiLang(props, '36140AIAC-000023'),/* 国际化处理： 删除*/
                content: loadMultiLang(props, '36140AIAC-000067'),/* 国际化处理： 确认要删除吗?*/
        ...
        case 'Copy':			
            ...
            if (copyData.length != 1) {
                // toast({ color: 'warning', content: loadMultiLang(props, '36140AIAC-000039') });/* 国际化处理： 请选择1条数据*/
                // return;
            }
            
            let ts = copyData[0].data.values.ts ? copyData[0].data.values.ts.value : null;
            let copyid = copyData[0].data.values.pk_accintobj ? copyData[0].data.values.pk_accintobj.value : null;
...
const printAll = function (props) {
	...
	if (selectDatas == null || selectDatas.length == 0) {
		toast({ color: 'warning', content: loadMultiLang(props, '36140AIAC-000039') });/* 国际化处理： 请选中一行数据！*/
		return;
	}
	let index = 0;
	let oids = [];
 ...
 * @param {*} props 页面内置对象
 */
const printlistAll = function (props) {
	let selectDatas = props.table.getCheckedRows(list_table_id);
	//判断是否有选中行
	if (selectDatas == null || selectDatas.length == 0) {
		toast({ color: 'warning', content: loadMultiLang(props, '36140AIAC-000039') });/* 国际化处理： 请选中一行数据！*/
...
const delHeadConfirm = function (props) {
	listMultiOperator(props, list_page_id, list_table_id, pk_name, requesturl.delete, loadMultiLang(props, '36140AIAC-000023'), dataSourceTam);/* 国际化处理： 删除*/
};
        ...
        ajax({
            ...
            success: (res) => {
                ...
                if (success) {
                    ...
                    toast({
                        duration: 3,
                        title: that.state.json['36140AIAC-000053'],/* 国际化处理： 刷新成功！*/
                        color: 'success'
                    })
                }
