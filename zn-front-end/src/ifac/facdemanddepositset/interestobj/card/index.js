...
class Card extends Component {
	...
	componentWillMount() {
		...
		getMultiLang({ moduleId: '36140AIAC', domainName: 'ifac', callback });
	...
	refreshcard = () => {
		...
		if (this.props.getUrlParam('status') == 'browse') {
			...
			if(!pk){
				...
				toast({	
					content: this.state.json['36140AIAC-000053'],/* 国际化处理： 刷新成功！*/
					color: 'success'
				})
			}else{
...
                      	processFormulamsg(this.props, res);				
	...
	delConfirm = () => {
		...
		ajax({
			...
			success: (res) => {
				if (res) {
					toast({ color: 'success', content: this.state.json['36140AIAC-000017'] });/* 国际化处理： 删除成功*/
	...
	beSureOrgBtnClick = () => {
		...
		if (!pk_org) {
			...
			// this.props.form.setFormItemsValue(card_from_id,{ 'apply_type': { value:'1',display:this.state.json['36140AIAC-000021']} }); /* 国际化处理： 销户*/
			// this.props.form.setFormItemsValue(card_from_id,{ 'vbillstatus': { value:'-1',display:this.state.json['36140AIAC-000022']} });/* 国际化处理： 自由*/
			this.props.button.setButtonDisabled(['SaveRow', 'DeleteRow'], true);
		} else {
			this.props.resMetaAfterPkorgEdit();
		...
		return (
			<div className="nc-bill-card">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
						...
						<div>
							{createBillHeadInfo({
								title: this.state.json['36140AIAC-000034'], //标题/* 国际化处理： 内部账户计息设置*/
				...
				>
					<NCModal.Header closeButton >
						<NCModal.Title>{this.state.json['36140AIAC-000034']}</NCModal.Title>
					</NCModal.Header>
