...
import { base_url, pageCodeList, searchId, tableId,searchArea,search_key } from '../../cons/constant.js';
...
const giveToast = function (props, resLength) {
    if (resLength && resLength > 0) {
        toast({
            duration: 6,
            color: 'success',
            title: this.props.MutiInit.getIntl("36140FDSR").get('36140FDSR-000050') + resLength + this.props.MutiInit.getIntl("36140FDSR").get('36140FDSR-000061')  /* 国际化处理： 查询成功！*/
        })
    }
    else {
        toast({
            duration: 6,
            color: 'warning',
            title: this.props.MutiInit.getIntl("36140FDSR").get('36140FDSR-000062')   /* 国际化处理： 未查询出符合条件的数据！*/
        })
    }
