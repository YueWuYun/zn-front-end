...
class List extends Component {
	...
	// 将所有查询条件赋值进缓存
	addQueryCache = () => {
		let searchVal = this.props.search.getQueryInfo(this.searchId, false).querycondition;
		if (searchVal) {
				...
				success: (res) => {
					...
					if (success) {
						if (data) {
							...
							// }
							let message = this.state.json['36140RFD-000046'] + data[this.tableId].allpks.length + this.state.json['36140RFD-000049'];
							toast({
			...
			<div className="nc-bill-list">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area" >
					<div className="header-title-search-area">
						{/* {createPageIcon()}
						<h2 className="title-search-detail">{this.state.json['36140RFD-000019']}</h2>国际化处理： 账户汇兑损益单 */}
						{createBillHeadInfo(
							{
