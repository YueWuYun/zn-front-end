	...
	meta[list_search_id].items.map((item) => {
		...
		if (item.attrcode == 'pk_org' ) {
			item.queryCondition = () => {
				return {
					...
					pk_fundpayorg: (props.search.getSearchValByField(list_search_id, 'pk_fundorg') || {}).value.firstvalue, 
		...
		if (item.attrcode == 'depositcode') {
			item.queryCondition = () => {
				return {
					funcode: app_code,
					pk_org: (props.search.getSearchValByField(list_search_id, 'pk_fundorg') || {}).value.firstvalue, 
		...
		if (item.attrcode == 'pk_depositacc') {
			item.queryCondition = () => {
				return {
					...
					pk_fundorg: (props.search.getSearchValByField(list_search_id, 'pk_fundorg') || {}).value.firstvalue,
