...
export default function (props, pks) {
    ...
    if(cardData){
        ...
        let vbillcode = cardData.head[this.formId].rows[0].values.depositcode.value;
        this.setState({ billno: vbillcode,vbillcode: vbillcode });
        ...
        ajax({
            ...
            success: (res) => {
                if (res.data) {
                    ...
                    this.setState({ billno: res.data.head[this.formId].rows[0].values.depositcode.value,vbillcode: res.data.head[this.formId].rows[0].values.depositcode.value });
                    //处理公式
