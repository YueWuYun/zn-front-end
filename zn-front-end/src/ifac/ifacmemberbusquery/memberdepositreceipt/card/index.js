...
                      	processFormulamsg(this.props, res);
						if (res.data.head) {
							...
							let vbillcode =res.data.head[card_from_id].rows[0].values.depositcode.value;
							this.setState({
								billno:vbillcode,
								vbillcode:vbillcode
							});
						...
						buttonVisible(this.props);
						toast({	
							content: this.state.json['36341FDLQ-000053'],/* 国际化处理： 刷新成功！*/
							color: 'success'
						})
					} else {
	...
	toggleShow = () => {
		...
		if (status == 'browse') {
			if (this.props.getUrlParam('saveres')) {
				toast({ color: 'success', content: this.state.json['36341FDLQ-000009']});/* 国际化处理： 保存成功*/
						...
						success: (res) => {
							if (res.data) {
								...
								if (res.data.head) {
									...
									let vbillcode = res.data.head[this.formId].rows[0].values.depositcode.value;
									this.setState({
										billno: vbillcode,
										vbillcode: vbillcode
									});
	...
	beSureOrgBtnClick = () => {
		...
		if (!pk_org) {
			...
			// this.props.form.setFormItemsValue(card_from_id,{ 'apply_type': { value:'1',display:this.state.json['36341FDLQ-000021']} }); /* 国际化处理： 销户*/
			// this.props.form.setFormItemsValue(card_from_id,{ 'vbillstatus': { value:'-1',display:this.state.json['36341FDLQ-000022']} });/* 国际化处理： 自由*/
			...
			<div className="nc-bill-card">
				...
				<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
						...
						<div className="header-title-search-area">
							{createBillHeadInfo({
								title: this.state.json['36341FDLQ-000034'], //标题/* 国际化处理： 内部账户计息设置*/
