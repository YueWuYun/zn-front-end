...
const giveToast = function (props, resLength) {
    if (resLength && resLength > 0) {
        toast({
            duration: 6,
            color: 'success',
            title: this.state.json['36341FNIBS-000020']
                 +this.state.json['36341FNIBS-000005']+resLength+this.state.json['36341FNIBS-000006']/**国际化处理：查询成功 */
        })
    }
    else {
        toast({
            duration: 6,
            color: 'warning',
            title: this.state.json['36341FNIBS-000019']/**国际化处理：为查询出符合条件的数据 */
        })
    }
}
/*YuO8szH0cVixePu/Bt+mGzpFGcu+B+/4LbnQ7jXcBB3F+W7ut3Ygc4tAx8nyRZYl*/