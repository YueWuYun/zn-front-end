...
export const calculateLink = function(props,type,appcode,pagecode,linkpath){
	...
	if(type&&type==='fixrate'){
		...
		if(!pk_aiacrate){
            toast({color:"warning",content:this.state.json['36341FNIBS-000022']}) 
            return;
        }
		rateLink.call(this,props,pk_aiacrate);
	}else if(type&&type==='voucher'){
        linkVoucherApp.call(this,props,pk_interest,pk_groupv,pk_orgv,'3601',billnov);
...
export const depositCodeLink = function(props,depositcode){
	if(!depositcode){
		toast({
			color: 'warning',
			content: this.state.json['36341FNIBS-000019']//{/* 国际化处理： 未查询出符合条件的数据！*/}
		});
	} else {
	...
	ajax({
		...
		success: (res) => {
			...
			if (!success || !data || !data.src || VOUCHER_INFO.LINK_VOUCHER != data.src || !data.pklist || data.pklist.length == 0) {
				toast({
					'color': 'warning',
					'content': this.state.json['36341FNIBS-000025']
				});
				return;
			}
			...
			if (data.des) {//跳转到凭证界面
				...
				if (pklist.length == 1) {
					props.openTo(url, {
						...
						pagecode,
						id: pklist[0],
						n: this.state.json['36341FNIBS-000001'],
						pagekey: 'link',
						backflag: 'noback'
					});
				}
