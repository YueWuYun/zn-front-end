 ...
 * 
 */
export const listMultiOperator = function (props, pageCode, tableCode, pkName, url, actionname, code,datasource, callback) {
    let that = this;
    let selectDatas = props.table.getCheckedRows(tableCode);
    //判断是否有选中行
    if (selectDatas == null || selectDatas.length == 0) {
        toast({ color: 'warning', content: that.state.json['36341FNIBS-000001']/**国际化处理：未选中行 */ });
        return;
    }
    //单笔数据选中，走单笔操作
    if (selectDatas.length == 1) {
        ...
        listSingleOperator.call(this,props, pageCode, tableCode, url, record, pkName, selectDatas[0].index, actionname, datasource, callback);
        return;
    }
    let pkMapTs = {};
    let pkMapVbillno = {};
    ...
    if (Object.keys(pkMapTs).length > 0) {
        ajax({
            ...
            success: (res) => {
                ...
                succContent = succCount > 0 ? succContent + that.state.json['36341FNIBS-000002']/**国际化处理：项 */ + actionname + that.state.json['36341FNIBS-000003']/**国际化处理：成功 */ : '';
                let content = (succContent == '' ? '' : succContent + '，') + (failCount > 0 ? errContent + that.state.json['36341FNIBS-000002']/**国际化处理：项 */  + actionname + that.state.json['36341FNIBS-000004']/**国际化处理：失败 */ : '');
                //全部成功
                if (errMsgArr.length == 0) {
                    toast({ color: 'success', content: actionname + that.state.json['36341FNIBS-000005']/**国际化处理：成功，共 */ + succCount + that.state.json['36341FNIBS-000006']/**国际化处理：条 */ });
                }
                //全部失败/部分失败
                    ...
                    toast({
                        ...
                        color: 'danger',
                        content,
                        TextArr: [that.state.json['36341FNIBS-000007']/**国际化处理：展开*/, that.state.json['36341FNIBS-000008']/**国际化处理：收起 */, that.state.json['36341FNIBS-000009']/**国际化处理：我知道了 */],
                        groupOperation: true,
...
export const listSingleOperatorNoRecord = function (props, pageCode, tableCode, url, pk, ts,rowIndex, actionname, datasource,extParam, callback) {
    let that = this;
    let pkMapTs = {};
    if(!ts){
        ts = 0;
        ...
        success: (res) => {
            ...
            if (data) {
                if (data && data.head && data.head[tableCode] && data.head[tableCode].rows && data.head[tableCode].rows.length == 1) {
                    ...
                    props.table.updateDataByIndexs(tableCode, updateDataArr);
                    if (!hasTbbMsg) {
                        toast({ color: 'success', content: actionname + that.state.json['36341FNIBS-000003']/**国际化处理：成功 */  });
                    }
                }
            } else {
                //删除缓存数据
 ...
 * @param {*} callback 回调
 */
export const listSingleOperator = function (props, pageCode, tableCode, url, record, pkName, rowIndex, actionname, datasource, callback) {
    ...
    listSingleOperatorNoRecord.call(this,props, pageCode, tableCode, url, pk, ts, rowIndex, actionname, datasource, callback);
}
