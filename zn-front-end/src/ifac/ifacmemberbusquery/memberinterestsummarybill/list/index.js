...
export default class Test extends Component {
  ...
  disposeSearch(meta, props) {
    ...
    meta[searchId].items.find((e) => e.attrcode === 'pk_interest').isMultiSelectedEnabled = true;
    // 参照过滤
        ...
        if (item.attrcode == 'pk_varieties') {
          ...
          item.queryCondition = () => {
            return {
              ...
              GridRefActionExt: 'nccloud.web.ifac.memberintersumbill.filter.NCCPkVarietiesFilter'
            }
          }
        }
        ...
        if (item.attrcode == 'pk_fundorg') {
          ...
          item.queryCondition = () => {
            return {
              ...
              TreeRefActionExt: 'nccloud.web.ifac.memberintersumbill.filter.NCCPkDepositorgFilter'
            }
          }
        }
         ...
         if (item.attrcode == 'pk_depositcode') {
          ...
          item.queryCondition = () => {
            return {
              pk_org:(props.search.getSearchValByField(searchId, 'pk_fundorg') || {}).value.firstvalue,
              pk_fundorg:(props.search.getSearchValByField(searchId, 'pk_org') || {}).value.firstvalue,
              pk_currtype:(props.search.getSearchValByField(searchId, 'pk_currtype') || {}).value.firstvalue,
              GridRefActionExt: 'nccloud.web.ifac.memberintersumbill.filter.NCCPkDepositreceiptFilter'
            }
          }
        }
        //根据资金组织、存款单位过滤清单编号
        if (item.attrcode == 'pk_interest') {
          ...
          item.queryCondition = () => {
            return {
              pk_org:(props.search.getSearchValByField(searchId, 'pk_org') || {}).value.firstvalue,
              pk_fundorg:(props.search.getSearchValByField(searchId, 'pk_fundorg') || {}).value.firstvalue,
              pk_depositcode:(props.search.getSearchValByField(searchId, 'pk_depositcode') || {}).value.firstvalue,
              GridRefActionExt: 'nccloud.web.ifac.memberintersumbill.filter.NCCPkInterestFilter'
            }
          }
        }
      });
